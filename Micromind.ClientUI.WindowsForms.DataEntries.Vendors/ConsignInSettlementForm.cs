...
{
	public class ConsignInSettlementForm : Form, IForm
	{
		...
		private ConsignInSettlementData currentData;

		private const string TABLENAME_CONST = "ConsignIn_Settlement";
		...
		private XPButton buttonBillings;

		private XPButton buttonLoadSales;

		private AmountTextBox textBoxCommissionAmount;
		...
		public ConsignInSettlementForm()
		{
			InitializeComponent();
			AddEvents();
		...
		private void AddEvents()
		{
			...
			textBoxCommissionPercent.Validated += textBoxDiscountPercent_Validated;
		...
		private void HideShowFCColumns()
		{
			...
			dataGridItems.DisplayLayout.Bands[0].Columns["AmountFC"].Header.Caption = "Amount (" + comboBoxCurrency.SelectedID + ")";
			dataGridExpense.DisplayLayout.Bands[0].Columns["AmountFC"].Header.Caption = "Amount (" + comboBoxCurrency.SelectedID + ")";
			if (trackDetailedSales)
			{
				dataGridItems.DisplayLayout.Bands[1].Columns["AmountFC"].Header.Caption = "Amount (" + comboBoxCurrency.SelectedID + ")";
			}
			dataGridItems.DisplayLayout.Bands[0].Columns["AmountFC"].Hidden = false;
			dataGridExpense.DisplayLayout.Bands[0].Columns["AmountFC"].Hidden = false;
			if (trackDetailedSales)
			{
				dataGridItems.DisplayLayout.Bands[1].Columns["AmountFC"].Hidden = false;
			}
		}
		...
		{
			decimal d = decimal.Parse(textBoxExpenses.Text, NumberStyles.Any);
			decimal.Parse(textBoxSubtotal.Text, NumberStyles.Any);
			_ = (d == 0m);
		}
		...
		private void comboBoxVendor_SelectedIndexChanged(object sender, EventArgs e)
		{
			if (!isDataLoading && comboBoxVendor.SelectedID != "")
			{
				...
				textBoxCommissionPercent.Text = comboBoxVendor.ConsignComPercent.ToString();
				string text2 = Factory.DatabaseSystem.GetFieldValue("Vendor", "PaymentTermID", "VendorID", comboBoxVendor.SelectedID).ToString();
				if (text2 != "")
				{
		...
		private void dataGridItems_AfterCellUpdate(object sender, CellEventArgs e)
		{
			try
			{
				...
				if (e.Cell.Column.Key == "Item Code")
				{
					...
					decimal productSalesPrice = Factory.ProductSystem.GetProductSalesPrice(comboBoxGridItem.SelectedID, comboBoxVendor.SelectedID);
			...
			else if (key == "Amount" && dataGridItems.ActiveCell != null && dataGridItems.ActiveCell.Column.Key == "Amount")
			{
				...
				ItemTypes itemTypes = (ItemTypes)checked((!trackDetailedSales) ? ((byte)int.Parse(dataGridItems.ActiveRow.Cells["ItemType"].Value.ToString())) : ((byte)int.Parse(dataGridItems.ActiveRow.ParentRow.Cells["ItemType"].Value.ToString())));
		...
		private bool GetData()
		{
			try
			{
				...
				{
					currentData = new ConsignInSettlementData();
				}
				DataRow dataRow = (!isNewRecord) ? currentData.ConsignInSettlementTable.Rows[0] : currentData.ConsignInSettlementTable.NewRow();
				...
				dataRow["CommissionAmount"] = textBoxCommissionAmount.Text;
				dataRow["DueDate"] = dateTimePickerDueDate.Value;
				dataRow["Total"] = decimal.Parse(textBoxTotal.Text, NumberStyles.Any);
				dataRow.EndEdit();
				if (IsNewRecord)
				{
					currentData.ConsignInSettlementTable.Rows.Add(dataRow);
				}
				currentData.ConsignInSettlementDetailTable.Rows.Clear();
				foreach (UltraGridRow row in dataGridItems.Rows)
				{
					if (trackDetailedSales)
					{
						...
						{
							DataRow dataRow2 = currentData.ConsignInSettlementDetailTable.NewRow();
							dataRow2.BeginEdit();
							if (row2.Cells["Quantity"].Value != null && !(row2.Cells["Quantity"].Value.ToString() == "") && !(decimal.Parse(row2.Cells["Quantity"].Value.ToString()) == 0m))
							{
								...
								dataRow2["AmountFC"] = row2.Cells["AmountFC"].Value.ToString();
								if (row.Cells["Description"].Value.ToString() == "")
								{
								...
								dataRow2.EndEdit();
								currentData.ConsignInSettlementDetailTable.Rows.Add(dataRow2);
					...
					{
						DataRow dataRow5 = currentData.ConsignInSettlementDetailTable.NewRow();
						dataRow5.BeginEdit();
						...
						if (!comboBoxCurrency.IsBaseCurrency)
						{
							dataRow5["AmountFC"] = row.Cells["AmountFC"].Value.ToString();
						}
						dataRow5["Description"] = row.Cells["Description"].Value.ToString();
						dataRow5["RowIndex"] = row.Index;
						dataRow5.EndEdit();
						currentData.ConsignInSettlementDetailTable.Rows.Add(dataRow5);
					}
				}
				...
				{
					DataRow dataRow6 = currentData.ExpenseTable.NewRow();
					dataRow6.BeginEdit();
					dataRow6["ExpenseID"] = row4.Cells["Expense"].Value.ToString();
					dataRow6["Description"] = row4.Cells["Description"].Value.ToString();
					dataRow6["Reference"] = row4.Cells["Ref"].Value.ToString();
					if (row4.Cells["SourceRowIndex"].Value != null && row4.Cells["SourceRowIndex"].Value.ToString() != "")
					{
						dataRow6["SourceRowIndex"] = row4.Cells["SourceRowIndex"].Value.ToString();
						dataRow6["SourceSysDocID"] = row4.Cells["SourceSysDocID"].Value.ToString();
						dataRow6["SourceVoucherID"] = row4.Cells["SourceVoucherID"].Value.ToString();
					}
					dataRow6["RowIndex"] = row4.Index;
					dataRow6["Amount"] = row4.Cells["Amount"].Value.ToString();
					if (!row4.Cells["Billable"].Value.IsDBNullOrEmpty())
					{
						dataRow6["BillableAmount"] = row4.Cells["Billable"].Value.ToString();
					}
					dataRow6["AmountFC"] = row4.Cells["AmountFC"].Value.ToString();
					dataRow6["CurrencyID"] = comboBoxCurrency.SelectedID;
					dataRow6["CurrencyRate"] = comboBoxCurrency.Rate;
					dataRow6.EndEdit();
					currentData.ExpenseTable.Rows.Add(dataRow6);
				}
				DataSet obj = dataGridItems.DataSource as DataSet;
				currentData.ConsignInSettledItemsTable.Rows.Clear();
				foreach (DataRow row5 in obj.Tables[2].Rows)
				{
					DataRow dataRow8 = currentData.ConsignInSettledItemsTable.NewRow();
					dataRow8.BeginEdit();
					dataRow8["SysDocID"] = comboBoxSysDoc.SelectedID;
					dataRow8["VoucherID"] = textBoxVoucherNumber.Text;
					dataRow8["SalesRowID"] = row5["SalesRowID"];
					dataRow8["ProductID"] = row5["ProductID"];
					dataRow8["UnitPrice"] = row5["SoldPrice"];
					dataRow8["Quantity"] = row5["SoldQty"];
					dataRow8.EndEdit();
					currentData.ConsignInSettledItemsTable.Rows.Add(dataRow8);
				}
				return true;
			}
			catch (Exception e)
		...
		private void SetupGrid()
		{
			try
			{
				...
				if (trackDetailedSales)
				{
					...
					dataTable2.Columns.Add("AmountFC", typeof(decimal));
					dataSet.Tables.Add(dataTable2);
					dataSet.Relations.Add("REL", dataTable.Columns["Item Code"], dataTable2.Columns["Item Code"], createConstraints: true);
				...
				HideShowFCColumns();
				DataTable dataTable3 = new DataTable("ConsignIn_Settled_Items");
				dataTable3.Columns.Add("SalesRowID", typeof(int));
				dataTable3.Columns.Add("ProductID");
				dataTable3.Columns.Add("SoldQty", typeof(decimal));
				dataTable3.Columns.Add("SoldPrice", typeof(decimal));
				...
				dataGridItems.DisplayLayout.Bands[0].Columns["ExpenseAmount"].Hidden = true;
				...
				dataGridItems.DisplayLayout.Bands[0].Columns["AmountFC"].CellActivation = Activation.NoEdit;
				...
				dataGridItems.DisplayLayout.Bands[0].Columns["AmountFC"].CellAppearance.BackColor = Color.WhiteSmoke;
				dataGridItems.DisplayLayout.Bands[0].Columns["Amount"].CellAppearance.BackColor = Color.WhiteSmoke;
				dataGridItems.DisplayLayout.Bands[0].Columns["Quantity"].CellAppearance.BackColor = Color.WhiteSmoke;
				dataGridItems.DisplayLayout.Bands[0].Columns["Price"].CellAppearance.BackColor = Color.WhiteSmoke;
				dataGridItems.DisplayLayout.Bands[0].Columns["AmountFC"].TabStop = false;
				dataGridItems.DisplayLayout.Bands[0].Columns["AmountFC"].Header.Caption = "Amount(" + comboBoxCurrency.SelectedID + ")";
				if (trackDetailedSales)
				{
					dataGridItems.DisplayLayout.Bands[1].Columns["AmountFC"].Header.Caption = "Amount(" + comboBoxCurrency.SelectedID + ")";
					dataGridItems.DisplayLayout.Bands[1].Columns["AmountFC"].CellActivation = Activation.NoEdit;
					dataGridItems.DisplayLayout.Bands[1].Columns["SoldQty"].CellActivation = Activation.NoEdit;
					dataGridItems.DisplayLayout.Bands[1].Columns["SoldPrice"].CellActivation = Activation.NoEdit;
					AppearanceBase cellAppearance = dataGridItems.DisplayLayout.Bands[1].Columns["SoldQty"].CellAppearance;
					AppearanceBase cellAppearance2 = dataGridItems.DisplayLayout.Bands[1].Columns["AmountFC"].CellAppearance;
					Color color = dataGridItems.DisplayLayout.Bands[1].Columns["SoldPrice"].CellAppearance.BackColor = Color.WhiteSmoke;
					Color color4 = cellAppearance.BackColor = (cellAppearance2.BackColor = color);
					UltraGridColumn ultraGridColumn = dataGridItems.DisplayLayout.Bands[1].Columns["SoldQty"];
					bool tabStop = dataGridItems.DisplayLayout.Bands[1].Columns["SoldPrice"].TabStop = false;
					ultraGridColumn.TabStop = tabStop;
					dataGridItems.DisplayLayout.Bands[0].Columns["Quantity"].CellActivation = Activation.NoEdit;
					dataGridItems.DisplayLayout.Bands[1].Override.AllowAddNew = AllowAddNew.Yes;
					dataGridItems.DisplayLayout.Bands[1].Columns["Item Code"].Hidden = true;
		...
		private void SetupExpenseGrid()
		{
			...
			dataTable.Columns.Add("Billable", typeof(decimal));
			dataTable.Columns.Add("Amount", typeof(decimal));
			dataTable.Columns.Add("AmountFC", typeof(decimal));
			...
			dataGridExpense.DisplayLayout.Bands[0].Columns["SourceSysDocID"].Header.Caption = "Bill DocID";
			dataGridExpense.DisplayLayout.Bands[0].Columns["SourceVoucherID"].Header.Caption = "Bill No";
			dataGridExpense.DisplayLayout.Bands[0].Columns["SourceRowIndex"].Hidden = true;
			...
			UltraGridColumn ultraGridColumn2 = dataGridExpense.DisplayLayout.Bands[0].Columns["Billable"];
			Activation activation2 = dataGridExpense.DisplayLayout.Bands[0].Columns["SourceSysDocID"].CellActivation = Activation.NoEdit;
			Activation activation5 = ultraGridColumn.CellActivation = (ultraGridColumn2.CellActivation = activation2);
			AppearanceBase cellAppearance = dataGridExpense.DisplayLayout.Bands[0].Columns["SourceVoucherID"].CellAppearance;
			AppearanceBase cellAppearance2 = dataGridExpense.DisplayLayout.Bands[0].Columns["Billable"].CellAppearance;
			Color color = dataGridExpense.DisplayLayout.Bands[0].Columns["SourceSysDocID"].CellAppearance.BackColor = Color.WhiteSmoke;
			Color color4 = cellAppearance.BackColor = (cellAppearance2.BackColor = color);
			AppearanceBase cellAppearance3 = dataGridExpense.DisplayLayout.Bands[0].Columns["SourceVoucherID"].CellAppearance;
			AppearanceBase cellAppearance4 = dataGridExpense.DisplayLayout.Bands[0].Columns["Billable"].CellAppearance;
			color = (dataGridExpense.DisplayLayout.Bands[0].Columns["SourceSysDocID"].CellAppearance.BackColorDisabled = Color.WhiteSmoke);
			...
			dataGridExpense.DisplayLayout.Bands[0].Columns["Billable"].MaxLength = 20;
			dataGridExpense.DisplayLayout.Bands[0].Columns["Billable"].CellAppearance.TextHAlign = HAlign.Right;
			dataGridExpense.DisplayLayout.Bands[0].Columns["Billable"].Format = Format.GridAmountFormat;
			dataGridExpense.DisplayLayout.Bands[0].Columns["Billable"].MinValue = new decimal(-999999999999L);
			dataGridExpense.DisplayLayout.Bands[0].Columns["Billable"].MaxValue = new decimal(999999999999L);
			if (dataGridExpense.DisplayLayout.Bands[0].Summaries.Count == 0)
			{
				...
				dataGridExpense.DisplayLayout.Bands[0].Summaries.Add("Billable", SummaryType.Sum, dataGridExpense.DisplayLayout.Bands[0].Columns["Billable"], SummaryPosition.UseSummaryPositionColumn);
				dataGridExpense.DisplayLayout.Bands[0].Summaries["Billable"].Appearance.BackColor = Color.White;
				dataGridExpense.DisplayLayout.Bands[0].Summaries["Billable"].Appearance.TextHAlign = HAlign.Right;
				dataGridExpense.DisplayLayout.Bands[0].Summaries["Billable"].SummaryDisplayArea = SummaryDisplayAreas.BottomFixed;
				dataGridExpense.DisplayLayout.Bands[0].Summaries["Billable"].DisplayFormat = "{0:n}";
			}
		...
		private void AdjustGridColumnSettings()
		{
			...
			dataGridItems.DisplayLayout.Bands[0].Columns["AmountFC"].MinValue = 0;
			dataGridItems.DisplayLayout.Bands[0].Columns["AmountFC"].MaxValue = new decimal(999999999999L);
			dataGridItems.DisplayLayout.Bands[0].Columns["AmountFC"].CellAppearance.TextHAlign = HAlign.Right;
			dataGridItems.DisplayLayout.Bands[0].Columns["AmountFC"].Format = Format.GridAmountFormat;
			dataGridItems.DisplayLayout.Bands[0].Columns["AmountFC"].CellActivation = Activation.NoEdit;
			Color color2 = dataGridItems.DisplayLayout.Bands[0].Columns["AmountFC"].CellAppearance.BackColor = (dataGridItems.DisplayLayout.Bands[0].Columns["AmountFC"].CellAppearance.BackColorDisabled = Color.WhiteSmoke);
			if (trackDetailedSales)
			{
				...
				dataGridItems.DisplayLayout.Bands[1].Columns["AmountFC"].MinValue = 0;
				dataGridItems.DisplayLayout.Bands[1].Columns["AmountFC"].MaxValue = new decimal(999999999999L);
				dataGridItems.DisplayLayout.Bands[1].Columns["AmountFC"].CellAppearance.TextHAlign = HAlign.Right;
				dataGridItems.DisplayLayout.Bands[1].Columns["AmountFC"].Format = Format.GridAmountFormat;
				dataGridItems.DisplayLayout.Bands[1].Columns["AmountFC"].CellActivation = Activation.NoEdit;
				color2 = (dataGridItems.DisplayLayout.Bands[1].Columns["AmountFC"].CellAppearance.BackColor = (dataGridItems.DisplayLayout.Bands[1].Columns["AmountFC"].CellAppearance.BackColorDisabled = Color.WhiteSmoke));
				dataGridItems.DisplayLayout.Bands[1].Columns["SoldQty"].MaxLength = 20;
				dataGridItems.DisplayLayout.Bands[1].Columns["SoldQty"].CellAppearance.TextHAlign = HAlign.Right;
				dataGridItems.DisplayLayout.Bands[1].Columns["SoldQty"].Format = "0.####";
				dataGridItems.DisplayLayout.Bands[1].Columns["SoldQty"].MinValue = -99999999m;
				dataGridItems.DisplayLayout.Bands[1].Columns["SoldQty"].MaxValue = 99999999m;
				dataGridItems.DisplayLayout.Bands[1].Columns["SoldPrice"].MaxLength = 20;
				dataGridItems.DisplayLayout.Bands[1].Columns["SoldPrice"].CellAppearance.TextHAlign = HAlign.Right;
				dataGridItems.DisplayLayout.Bands[1].Columns["SoldPrice"].Format = "#,0.00##";
				dataGridItems.DisplayLayout.Bands[1].Columns["SoldPrice"].MinValue = 0;
				dataGridItems.DisplayLayout.Bands[1].Columns["SoldPrice"].MaxValue = new decimal(999999999999L);
			}
			...
			if (dataGridItems.DisplayLayout.Bands[0].Summaries.Count == 0)
			{
				...
				dataGridItems.DisplayLayout.Bands[0].Summaries.Add("TotalAmt", SummaryType.Sum, dataGridItems.DisplayLayout.Bands[0].Columns["Amount"], SummaryPosition.UseSummaryPositionColumn);
				dataGridItems.DisplayLayout.Bands[0].Summaries["TotalAmt"].Appearance.BackColor = Color.White;
				dataGridItems.DisplayLayout.Bands[0].Summaries["TotalAmt"].Appearance.TextHAlign = HAlign.Right;
				dataGridItems.DisplayLayout.Bands[0].Summaries["TotalAmt"].SummaryDisplayArea = SummaryDisplayAreas.BottomFixed;
				dataGridItems.DisplayLayout.Bands[0].Summaries["TotalAmt"].DisplayFormat = "{0:n}";
				dataGridItems.DisplayLayout.Bands[0].Summaries.Add("TotalFC", SummaryType.Sum, dataGridItems.DisplayLayout.Bands[0].Columns["AmountFC"], SummaryPosition.UseSummaryPositionColumn);
				dataGridItems.DisplayLayout.Bands[0].Summaries["TotalFC"].Appearance.BackColor = Color.White;
				dataGridItems.DisplayLayout.Bands[0].Summaries["TotalFC"].Appearance.TextHAlign = HAlign.Right;
				dataGridItems.DisplayLayout.Bands[0].Summaries["TotalFC"].SummaryDisplayArea = SummaryDisplayAreas.BottomFixed;
				dataGridItems.DisplayLayout.Bands[0].Summaries["TotalFC"].DisplayFormat = "{0:n}";
			}
			if (trackDetailedSales && dataGridItems.DisplayLayout.Bands[1].Summaries.Count == 0)
			{
				dataGridItems.DisplayLayout.Bands[1].Summaries.Add("Qty", SummaryType.Sum, dataGridItems.DisplayLayout.Bands[1].Columns["Quantity"], SummaryPosition.UseSummaryPositionColumn);
				...
				dataGridItems.DisplayLayout.Bands[1].Summaries.Add("AmountFC", SummaryType.Sum, dataGridItems.DisplayLayout.Bands[1].Columns["AmountFC"], SummaryPosition.UseSummaryPositionColumn);
				dataGridItems.DisplayLayout.Bands[1].Summaries["Qty"].Appearance.BackColor = Color.White;
				dataGridItems.DisplayLayout.Bands[1].Summaries["Qty"].Appearance.TextHAlign = HAlign.Right;
				dataGridItems.DisplayLayout.Bands[1].Summaries["Qty"].SummaryDisplayArea = SummaryDisplayAreas.BottomFixed;
				dataGridItems.DisplayLayout.Bands[1].Summaries["Qty"].DisplayFormat = "{0:n}";
				...
				dataGridItems.DisplayLayout.Bands[1].Summaries["AmountFC"].Appearance.BackColor = Color.White;
				dataGridItems.DisplayLayout.Bands[1].Summaries["AmountFC"].Appearance.TextHAlign = HAlign.Right;
				dataGridItems.DisplayLayout.Bands[1].Summaries["AmountFC"].SummaryDisplayArea = SummaryDisplayAreas.BottomFixed;
				dataGridItems.DisplayLayout.Bands[1].Summaries["AmountFC"].DisplayFormat = "{0:n}";
			}
		}
		...
		public void LoadData(string voucherID)
		{
			try
			{
				...
				{
					currentData = Factory.ConsignInSettlementSystem.GetSettlementByID(SystemDocID, voucherID);
					if (currentData != null)
					{
						FillData();
						IsNewRecord = false;
		...
		private void FillData()
		{
			try
			{
				...
				{
					DataRow dataRow = currentData.Tables["ConsignIn_Settlement"].Rows[0];
					...
					{
						textBoxCommissionPercent.Text = "0";
					}
					...
					dataTable3 = dataSet.Tables["ConsignIn_Settled_Items"];
					if (trackDetailedSales)
					{
					...
					if (currentData.Tables.Contains("ConsignIn_Settlement_Detail") && currentData.ConsignInSettlementDetailTable.Rows.Count != 0)
					{
						decimal num = default(decimal);
						decimal num2 = default(decimal);
						foreach (DataRow row in currentData.Tables["ConsignIn_Settlement_Detail"].Rows)
						{
							string value = row["ProductID"].ToString();
							if (trackDetailedSales)
							{
								DataRow dataRow3;
								if (dataGridItems.ExistCellValue("Item Code", value) < 0)
								{
						...
						dataTable.Clear();
						foreach (DataRow row2 in currentData.Tables["ConsignIn_Expense"].Rows)
						{
							DataRow dataRow3 = dataTable.NewRow();
							dataRow3["Expense"] = row2["ExpenseID"];
							dataRow3["Description"] = row2["Description"];
							dataRow3["Ref"] = row2["Reference"];
							dataRow3["SourceRowIndex"] = row2["SourceRowIndex"];
							dataRow3["SourceSysDocID"] = row2["SourceSysDocID"];
							dataRow3["SourceVoucherID"] = row2["SourceVoucherID"];
							...
							{
								dataRow3["Billable"] = decimal.Parse(row2["BillableAmount"].ToString()).ToString(Format.TotalAmountFormat);
							}
							else if (!row2["SourceVoucherID"].IsDBNullOrEmpty())
							{
								dataRow3["Billable"] = decimal.Parse(row2["Amount"].ToString()).ToString(Format.TotalAmountFormat);
							}
							else
							{
								dataRow3["Billable"] = DBNull.Value;
							}
							dataRow3.EndEdit();
							dataTable.Rows.Add(dataRow3);
						}
						...
						foreach (DataRow row3 in currentData.Tables["ConsignIn_Settled_Items"].Rows)
						{
							DataRow dataRow6 = dataTable3.NewRow();
							dataRow6["SalesRowID"] = row3["SalesRowID"];
							dataRow6["ProductID"] = row3["ProductID"];
							dataRow6["SOldQty"] = row3["Quantity"];
							dataRow6["SoldPrice"] = row3["UnitPrice"];
							dataRow6.EndEdit();
						...
						{
							if (row4.Cells["IsDNRow"].Value != null && row4.Cells["IsDNRow"].Value.ToString() != "" && bool.Parse(row4.Cells["IsDNRow"].Value.ToString()))
							{
							...
							{
								if (!row5.Cells["SourceSysDocID"].Value.IsNullOrEmpty())
								{
									row5.Cells["Expense"].Activation = Activation.Disabled;
									row5.Cells["Expense"].Appearance.BackColorDisabled = Color.WhiteSmoke;
									row5.Cells["Expense"].Appearance.ForeColorDisabled = Color.Black;
								}
							}
							string productID = row4.Cells["Item Code"].Value.ToString();
							row4.Cells["Unit"].ValueList = comboBoxGridItem.GetProductUnitsValueList(productID);
							if (checked((byte)int.Parse(row4.Cells["ItemType"].Value.ToString())) == 4)
							{
								row4.Cells["Quantity"].Activation = Activation.Disabled;
							}
						}
		...
		private bool SaveData(bool clearAfter)
		{
			...
			try
			{
				bool flag = Factory.ConsignInSettlementSystem.CreateSettlement(currentData, !isNewRecord);
				if (!flag)
				{
		...
		private bool ValidateData()
		{
			...
			if (!IsNewRecord && !Global.IsUserAdmin && !AllowEditTransaction && Global.CurrentUser != Factory.SystemDocumentSystem.GetTransUserID("ConsignIn_Settlement", "VoucherID", SystemDocID, textBoxVoucherNumber.Text))
			{
			...
			checked
			{
				...
				if (trackDetailedSales && (dataGridItems.DataSource as DataSet).Tables["ConsignIn_Detail"].Rows.Count == 0)
				{
					ErrorHelper.InformationMessage("There should be at least one settlement row.");
					return false;
				}
				if (formManager.IsFieldDirty(textBoxVoucherNumber) && Factory.SystemDocumentSystem.ExistDocumentNumber("ConsignIn_Settlement", "VoucherID", SystemDocID, textBoxVoucherNumber.Text))
				{
					ErrorHelper.WarningMessage(UIMessages.DocumentNumberInUse);
		...
		private bool Delete()
		{
			try
			{
				...
				return Factory.ConsignInSettlementSystem.DeleteSettlement(SystemDocID, textBoxVoucherNumber.Text);
			}
			catch (CompanyException ex)
			{
				if (ex.Number == 1061)
		...
		{
			string nextID = DatabaseHelper.GetNextID("ConsignIn_Settlement", "VoucherID", textBoxVoucherNumber.Text, "SysDocID", SystemDocID);
			if (!(nextID == ""))
			{
				LoadData(nextID);
			}
		...
		{
			string previousID = DatabaseHelper.GetPreviousID("ConsignIn_Settlement", "VoucherID", textBoxVoucherNumber.Text, "SysDocID", SystemDocID);
			if (!(previousID == ""))
			{
				LoadData(previousID);
			}
		...
		{
			string lastID = DatabaseHelper.GetLastID("ConsignIn_Settlement", "VoucherID", "SysDocID", SystemDocID);
			if (!(lastID == ""))
			{
				LoadData(lastID);
			}
		...
		{
			string firstID = DatabaseHelper.GetFirstID("ConsignIn_Settlement", "VoucherID", "SysDocID", SystemDocID);
			if (!(firstID == ""))
			{
				LoadData(firstID);
			}
		...
		private void toolStripButtonFind_Click(object sender, EventArgs e)
		{
			try
			{
				...
				{
					string text = Factory.DatabaseSystem.FindDocumentByNumber("ConsignIn_Settlement", "VoucherID", SystemDocID, toolStripTextBoxFind.Text);
					if (text != "")
					{
						LoadData(text);
					}
		...
		private void Form_Load(object sender, EventArgs e)
		{
			try
			{
				...
				comboBoxSysDoc.FilterByType(SysDocTypes.ConsignInSettlement);
				SetSecurity();
				IsNewRecord = true;
				if (!base.IsDisposed)
				{
		...
		private bool Void(bool isVoid)
		{
			try
			{
				...
				return Factory.ConsignInSettlementSystem.VoidSettlement(SystemDocID, textBoxVoucherNumber.Text, isVoid);
			}
			catch (CompanyException ex)
			{
				if (ex.Number == 1061)
		...
		{
			new FormHelper().EditSysDoc(comboBoxSysDoc.SelectedID, SysDocTypes.ConsignInSettlement);
		}
		...
		private void CalculateTotal()
		{
			...
			if (trackDetailedSales)
			{
				foreach (UltraGridRow row in dataGridItems.Rows)
				{
					...
					foreach (UltraGridRow row2 in row.ChildBands[0].Rows)
					{
						...
						{
							decimal num5 = decimal.Parse(row2.Cells["Quantity"].Value.ToString());
							decimal.TryParse(row2.Cells["Amount"].Value.ToString(), out result);
							decimal num6 = Math.Round(comboBoxCurrency.GetFCEquivalant(result), Global.CurDecimalPoints);
							row2.Cells["AmountFC"].Value = num6;
							num2 += num5;
							num3 += result;
							num4 += num6;
							d += num6;
					...
					if (num2 != 0m)
					{
						row.Cells["Price"].Value = Math.Round(num3 / num2, 5);
					}
					else
					{
						row.Cells["Price"].Value = 0;
					}
					row.Cells["Amount"].Value = num3;
					row.Cells["AmountFC"].Value = num4;
				}
			}
			else
			{
				foreach (UltraGridRow row3 in dataGridItems.Rows)
				{
					if (row3.Cells["Amount"].Value != null && !(row3.Cells["Amount"].Value.ToString() == ""))
					{
						...
						decimal num7 = decimal.Parse(row3.Cells["Quantity"].Value.ToString());
						decimal num8 = Math.Round(comboBoxCurrency.GetFCEquivalant(result), Global.CurDecimalPoints);
						row3.Cells["AmountFC"].Value = num8;
						num2 += num7;
						d += num8;
					}
				}
			...
			decimal result2 = default(decimal);
			decimal.TryParse(textBoxCommissionPercent.Text, out result2);
			decimal d3 = Math.Round(d * result2 / 100m);
			textBoxCommissionAmount.Text = d3.ToString();
			foreach (UltraGridRow row4 in dataGridExpense.Rows)
			{
				if (row4.Cells["Amount"].Value != null && !(row4.Cells["Amount"].Value.ToString() == ""))
				{
					...
					decimal num9 = Math.Round(comboBoxCurrency.GetFCEquivalant(result), Global.CurDecimalPoints);
					row4.Cells["AmountFC"].Value = num9;
					d2 += num9;
				}
			}
		...
		private void Print(bool isPrint, bool showPrintDialog, bool saveChanges)
		{
			try
			{
				if (!(IsDirty && saveChanges) || (ErrorHelper.QuestionMessage(MessageBoxButtons.YesNo, "You must save the document before printing.", "Do you want to save?") == DialogResult.Yes && SaveData(clearAfter: false)))
				{
					...
					DataSet settlementToPrint = Factory.ConsignInSettlementSystem.GetSettlementToPrint(selectedID, text);
					...
					{
						PrintHelper.PrintDocument(settlementToPrint, selectedID, "Consignment In Settlement", SysDocTypes.ConsignInSettlement, isPrint, showPrintDialog);
					}
				}
			}
			catch (Exception e)
		...
		private void buttonSelectConsignment_Click(object sender, EventArgs e)
		{
			...
			if (selectDocumentDialog.SelectedRows.Count > 0)
			{
				...
				string text = ultraGridRow.Cells["Number"].Value.ToString();
				textBoxConsignment.Text = text;
				consignSysDocID = sysDocID;
				consignVoucherID = text;
				buttonLoadSales.Enabled = true;
				num = checked(num + 1);
		...
		private void ConsignInSettlementForm_Load(object sender, EventArgs e)
		{
		}
		...
		private void buttonBillings_Click(object sender, EventArgs e)
		{
			checked
			{
				try
				{
					...
					{
						SelectConsignInBillingDialog selectConsignInBillingDialog = new SelectConsignInBillingDialog();
						selectConsignInBillingDialog.ConsignSysDocID = consignSysDocID;
						selectConsignInBillingDialog.ConsignVoucherID = consignVoucherID;
						selectConsignInBillingDialog.SelectedItemsTable = (dataGridExpense.DataSource as DataTable);
						if (selectConsignInBillingDialog.ShowDialog() == DialogResult.OK)
						{
							...
							if (selectConsignInBillingDialog.SelectedRows.Count > 0)
							{
							...
							foreach (UltraGridRow selectedRow in selectConsignInBillingDialog.SelectedRows)
							{
								...
								dataRow["Billable"] = selectedRow.Cells["Amount"].Value.ToString();
							...
							foreach (UltraGridRow row in dataGridExpense.Rows)
							{
								...
								{
									row.Cells["Expense"].Activation = Activation.NoEdit;
									row.Cells["Expense"].Appearance.BackColor = Color.WhiteSmoke;
									row.Cells["Expense"].Appearance.BackColorDisabled = Color.WhiteSmoke;
									Activation activation3 = row.Cells["Amount"].Activation = (row.Cells["Description"].Activation = Activation.AllowEdit);
								}
							}
						}
						CalculateTotal();
		...
		private void buttonLoadSales_Click(object sender, EventArgs e)
		{
			try
			{
				if (!(consignVoucherID == ""))
				{
					DataSet consignInSoldItems = Factory.ConsignInSettlementSystem.GetConsignInSoldItems(consignSysDocID, consignVoucherID);
					DataSet obj = (DataSet)dataGridItems.DataSource;
					DataTable dataTable = obj.Tables[1];
					DataTable dataTable2 = obj.Tables["ConsignIn_Settled_Items"];
					DataTable dataTable3 = obj.Tables[0];
					dataTable.Rows.Clear();
					foreach (DataRow row in consignInSoldItems.Tables[0].Rows)
					{
						string str = row["ItemCode"].ToString();
						if (dataTable3.Select("[Item Code] = '" + str + "'").Length != 0)
						{
							DataRow dataRow2 = dataTable.NewRow();
							decimal result = default(decimal);
							decimal result2 = default(decimal);
							decimal result3 = default(decimal);
							decimal.TryParse(row["QtyInvoiced"].ToString(), out result);
							decimal.TryParse(row["UnitPrice"].ToString(), out result2);
							decimal.TryParse(row["FOC"].ToString(), out result3);
							if (!(result < 0m))
							{
								dataRow2["Item Code"] = row["ItemCode"];
								object obj4 = dataRow2["Price"] = (dataRow2["SoldPrice"] = row["UnitPrice"]);
								obj4 = (dataRow2["Quantity"] = (dataRow2["SoldQty"] = row["QtyInvoiced"]));
								dataRow2["Description"] = "Sales";
								...
								if (result3 > 0m)
								{
									...
									dataRow2["Item Code"] = row["ItemCode"];
									obj4 = (dataRow2["Price"] = (dataRow2["SoldPrice"] = 0));
									obj4 = (dataRow2["Quantity"] = (dataRow2["SoldQty"] = result3));
									dataRow2["Description"] = "FOC";
									dataRow2["Amount"] = 0;
									dataTable.Rows.Add(dataRow2);
								}
								dataRow2 = dataTable2.NewRow();
								dataRow2["ProductID"] = row["ItemCode"];
								dataRow2["SalesRowID"] = row["RecordID"];
								dataRow2["SoldPrice"] = row["UnitPrice"];
								dataRow2["SoldQty"] = row["QtyInvoiced"];
		...
		{
			FormActivator.BringFormToFront(FormActivator.ConsignInSettlementListFormObj);
		}
		...
		private void InitializeComponent()
		{
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Micromind.ClientUI.WindowsForms.DataEntries.Vendors.ConsignInSettlementForm));
			tabPageGeneral = new Infragistics.Win.UltraWinTabControl.UltraTabPageControl();
			buttonLoadSales = new Micromind.UISupport.XPButton();
			...
			buttonBillings = new Micromind.UISupport.XPButton();
			...
			textBoxCommissionAmount = new Micromind.UISupport.AmountTextBox();
			...
			SuspendLayout();
			tabPageGeneral.Controls.Add(buttonLoadSales);
			...
			tabPageGeneral.Size = new System.Drawing.Size(776, 279);
			buttonLoadSales.AdjustImageLocation = new System.Drawing.Point(0, 0);
			buttonLoadSales.Anchor = (System.Windows.Forms.AnchorStyles.Top | System.Windows.Forms.AnchorStyles.Right);
			buttonLoadSales.BackColor = System.Drawing.Color.DarkGray;
			buttonLoadSales.BtnShape = Micromind.UISupport.BtnShape.Rectangle;
			buttonLoadSales.BtnStyle = Micromind.UISupport.XPStyle.Default;
			buttonLoadSales.ImeMode = System.Windows.Forms.ImeMode.NoControl;
			buttonLoadSales.Location = new System.Drawing.Point(672, 6);
			buttonLoadSales.Name = "buttonLoadSales";
			buttonLoadSales.Size = new System.Drawing.Size(100, 22);
			buttonLoadSales.TabIndex = 123;
			buttonLoadSales.Text = "Load Sales...";
			buttonLoadSales.UseVisualStyleBackColor = false;
			buttonLoadSales.Click += new System.EventHandler(buttonLoadSales_Click);
			...
			dataGridItems.Location = new System.Drawing.Point(4, 30);
			...
			dataGridItems.Size = new System.Drawing.Size(769, 246);
			...
			tabPageDetails.Controls.Add(buttonBillings);
			...
			tabPageDetails.Size = new System.Drawing.Size(776, 279);
			buttonBillings.AdjustImageLocation = new System.Drawing.Point(0, 0);
			buttonBillings.BackColor = System.Drawing.Color.DarkGray;
			buttonBillings.BtnShape = Micromind.UISupport.BtnShape.Rectangle;
			buttonBillings.BtnStyle = Micromind.UISupport.XPStyle.Default;
			buttonBillings.ImeMode = System.Windows.Forms.ImeMode.NoControl;
			buttonBillings.Location = new System.Drawing.Point(672, 5);
			buttonBillings.Name = "buttonBillings";
			buttonBillings.Size = new System.Drawing.Size(100, 22);
			buttonBillings.TabIndex = 7;
			buttonBillings.Text = "Billables...";
			buttonBillings.UseVisualStyleBackColor = false;
			buttonBillings.Click += new System.EventHandler(buttonBillings_Click);
			...
			dataGridExpense.Location = new System.Drawing.Point(3, 29);
			...
			dataGridExpense.Size = new System.Drawing.Size(770, 245);
			...
			comboBoxGridExpenseCode.ExpenseCodeType = Micromind.Common.Data.ExpenseCodeTypes.ConsignmentIn;
			...
			textBoxTotalExpense.Location = new System.Drawing.Point(635, 254);
			...
			panelDetails.Size = new System.Drawing.Size(749, 121);
			panelDetails.TabIndex = 0;
			appearance75.FontData.BoldAsString = "False";
			appearance75.FontData.Name = "Tahoma";
			ultraFormattedLinkLabel3.Appearance = appearance75;
			ultraFormattedLinkLabel3.AutoSize = true;
			ultraFormattedLinkLabel3.Location = new System.Drawing.Point(472, 66);
			ultraFormattedLinkLabel3.Name = "ultraFormattedLinkLabel3";
			ultraFormattedLinkLabel3.Size = new System.Drawing.Size(33, 15);
			ultraFormattedLinkLabel3.TabIndex = 156;
			ultraFormattedLinkLabel3.TabStop = true;
			ultraFormattedLinkLabel3.TreatValueAs = Infragistics.Win.FormattedLinkLabel.TreatValueAs.URL;
			ultraFormattedLinkLabel3.UnderlineLinks = Infragistics.Win.FormattedLinkLabel.UnderlineLink.WhenHovered;
			ultraFormattedLinkLabel3.Value = "Term:";
			appearance76.ForeColor = System.Drawing.Color.Blue;
			ultraFormattedLinkLabel3.VisitedLinkAppearance = appearance76;
			...
			labelCurrency.Location = new System.Drawing.Point(472, 87);
			...
			ultraFormattedLinkLabel1.VisitedLinkAppearance = appearance80;
			...
			buttonSelectConsignment.Location = new System.Drawing.Point(362, 44);
			...
			textBoxSettleNumber.TabIndex = 8;
			...
			panel1.Controls.Add(textBoxCommissionAmount);
			panel1.Controls.Add(label6);
			panel1.Controls.Add(label10);
			panel1.Controls.Add(textBoxCommissionPercent);
			...
			panel1.TabIndex = 3;
			textBoxCommissionAmount.AllowDecimal = true;
			textBoxCommissionAmount.BackColor = System.Drawing.Color.WhiteSmoke;
			textBoxCommissionAmount.CustomReportFieldName = "";
			textBoxCommissionAmount.CustomReportKey = "";
			textBoxCommissionAmount.CustomReportValueType = 1;
			textBoxCommissionAmount.IsComboTextBox = false;
			textBoxCommissionAmount.IsModified = false;
			textBoxCommissionAmount.Location = new System.Drawing.Point(140, 21);
			textBoxCommissionAmount.MaxValue = new decimal(new int[4]
			{
				-1,
				-1,
				-1,
			...
			textBoxCommissionAmount.MinValue = new decimal(new int[4]
			{
				-1,
				-1,
				-1,
			...
			textBoxCommissionAmount.Name = "textBoxCommissionAmount";
			textBoxCommissionAmount.NullText = "0";
			textBoxCommissionAmount.ReadOnly = true;
			textBoxCommissionAmount.Size = new System.Drawing.Size(86, 20);
			textBoxCommissionAmount.TabIndex = 2;
			textBoxCommissionAmount.Text = "0.00";
			textBoxCommissionAmount.TextAlign = System.Windows.Forms.HorizontalAlignment.Right;
			textBoxCommissionAmount.Value = new decimal(new int[4]
			{
				0,
				0,
				0,
			...
			textBoxCommissionAmount.TextChanged += new System.EventHandler(textBoxCommissionAmount_TextChanged);
			label6.AutoSize = true;
			label6.Location = new System.Drawing.Point(123, 24);
			label6.Name = "label6";
			...
			textBoxCommissionPercent.Location = new System.Drawing.Point(88, 21);
			textBoxCommissionPercent.MaxLength = 4;
			textBoxCommissionPercent.Name = "textBoxCommissionPercent";
			textBoxCommissionPercent.Size = new System.Drawing.Size(34, 20);
			textBoxCommissionPercent.TabIndex = 1;
			textBoxCommissionPercent.Text = "0";
			...
			label8.Location = new System.Drawing.Point(4, 69);
			label8.Name = "label8";
			...
			textBoxTotal.Location = new System.Drawing.Point(87, 66);
			...
			label7.Location = new System.Drawing.Point(4, 47);
			label7.Name = "label7";
			...
			textBoxExpenses.Location = new System.Drawing.Point(88, 44);
			...
			ultraTabControl1.Location = new System.Drawing.Point(11, 158);
			ultraTabControl1.MinTabWidth = 80;
			ultraTabControl1.Name = "ultraTabControl1";
			ultraTabControl1.SharedControlsPage = ultraTabSharedControlsPage1;
			ultraTabControl1.Size = new System.Drawing.Size(778, 300);
			...
			ultraTab.Appearance = appearance121;
			...
			ultraTabSharedControlsPage1.Size = new System.Drawing.Size(776, 279);
			...
			base.Name = "ConsignInSettlementForm";
			Text = "Consignment In Settlement";
			base.Load += new System.EventHandler(ConsignInSettlementForm_Load);
