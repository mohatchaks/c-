...
{
	public class InventoryAdjustmentsForm : Form, IForm
	{
		...
		private InventoryAdjustmentData currentData;

		private const string TABLENAME_CONST = "Inventory_Adjustment";
		...
		private bool Isupdatecosting = CompanyPreferences.ActivateCostingonDelete;
		...
		private ToolStripMenuItem loadItemToolStripMenuItem;
		...
		public InventoryAdjustmentsForm()
		{
			InitializeComponent();
			AddEvents();
		...
		{
			if (e.Cell.Appearance.FontData.Underline == DefaultableBoolean.True && e.Cell.Column.Key == "Adj Qty")
			{
		...
		private bool AllocateQuantityToLot(UltraGridCell cell)
		{
			try
			{
				...
				if (result)
				{
					...
					decimal.TryParse(cell.Text.ToString(), out result3);
					if (result3 > 0m)
					{
					...
					else if (result3 < 0m)
					{
						...
						issueLotSelectionForm.RowQuantity = Math.Abs(float.Parse(dataGridItems.ActiveCell.Text));
						if (!isNewRecord)
						{
				...
				if (dataGridItems.Rows.Count != 0)
				{
					...
					foreach (UltraGridRow row2 in dataGridItems.Rows)
					{
						if (row2.Cells["Item Code"].Value != null && !(row2.Cells["Item Code"].Value.ToString() == ""))
						{
							...
							decimal.TryParse(row2.GetCellValue("Adj Qty").ToString(), out result);
		...
		private void dataGridItems_AfterCellUpdate(object sender, CellEventArgs e)
		{
			...
			if (e.Cell.Column.Key == "Item Code")
			{
				...
				if (result | result2)
				{
					dataGridItems.ActiveRow.Cells["New Qty"].Activation = Activation.Disabled;
				}
				else
				{
					dataGridItems.ActiveRow.Cells["New Qty"].Activation = Activation.AllowEdit;
				}
				if (itemTypes == ItemTypes.Matrix)
				{
					...
					if (matrixSelectionForm.ShowDialog(this) == DialogResult.OK)
					{
						foreach (DataRow row in matrixSelectionForm.SelectedItems.Tables[0].Rows)
						{
							...
							ultraGridRow.Cells["Adj Qty"].Value = row["Quantity"].ToString();
							ultraGridRow.Update();
							decimal result3 = default(decimal);
							decimal result4 = default(decimal);
							decimal.TryParse(ultraGridRow.Cells["Adj Qty"].Value.ToString(), out result3);
							decimal.TryParse(ultraGridRow.Cells["Onhand"].Value.ToString(), out result4);
							ultraGridRow.Cells["New Qty"].Value = result4 - result3;
							if (result3 < 0m)
							{
				...
				dataGridItems.ActiveRow.Cells["Adj Qty"].Value = 0;
				dataGridItems.ActiveRow.Cells["Adj Qty"].Tag = null;
				dataGridItems.ActiveRow.Cells["Adj Qty"].Appearance.FontData.Underline = DefaultableBoolean.False;
			...
			if (isNewRecord && e.Cell.Column.Key == "Adj Qty" && dataGridItems.ActiveCell != null && dataGridItems.ActiveCell.Column.Key == "Adj Qty")
			{
				float result5 = 0f;
				float result6 = 0f;
				float.TryParse(e.Cell.Row.Cells["Adj Qty"].Text.ToString(), out result6);
				float.TryParse(e.Cell.Row.Cells["Onhand"].Value.ToString(), out result5);
				e.Cell.Row.Cells["New Qty"].Value = (result5 + result6).ToString();
			}
			else if (isNewRecord && e.Cell.Column.Key == "New Qty" && dataGridItems.ActiveCell != null && dataGridItems.ActiveCell.Column.Key == "New Qty")
			{
				float result7 = 0f;
				float result8 = 0f;
				float.TryParse(e.Cell.Row.Cells["New Qty"].Text.ToString(), out result8);
				float.TryParse(e.Cell.Row.Cells["Onhand"].Value.ToString(), out result7);
				e.Cell.Row.Cells["Adj Qty"].Value = (result8 - result7).ToString();
			}
			else if (isNewRecord && e.Cell.Column.Key == "Onhand" && e.Cell.Row.Cells["Adj Qty"].Value != null && e.Cell.Row.Cells["Adj Qty"].Value.ToString() != "")
			{
				float result9 = 0f;
				float result10 = 0f;
				float.TryParse(e.Cell.Row.Cells["Adj Qty"].Text.ToString(), out result10);
				float.TryParse(e.Cell.Row.Cells["Onhand"].Value.ToString(), out result9);
				e.Cell.Row.Cells["New Qty"].Value = (result9 + result10).ToString();
			}
		...
		private void dataGrid_BeforeCellDeactivate(object sender, CancelEventArgs e)
		{
			...
			if (dataGridItems.ActiveCell.Column.Key == "Item Code")
			{
			...
			else if (!(dataGridItems.ActiveCell.Column.Key == "Cost"))
			{
				_ = (dataGridItems.ActiveCell.Column.Key == "Adj Qty");
			}
		}
		...
		private void dataGrid_BeforeRowDeactivate(object sender, CancelEventArgs e)
		{
			...
			if (activeRow.Cells["Adj Qty"].Value.ToString() == "")
			{
				activeRow.Cells["Adj Qty"].Value = 0;
			}
			labelBalance.Text = GetTransactionBalance().ToString(Format.TotalAmountFormat);
		}
		...
		{
			if (e.Cell.Column.Key == "Adj Qty" && isNewRecord)
			{
				labelBalance.Text = GetTransactionBalance().ToString(Format.TotalAmountFormat);
			}
			if (e.Cell.Column.Key == "Adj Qty" && dataGridItems.ActiveCell != null && dataGridItems.ActiveCell.Column.Key == "Adj Qty" && !AllocateQuantityToLot(e.Cell))
			{
				e.Cancel = true;
			}
		}
		...
		private void dataGrid_CellDataError(object sender, CellDataErrorEventArgs e)
		{
			...
			else if (dataGridItems.ActiveCell.Column.Key.ToString() == "Adj Qty")
			{
				e.RaiseErrorEvent = false;
				ErrorHelper.InformationMessage("Invalid quantity. Please enter a numeric value.");
			}
		...
		private bool GetData()
		{
			try
			{
				...
				{
					currentData = new InventoryAdjustmentData();
				}
				DataRow dataRow = (!isNewRecord) ? currentData.InventoryAdjustmentTable.Rows[0] : currentData.InventoryAdjustmentTable.NewRow();
				...
				if (IsNewRecord)
				{
					currentData.InventoryAdjustmentTable.Rows.Add(dataRow);
				}
				currentData.InventoryAdjustmentDetailsTable.Rows.Clear();
				currentData.Tables["Product_Lot_Receiving_Detail"].Rows.Clear();
				currentData.Tables["Product_Lot_Issue_Detail"].Rows.Clear();
				foreach (UltraGridRow row in dataGridItems.Rows)
				{
					DataRow dataRow2 = currentData.InventoryAdjustmentDetailsTable.NewRow();
					dataRow2.BeginEdit();
					dataRow2["ProductID"] = row.Cells["Item Code"].Value.ToString();
					if (row.Cells["Adj Qty"].Value != null && row.Cells["Adj Qty"].Value.ToString() != "")
					{
						dataRow2["Quantity"] = row.Cells["Adj Qty"].Value.ToString();
					}
					else
					{
						dataRow2["Quantity"] = 0;
					...
					dataRow2.EndEdit();
					currentData.InventoryAdjustmentDetailsTable.Rows.Add(dataRow2);
					string selectedID = comboBoxLocation.SelectedID;
					string text = row.Cells["Item Code"].Value.ToString();
					decimal result = default(decimal);
					decimal.TryParse(row.Cells["Adj Qty"].Value.ToString(), out result);
					if (row.Cells["Adj Qty"].Tag != null)
					{
						if (result > 0m)
						{
							foreach (DataRow row2 in (row.Cells["Adj Qty"].Tag as DataTable).Rows)
							{
						...
						{
							foreach (DataRow row3 in (row.Cells["Adj Qty"].Tag as DataTable).Rows)
							{
		...
		private void SetupGrid()
		{
			try
			{
				...
				dataTable.Columns.Add("Adj Qty", typeof(decimal));
				dataTable.Columns.Add("Cost", typeof(decimal));
				dataTable.Columns.Add("New Qty", typeof(decimal));
				...
				dataGridItems.DisplayLayout.Bands[0].Columns["Adj Qty"].MaxLength = 20;
				dataGridItems.DisplayLayout.Bands[0].Columns["Adj Qty"].CellAppearance.TextHAlign = HAlign.Right;
				dataGridItems.DisplayLayout.Bands[0].Columns["Adj Qty"].Format = "n";
				...
				dataGridItems.DisplayLayout.Bands[0].Columns["New Qty"].CellAppearance.TextHAlign = HAlign.Right;
				dataGridItems.DisplayLayout.Bands[0].Columns["New Qty"].MaxValue = 99999999m;
				dataGridItems.DisplayLayout.Bands[0].Columns["New Qty"].InvalidValueBehavior = InvalidValueBehavior.RetainValueAndFocus;
				dataGridItems.DisplayLayout.Bands[0].Columns["New Qty"].Format = "n";
				...
				dataGridItems.DisplayLayout.Bands[0].Columns["Value"].CellActivation = Activation.Disabled;
				dataGridItems.DisplayLayout.Bands[0].Columns["Value"].CellAppearance.BackColorDisabled = Color.WhiteSmoke;
				dataGridItems.DisplayLayout.Bands[0].Columns["Value"].CellAppearance.ForeColorDisabled = Color.Black;
				dataGridItems.CalcManager = new UltraCalcManager(base.Container);
				dataGridItems.DisplayLayout.Bands[0].Columns["Value"].Formula = "[Adj Qty] * [Cost]";
				...
				dataGridItems.DisplayLayout.Bands[0].Columns["Onhand"].Width = checked(10 * dataGridItems.Width) / 100;
				dataGridItems.DisplayLayout.Bands[0].Columns["Adj Qty"].Width = checked(10 * dataGridItems.Width) / 100;
				dataGridItems.DisplayLayout.Bands[0].Columns["New Qty"].Width = checked(10 * dataGridItems.Width) / 100;
				dataGridItems.DisplayLayout.Bands[0].Columns["New Qty"].Width = checked(10 * dataGridItems.Width) / 100;
				...
				dataGridItems.DisplayLayout.Bands[0].Columns["Onhand"].Format = "#,##0.0000#";
				dataGridItems.DisplayLayout.Bands[0].Summaries.Add("TotalQty", SummaryType.Sum, dataGridItems.DisplayLayout.Bands[0].Columns["Adj Qty"], SummaryPosition.UseSummaryPositionColumn);
				...
				dataGridItems.DisplayLayout.Bands[0].Summaries.Add("Value", SummaryType.Sum, dataGridItems.DisplayLayout.Bands[0].Columns["Value"], SummaryPosition.UseSummaryPositionColumn);
		...
		public void LoadData(string voucherID)
		{
			try
			{
				...
				{
					currentData = Factory.InventoryAdjustmentSystem.GetInventoryAdjustmentByID(SystemDocID, voucherID);
					if (currentData != null)
					{
						FillData();
						IsNewRecord = false;
		...
		private void FillData()
		{
			try
			{
				...
				{
					DataRow dataRow = currentData.Tables["Inventory_Adjustment"].Rows[0];
					...
					if (currentData.Tables.Contains("Inventory_Adjustment_Detail") && currentData.InventoryAdjustmentDetailsTable.Rows.Count != 0)
					{
						dataGridItems.BeginUpdate();
						foreach (DataRow row in currentData.Tables["Inventory_Adjustment_Detail"].Rows)
						{
							DataRow dataRow3 = dataTable.NewRow();
							dataRow3["Item Code"] = row["ProductID"];
							dataRow3["Adj Qty"] = row["Quantity"];
						...
						foreach (UltraGridRow row2 in dataGridItems.Rows)
						{
							decimal d = decimal.Parse(row2.Cells["Adj Qty"].Value.ToString());
							if (d < 0m)
							{
							...
							{
								row2.Cells["New Qty"].Activation = Activation.Disabled;
								if (d > 0m)
								{
									...
									if (array.Length != 0)
									{
										...
										row2.Cells["Adj Qty"].Tag = tag;
										row2.Cells["Adj Qty"].Appearance.FontData.Underline = DefaultableBoolean.True;
									}
								}
								else
								{
		...
		private bool SaveData(bool clearAfter)
		{
			...
			try
			{
				bool flag2 = Factory.InventoryAdjustmentSystem.CreateInventoryAdjustment(currentData, !isNewRecord);
				if (flag2)
				{
					flag = true;
				}
		...
		public void ShowForApproval(string sysDocID, string voucherID, int approvalTaskID)
		{
			...
			formManager.ShowApprovalPanel(approvalTaskID, "Inventory_Adjustment", "VoucherID");
		}

		private bool ValidateData()
		{
			if (!IsNewRecord && !Global.IsUserAdmin && !AllowEditTransaction && Global.CurrentUser != Factory.SystemDocumentSystem.GetTransUserID("Inventory_Adjustment", "VoucherID", SystemDocID, textBoxVoucherNumber.Text))
			{
			...
			if (!IsNewRecord)
			{
				DataSet entityApprovalStatus = Factory.EntityDocSystem.GetEntityApprovalStatus(currentData, SysDocTypes.InventoryAdjustment, Global.CurrentUser, includeApproveUser: false);
			...
			checked
			{
				...
				if (formManager.IsFieldDirty(textBoxVoucherNumber) && Factory.SystemDocumentSystem.ExistDocumentNumber("Inventory_Adjustment", "VoucherID", SystemDocID, textBoxVoucherNumber.Text))
				{
					ErrorHelper.WarningMessage(UIMessages.DocumentNumberInUse);
		...
		private decimal GetTransactionBalance()
		{
			...
			foreach (UltraGridRow row in dataGridItems.Rows)
			{
				if (row.GetCellValue("Adj Qty") != null && row.GetCellValue("Adj Qty").ToString() != "")
				{
					result += decimal.Parse(row.Cells["Adj Qty"].Value.ToString());
				}
			}
			return result;
		}
		...
		private bool Delete()
		{
			try
			{
				...
				return Factory.InventoryAdjustmentSystem.DeleteInventoryAdjustment(SystemDocID, textBoxVoucherNumber.Text, Isupdatecosting);
			}
			catch (Exception e)
			{
				ErrorHelper.ProcessError(e);
		...
		{
			string nextID = DatabaseHelper.GetNextID("Inventory_Adjustment", "VoucherID", textBoxVoucherNumber.Text, "SysDocID", SystemDocID);
			if (!(nextID == ""))
			{
				LoadData(nextID);
			}
		...
		{
			string previousID = DatabaseHelper.GetPreviousID("Inventory_Adjustment", "VoucherID", textBoxVoucherNumber.Text, "SysDocID", SystemDocID);
			if (!(previousID == ""))
			{
				LoadData(previousID);
			}
		...
		{
			string lastID = DatabaseHelper.GetLastID("Inventory_Adjustment", "VoucherID", "SysDocID", SystemDocID);
			if (!(lastID == ""))
			{
				LoadData(lastID);
			}
		...
		{
			string firstID = DatabaseHelper.GetFirstID("Inventory_Adjustment", "VoucherID", "SysDocID", SystemDocID);
			if (!(firstID == ""))
			{
				LoadData(firstID);
			}
		...
		private void toolStripButtonFind_Click(object sender, EventArgs e)
		{
			try
			{
				...
				{
					string text = Factory.DatabaseSystem.FindDocumentByNumber("Inventory_Adjustment", "VoucherID", SystemDocID, toolStripTextBoxFind.Text);
					if (text != "")
					{
						LoadData(text);
					}
		...
		private void JournalLeavesForm_Load(object sender, EventArgs e)
		{
			try
			{
				...
				SetupGrid();
				comboBoxSysDoc.FilterByType(SysDocTypes.InventoryAdjustment);
				dateTimePickerDate.Value = DateTime.Now;
				SetSecurity();
				if (!base.IsDisposed)
				{
		...
		{
			new FormHelper().EditSysDoc(comboBoxSysDoc.SelectedID, SysDocTypes.InventoryAdjustment);
		}
		...
		private bool SaveDraft()
		{
			try
			{
				if (GetData())
				{
					...
					{
						return Global.CompanySettings.SaveTransactionDraft(currentData, enterNameDialog.EnteredName, SysDocTypes.InventoryAdjustment);
					}
				}
				return false;
			}
		...
		private bool LoadDraft()
		{
			try
			{
				DataSet settingsList = Factory.SettingSystem.GetSettingsList("", 18.ToString());
				...
				if (selectDocumentDialog.ShowDialog(this) == DialogResult.OK)
				{
					...
					DataSet dataSet = Global.CompanySettings.LoadTransactionDraft(key, SysDocTypes.InventoryAdjustment);
					currentData = (dataSet as InventoryAdjustmentData);
					FillData();
				}
				return true;
			}
		...
		private void Print(bool isPrint, bool showPrintDialog, bool saveChanges)
		{
			try
			{
				if (!(IsDirty && saveChanges) || (ErrorHelper.QuestionMessage(MessageBoxButtons.YesNo, "You must save the document before printing.", "Do you want to save?") == DialogResult.Yes && SaveData(clearAfter: false)))
				{
					...
					DataSet inventoryAdjustmentToPrint = Factory.InventoryAdjustmentSystem.GetInventoryAdjustmentToPrint(selectedID, text);
					if (inventoryAdjustmentToPrint == null || inventoryAdjustmentToPrint.Tables.Count == 0)
					{
						ErrorHelper.ErrorMessage("Cannot print the document.", "Document not found.");
					}
					else
					{
						PrintHelper.PrintDocument(inventoryAdjustmentToPrint, selectedID, "Inventory Adjustment", SysDocTypes.InventoryAdjustment, isPrint, showPrintDialog);
					}
				}
			}
			catch (Exception e)
					...
					foreach (UltraGridRow row in dataGridItems.Rows)
					{
						if (row.Cells["Item Code"].Value.ToString() != "")
						{
							...
							if (Factory.DatabaseSystem.ExistFieldValue("Product", "ProductID", text))
							{
								...
								num = decimal.Parse(row.Cells["Adj Qty"].Value.ToString());
								text3 = row.Cells["BinID"].Value.ToString();
								text4 = row.Cells["RackId"].Value.ToString();
							}
						}
						DataRow[] array = null;
						array = dataSet.Tables[0].Select("[Item Code]='" + text + "' AND LotNumber='" + text2 + "'");
						if (!string.IsNullOrEmpty(text3) && !string.IsNullOrEmpty(text4))
						{
							array = dataSet.Tables[0].Select("[Item Code]='" + text + "' AND LotNumber='" + text2 + "' AND BinID='" + text3 + "' AND RackId='" + text4 + "'");
						}
						DataTable dataTable = new DataTable();
						...
						for (int i = 0; i < array.Length; i = checked(i + 1))
						{
							...
							bool flag = false;
							if (!dataTable.Columns.Contains("SoldQty") && num > 0m)
							{
						...
						row.Cells["Adj Qty"].Tag = dataTable;
						row.Cells["Adj Qty"].Appearance.FontData.Underline = DefaultableBoolean.True;
					}
		...
		private void toolStripMenuItem1_Click(object sender, EventArgs e)
		{
			if (ErrorHelper.QuestionMessageYesNo("Are you sure to duplicate this document?") == DialogResult.Yes)
			{
				...
				if (!IsDirty)
				{
					...
					foreach (UltraGridRow row in dataGridItems.Rows)
					{
						if (!string.IsNullOrEmpty(row.Cells["Adj Qty"].ToString()))
						{
							row.Cells["Adj Qty"].Tag = null;
						}
					}
				}
				else if (CanClose())
		...
		private void loadFromTransactionToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			DataSet itemTransactionList = Factory.ItemTransactionSystem.GetItemTransactionList("", SysDocTypes.InventoryAdjustment);
			SelectDocumentDialog selectDocumentDialog = new SelectDocumentDialog();
			...
			if (itemTransactionByID.Tables.Contains("Item_Transaction_Detail") && itemTransactionByID.ItemTransactionDetailTable.Rows.Count != 0)
			{
				...
				foreach (DataRow row2 in itemTransactionByID.Tables["Item_Transaction_Detail"].Rows)
				{
					...
					{
						dataRow5["Adj Qty"] = row2["Quantity"];
					}
		...
		{
			InventoryTransactionsReport inventoryTransactionsReport = new InventoryTransactionsReport();
			inventoryTransactionsReport.istype = "A";
			inventoryTransactionsReport.ShowDialog();
			inventoryTransactionsReport.Close();
			DataSet getData = inventoryTransactionsReport.GetData;
			DataTable dataTable = dataGridItems.DataSource as DataTable;
			dataTable.Rows.Clear();
			dataGridItems.BeginUpdate();
			foreach (DataRow row in getData.Tables[0].Rows)
			{
				DataRow dataRow2 = dataTable.NewRow();
				dataRow2["Item Code"] = row["Item Code"];
				dataRow2["Description"] = row["Item Description"];
				dataRow2.EndEdit();
		...
		{
			FormActivator.BringFormToFront(FormActivator.InventoryAdjustmentListFormObj);
		}
		...
		private void InitializeComponent()
		{
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Micromind.ClientUI.WindowsForms.DataEntries.Inventory.InventoryAdjustmentsForm));
			...
			loadItemToolStripMenuItem = new System.Windows.Forms.ToolStripMenuItem();
			...
			ultraFormattedLinkLabel3.Location = new System.Drawing.Point(265, 28);
			ultraFormattedLinkLabel3.Name = "ultraFormattedLinkLabel3";
			ultraFormattedLinkLabel3.Size = new System.Drawing.Size(103, 15);
			ultraFormattedLinkLabel3.TabIndex = 122;
			ultraFormattedLinkLabel3.TabStop = true;
			ultraFormattedLinkLabel3.TreatValueAs = Infragistics.Win.FormattedLinkLabel.TreatValueAs.URL;
			ultraFormattedLinkLabel3.UnderlineLinks = Infragistics.Win.FormattedLinkLabel.UnderlineLink.WhenHovered;
			ultraFormattedLinkLabel3.Value = "Adjustment Type:";
			...
			adjustmentTypeComboBox1.DisplayLayout.Appearance = appearance22;
			...
			adjustmentTypeComboBox1.DisplayLayout.GroupByBox.Appearance = appearance23;
			appearance24.ForeColor = System.Drawing.SystemColors.GrayText;
			adjustmentTypeComboBox1.DisplayLayout.GroupByBox.BandLabelAppearance = appearance24;
			...
			adjustmentTypeComboBox1.DisplayLayout.GroupByBox.PromptAppearance = appearance25;
			...
			adjustmentTypeComboBox1.DisplayLayout.Override.ActiveCellAppearance = appearance26;
			...
			adjustmentTypeComboBox1.DisplayLayout.Override.ActiveRowAppearance = appearance27;
			...
			adjustmentTypeComboBox1.DisplayLayout.Override.CardAreaAppearance = appearance28;
			appearance29.BorderColor = System.Drawing.Color.Silver;
			appearance29.TextTrimming = Infragistics.Win.TextTrimming.EllipsisCharacter;
			adjustmentTypeComboBox1.DisplayLayout.Override.CellAppearance = appearance29;
			...
			adjustmentTypeComboBox1.DisplayLayout.Override.GroupByRowAppearance = appearance30;
			appearance31.TextHAlignAsString = "Left";
			adjustmentTypeComboBox1.DisplayLayout.Override.HeaderAppearance = appearance31;
			...
			adjustmentTypeComboBox1.DisplayLayout.Override.RowAppearance = appearance32;
			...
			adjustmentTypeComboBox1.DisplayLayout.Override.TemplateAddRowAppearance = appearance33;
			...
			adjustmentTypeComboBox1.TabIndex = 6;
			...
			toolStripButtonDistribution.Text = "toolStripButton2";
			toolStripButtonDistribution.Click += new System.EventHandler(toolStripButtonDistribution_Click);
			toolStripButton1.DisplayStyle = System.Windows.Forms.ToolStripItemDisplayStyle.Text;
			toolStripButton1.DropDownItems.AddRange(new System.Windows.Forms.ToolStripItem[5]
			{
				toolStripMenuItem1,
			...
			toolStripMenuItem1.Text = "Duplicate";
			...
			saveADraftToolStripMenuItem.Size = new System.Drawing.Size(199, 22);
			...
			loadItemToolStripMenuItem.Name = "loadItemToolStripMenuItem";
			loadItemToolStripMenuItem.Size = new System.Drawing.Size(199, 22);
			loadItemToolStripMenuItem.Text = "Load Items..";
			loadItemToolStripMenuItem.Click += new System.EventHandler(loadItemToolStripMenuItem_Click);
			...
			base.Name = "InventoryAdjustmentsForm";
