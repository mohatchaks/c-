...
{
	public class InventoryTransferReturnForm : Form, IForm
	{
		...
		private bool IsNewRecord
		{
			...
			set
			{
				...
				if (value)
				{
					...
					enabled = (comboBoxSysDocID.Enabled = true);
				...
				else
				{
					...
					enabled = (comboBoxSysDocID.Enabled = false);
		...
		public InventoryTransferReturnForm()
		{
			InitializeComponent();
			AddEvents();
		}
		...
		private void dataGrid_BeforeRowDeactivate(object sender, CancelEventArgs e)
		{
			...
			if (activeRow.Cells["Item Code"].Value.ToString() == "" && activeRow.Cells["Rejected Qty"].Value.ToString() != "")
			{
			...
			if (activeRow.Cells["Rejected Qty"].Value.ToString() == "")
			{
				activeRow.Cells["Rejected Qty"].Value = 0;
			}
			labelBalance.Text = GetTransactionBalance().ToString(Format.TotalAmountFormat);
		}
		...
		{
			if (e.Cell.Column.Key == "Rejected Qty")
			{
				if (isNewRecord)
				{
					labelBalance.Text = GetTransactionBalance().ToString(Format.TotalAmountFormat);
				...
				float num2 = 0f;
				float num3 = 0f;
				if (dataGridItems.ActiveRow.Cells["Transfered Qty"].Value != null && dataGridItems.ActiveRow.Cells["Transfered Qty"].Value.ToString() != "")
				{
					num = float.Parse(dataGridItems.ActiveRow.Cells["Transfered Qty"].Value.ToString());
				}
				if (dataGridItems.ActiveRow.Cells["Accepted Qty"].Value != null && dataGridItems.ActiveRow.Cells["Accepted Qty"].Value.ToString() != "")
				{
					num2 = float.Parse(dataGridItems.ActiveRow.Cells["Accepted Qty"].Value.ToString());
				}
				if (e.NewValue != null && e.NewValue.ToString() != "")
				{
					num3 = float.Parse(e.NewValue.ToString());
				}
				if (num3 > num - num2)
				{
					ErrorHelper.InformationMessage("Returned quantity should not exceed the total quantity.");
					e.Cancel = true;
				}
			}
		}
		...
		private void dataGrid_CellDataError(object sender, CellDataErrorEventArgs e)
		{
			...
			else if (dataGridItems.ActiveCell.Column.Key.ToString() == "Rejected Qty")
			{
				e.RaiseErrorEvent = false;
		...
		private bool GetData()
		{
			try
			{
				...
				dataRow["RejectReference"] = textBoxReference.Text;
				dataRow["RejectNote"] = textBoxNote.Text;
				dataRow["RejectDate"] = dateTimePickerDate.Value;
				dataRow["RejectSysDocID"] = comboBoxSysDocID.SelectedID;
				dataRow.EndEdit();
				...
				foreach (UltraGridRow row in dataGridItems.Rows)
				{
					...
					dataRow2.SetAdded();
					if (row.Cells["Rejected Qty"].Tag != null)
					{
						foreach (DataRow row2 in (row.Cells["Rejected Qty"].Tag as DataTable).Rows)
						{
		...
		private void SetupGrid()
		{
			try
			{
				...
				dataGridItems.DisplayLayout.Bands[0].Columns["Accepted Qty"].CellActivation = Activation.Disabled;
				dataGridItems.DisplayLayout.Bands[0].Columns["Accepted Qty"].CellAppearance.BackColorDisabled = Color.WhiteSmoke;
				dataGridItems.DisplayLayout.Bands[0].Columns["Accepted Qty"].CellAppearance.ForeColorDisabled = Color.Black;
				...
				dataGridItems.DisplayLayout.Bands[0].Columns["Rejected Qty"].Width = checked(10 * dataGridItems.Width) / 100;
				dataGridItems.DisplayLayout.Bands[0].Summaries.Add("TotalQty", SummaryType.Sum, dataGridItems.DisplayLayout.Bands[0].Columns["Rejected Qty"], SummaryPosition.UseSummaryPositionColumn);
		...
		public void LoadData(string voucherID)
		{
			try
			{
				...
				{
					currentData = Factory.InventoryTransferSystem.GetInventoryTransferRejectionByID(SystemDocID, voucherID);
					if (currentData != null)
					{
						FillData();
						IsNewRecord = false;
		...
		private void FillData()
		{
			try
			{
				if (currentData != null && currentData.Tables.Count != 0 && currentData.Tables[0].Rows.Count != 0)
				{
					...
					comboBoxSysDocID.Text = dataRow["RejectSysDocID"].ToString();
					textBoxSysDocID.Text = dataRow["SysDocID"].ToString();
					textBoxFrom.Text = dataRow["LocationFromID"].ToString();
					textBoxNote.Text = dataRow["RejectNote"].ToString();
					textBoxReference.Text = dataRow["RejectReference"].ToString();
					dateTimePickerDate.Value = DateTime.Parse(dataRow["RejectDate"].ToString());
					...
					if (currentData.Tables.Contains("Inventory_Transfer_Detail") && currentData.InventoryTransferDetailsTable.Rows.Count != 0)
					{
						foreach (DataRow row in currentData.Tables["Inventory_Transfer_Detail"].Rows)
						{
							...
							{
								dataRow3["Transfered Qty"] = row["Quantity"];
								d = decimal.Parse(row["Quantity"].ToString());
							}
							else
							{
							...
							if (row["AcceptedQuantity"] != DBNull.Value)
							{
								...
								d2 = decimal.Parse(row["AcceptedQuantity"].ToString());
							}
							else
							{
								dataRow3["Accepted Qty"] = 0;
							}
							dataRow3["Rejected Qty"] = d - d2;
						...
						foreach (UltraGridRow row2 in dataGridItems.Rows)
						{
							if (row2.Cells["IsTrackLot"].Value != DBNull.Value && bool.Parse(row2.Cells["IsTrackLot"].Value.ToString()))
							{
								...
								if (array.Length != 0)
								{
									...
									row2.Cells["Rejected Qty"].Tag = tag;
									row2.Cells["Rejected Qty"].Appearance.FontData.Underline = DefaultableBoolean.True;
								}
							}
						}
					}
		...
		private bool SaveData(bool clearAfter)
		{
			...
			try
			{
				bool flag2 = Factory.InventoryTransferSystem.RejectInventoryTransfer(currentData);
				if (flag2)
				{
					flag = true;
				}
		...
		private decimal GetTransactionBalance()
		{
			...
			foreach (UltraGridRow row in dataGridItems.Rows)
			{
				if (row.Cells["Rejected Qty"].Value != null && row.Cells["Rejected Qty"].Value.ToString() != "")
				{
					result += decimal.Parse(row.Cells["Rejected Qty"].Value.ToString());
				}
			}
			return result;
		}
		...
		{
			string nextID = DatabaseHelper.GetNextID("Inventory_Transfer", "VoucherID", textBoxVoucherNumber.Text, "RejectSysDocID", SystemDocID);
			if (!(nextID == ""))
			{
				LoadData(nextID);
			}
		...
		{
			string previousID = DatabaseHelper.GetPreviousID("Inventory_Transfer", "VoucherID", textBoxVoucherNumber.Text, "RejectSysDocID", SystemDocID);
			if (!(previousID == ""))
			{
				LoadData(previousID);
			}
		...
		{
			string lastID = DatabaseHelper.GetLastID("Inventory_Transfer", "VoucherID", "RejectSysDocID", SystemDocID);
			if (!(lastID == ""))
			{
				LoadData(lastID);
			}
		...
		{
			string firstID = DatabaseHelper.GetFirstID("Inventory_Transfer", "VoucherID", "RejectSysDocID", SystemDocID);
			if (!(firstID == ""))
			{
				LoadData(firstID);
			}
		...
		private void JournalLeavesForm_Load(object sender, EventArgs e)
		{
			try
			{
				comboBoxSysDocID.FilterByType(SysDocTypes.ReturnTransitTransfer);
		...
		private void buttonSelectCheque_Click(object sender, EventArgs e)
		{
			try
			{
				...
				else
				{
					...
					selectDocumentDialog.DataSource = Factory.InventoryTransferSystem.GetInventoryTransfersToReturn(comboBoxLocation.SelectedID);
					...
					if (selectDocumentDialog.ShowDialog() == DialogResult.OK)
					{
						...
						if (selectedRow != null)
						{
							...
							foreach (UltraGridRow row2 in dataGridItems.Rows)
							{
								...
								if (row2.Cells["IsTrackLot"].Value != DBNull.Value && bool.Parse(row2.Cells["IsTrackLot"].Value.ToString()))
								{
									DataRow[] array = currentData.Tables["Transit_Lots"].Select("ProductID = '" + row2.Cells["Item Code"].Value.ToString() + "' AND RowIndex = " + row2.Index);
									decimal result = default(decimal);
									decimal.TryParse(row2.Cells["Rejected Qty"].Value.ToString(), out result);
									if (array.Length != 0)
									{
										for (int i = 0; i < array.Length; i = checked(i + 1))
										{
											...
											decimal.TryParse(array[i]["LotQty"].ToString(), out result2);
											...
											if (result2 > result)
											{
												dataRow4["LotQty"] = result;
												result = default(decimal);
											}
											else
											{
												dataRow4["LotQty"] = result2;
												result -= result2;
											}
											dataRow4.EndEdit();
											dataTable2.Rows.Add(dataRow4);
										}
										row2.Cells["Rejected Qty"].Tag = dataTable2;
										row2.Cells["Rejected Qty"].Appearance.FontData.Underline = DefaultableBoolean.True;
									}
								}
							}
		...
		{
			new FormHelper().EditSysDoc(comboBoxSysDocID.SelectedID, SysDocTypes.ReturnTransitTransfer);
		}
		...
		private void Print(bool isPrint, bool showPrintDialog, bool saveChanges)
		{
			try
			{
				if (!(IsDirty && saveChanges) || (ErrorHelper.QuestionMessage(MessageBoxButtons.YesNo, "You must save the document before printing.", "Do you want to save?") == DialogResult.Yes && SaveData()))
				{
					...
					DataSet rejectedInventoryTransferToPrint = Factory.InventoryTransferSystem.GetRejectedInventoryTransferToPrint(selectedID, text);
					if (rejectedInventoryTransferToPrint == null || rejectedInventoryTransferToPrint.Tables.Count == 0)
					{
						ErrorHelper.ErrorMessage("Cannot print the document.", "Document not found.");
					}
					else
					{
						PrintHelper.PrintDocument(rejectedInventoryTransferToPrint, selectedID, "Inventory Transfer Return", SysDocTypes.DirectInventoryTransfer, isPrint, showPrintDialog);
					}
				}
			}
			catch (Exception e)
		...
		{
			FormActivator.BringFormToFront(FormActivator.InventoryTransferReturnListFormObj);
		}
		...
		private void comboBoxSysDocID_SelectedIndexChanged(object sender, EventArgs e)
		{
			if (isNewRecord)
			{
				textBoxVoucherNumber.Text = GetNextVoucherNumber();
		...
		private void InitializeComponent()
		{
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Micromind.ClientUI.WindowsForms.DataEntries.Inventory.InventoryTransferReturnForm));
			...
			dataGridItems.Location = new System.Drawing.Point(12, 150);
			...
			dataGridItems.Size = new System.Drawing.Size(642, 206);
			...
			textBoxVoucherNumber.Location = new System.Drawing.Point(314, 3);
			...
			mmLabel1.Location = new System.Drawing.Point(432, 26);
			...
			ultraGroupBox1.Location = new System.Drawing.Point(11, 355);
			...
			panelDetails.Size = new System.Drawing.Size(655, 116);
			panelDetails.TabIndex = 0;
			appearance16.FontData.BoldAsString = "True";
			appearance16.FontData.Name = "Tahoma";
			ultraFormattedLinkLabel1.Appearance = appearance16;
			ultraFormattedLinkLabel1.AutoSize = true;
			ultraFormattedLinkLabel1.Location = new System.Drawing.Point(14, 29);
			...
			ultraFormattedLinkLabel5.Location = new System.Drawing.Point(14, 6);
			...
			comboBoxSysDocID.Location = new System.Drawing.Point(103, 4);
			...
			comboBoxSysDocID.SelectedIndexChanged += new System.EventHandler(comboBoxSysDocID_SelectedIndexChanged);
			label3.AutoSize = true;
			label3.Location = new System.Drawing.Point(202, 6);
			label3.Name = "label3";
			...
			comboBoxLocation.Location = new System.Drawing.Point(103, 26);
			...
			buttonSelectCheque.Location = new System.Drawing.Point(428, 3);
			...
			label7.Location = new System.Drawing.Point(216, 60);
			label7.Name = "label7";
			label7.Size = new System.Drawing.Size(60, 13);
			label7.TabIndex = 133;
			label7.Text = "Reference:";
			textBoxReference.Location = new System.Drawing.Point(279, 57);
			...
			dateTimePickerDate.Location = new System.Drawing.Point(103, 57);
			...
			textBoxTransferDate.Location = new System.Drawing.Point(509, 24);
			...
			textBoxTransferDate.TabIndex = 5;
			label5.AutoSize = true;
			label5.Location = new System.Drawing.Point(487, 5);
			label5.Name = "label5";
			...
			label4.Location = new System.Drawing.Point(248, 28);
			label4.Name = "label4";
			...
			mmLabel2.Location = new System.Drawing.Point(14, 60);
			mmLabel2.Name = "mmLabel2";
			mmLabel2.PenWidth = 1f;
			mmLabel2.ShowBorder = false;
			mmLabel2.Size = new System.Drawing.Size(79, 13);
			mmLabel2.TabIndex = 2;
			mmLabel2.Text = "Reject Date:";
			...
			label8.TabIndex = 20;
			label8.Text = "Note:";
			textBoxNote.Location = new System.Drawing.Point(103, 78);
			...
			textBoxSysDocID.ForeColor = System.Drawing.Color.Black;
			textBoxSysDocID.Location = new System.Drawing.Point(537, 3);
			textBoxSysDocID.MaxLength = 15;
			textBoxSysDocID.Name = "textBoxSysDocID";
			textBoxSysDocID.ReadOnly = true;
			textBoxSysDocID.Size = new System.Drawing.Size(103, 20);
			textBoxSysDocID.TabIndex = 2;
			textBoxSysDocID.Text = "Invisible";
			textBoxSysDocID.Visible = false;
			textBoxFrom.BackColor = System.Drawing.Color.WhiteSmoke;
			textBoxFrom.ForeColor = System.Drawing.Color.Black;
			textBoxFrom.Location = new System.Drawing.Point(325, 25);
			textBoxFrom.MaxLength = 15;
			textBoxFrom.Name = "textBoxFrom";
			textBoxFrom.ReadOnly = true;
			textBoxFrom.Size = new System.Drawing.Size(101, 20);
			textBoxFrom.TabIndex = 4;
			labelVoided.Anchor = (System.Windows.Forms.AnchorStyles.Bottom | System.Windows.Forms.AnchorStyles.Left | System.Windows.Forms.AnchorStyles.Right);
			labelVoided.BackColor = System.Drawing.Color.White;
			labelVoided.Font = new System.Drawing.Font("Microsoft Sans Serif", 14f, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, 0);
			labelVoided.ForeColor = System.Drawing.Color.DarkRed;
			labelVoided.Location = new System.Drawing.Point(13, 306);
			...
			base.ClientSize = new System.Drawing.Size(668, 422);
			...
			base.Name = "InventoryTransferReturnForm";
			Text = "Reject Inventory Transfer";
