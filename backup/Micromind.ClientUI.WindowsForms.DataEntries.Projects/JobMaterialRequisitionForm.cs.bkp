...
{
	public class JobMaterialRequisitionForm : Form, IForm, IWorkFlowForm
	{
		private bool supressInventoryMessage;

		private JobMaterialRequisitionData currentData;

		private const string TABLENAME_CONST = "Job_Material_Requisition";
		...
		private DateTimePicker dateTimePickerReqonDate;
		...
		private ToolStripMenuItem loadFromMaterialEstimationToolStripMenuItem;
		...
		public JobMaterialRequisitionForm()
		{
			InitializeComponent();
			AddEvents();
		...
		private void AddEvents()
		{
			...
			dataGridItems.KeyDown += SalesOrderForm_KeyDown;
		}
			...
			try
			{
				if (dataGridItems.ActiveRow != null && e.Cell.Column.Key == "Item Code")
				{
					...
					if (itemTypes == ItemTypes.Matrix)
					{
						...
						if (dataGridItems.ActiveRow.Cells["JobID"].Value.ToString() == "" && useJobCosting)
						{
		...
		private void dataGridItems_AfterRowActivate(object sender, EventArgs e)
		{
			...
			checked
			{
				if (activeRow != null && IsNewRecord)
				{
					...
					if (dataGridItems.ActiveRow.Index > 0 && dataGridItems.ActiveRow.Cells["CostCategoryID"].Value.ToString() == "")
					{
		...
		private void dataGrid_BeforeRowDeactivate(object sender, CancelEventArgs e)
		{
			...
			if (activeRow != null)
			{
				...
				else if (activeRow.Cells["JobID"].Value.ToString() == "" && useJobCosting)
				{
					ErrorHelper.InformationMessage("Please select a project.");
					e.Cancel = true;
		...
		private void dataGrid_BeforeCellUpdate(object sender, BeforeCellUpdateEventArgs e)
		{
			...
			if (e.Cell.Column.Key == "Item Code")
			{
				...
				if (itemTypes != ItemTypes.Inventory && itemTypes != ItemTypes.Matrix && itemTypes != ItemTypes.Assembly && itemTypes != ItemTypes.Service && itemTypes != ItemTypes.NonInventory)
				{
		...
		private bool GetData()
		{
			try
			{
				...
				{
					currentData = new JobMaterialRequisitionData();
				}
				DataRow dataRow = (!isNewRecord) ? currentData.JobMaterialRequisitionTable.Rows[0] : currentData.JobMaterialRequisitionTable.NewRow();
				...
				if (dateTimePickerReqonDate.Checked)
				{
					dataRow["ReqonDate"] = dateTimePickerReqonDate.Value;
				}
				else
				{
					dataRow["ReqonDate"] = DBNull.Value;
				}
				dataRow.EndEdit();
				if (IsNewRecord)
				{
					currentData.JobMaterialRequisitionTable.Rows.Add(dataRow);
				}
				foreach (UltraGridColumn column in dataGridItems.DisplayLayout.Bands[0].Columns)
				{
					if (!currentData.JobMaterialRequisitionDetailTable.Columns.Contains(column.Key))
					{
						currentData.JobMaterialRequisitionDetailTable.Columns.Add(column.Key, column.DataType);
					}
				}
				currentData.JobMaterialRequisitionDetailTable.Rows.Clear();
				foreach (UltraGridRow row in dataGridItems.Rows)
				{
					DataRow dataRow2 = currentData.JobMaterialRequisitionDetailTable.NewRow();
					dataRow2.BeginEdit();
					...
					if (!string.IsNullOrEmpty(row.Cells["OnHand"].Value.ToString()))
					{
						dataRow2["OnHand"] = row.Cells["OnHand"].Value.ToString();
					}
					...
					dataRow2.EndEdit();
					currentData.JobMaterialRequisitionDetailTable.Rows.Add(dataRow2);
				}
				return true;
			}
			catch (Exception e)
		...
		private void SetupGrid()
		{
			try
			{
				...
				UltraGridColumn ultraGridColumn4 = dataGridItems.DisplayLayout.Bands[0].Columns["SpecificationID"];
				UltraGridColumn ultraGridColumn5 = dataGridItems.DisplayLayout.Bands[0].Columns["DefaultDescription"];
				bool flag2 = dataGridItems.DisplayLayout.Bands[0].Columns["Style"].Hidden = true;
				...
				Activation activation2 = dataGridItems.DisplayLayout.Bands[0].Columns["Attribute3"].CellActivation = Activation.NoEdit;
				Activation activation5 = ultraGridColumn6.CellActivation = (ultraGridColumn7.CellActivation = activation2);
				...
				flag8 = (dataGridItems.DisplayLayout.Bands[0].Columns["Attribute3"].TabStop = false);
				hidden = (ultraGridColumn9.TabStop = flag8);
				ultraGridColumn8.TabStop = hidden;
				...
				dataGridItems.DisplayLayout.Bands[0].Columns["JobID"].Hidden = !useJobCosting;
				dataGridItems.DisplayLayout.Bands[0].Columns["CostCategoryID"].Hidden = !useJobCosting;
				if (useJobCosting)
				{
					dataGridItems.DisplayLayout.Bands[0].Columns["JobID"].ExcludeFromColumnChooser = ExcludeFromColumnChooser.False;
				}
				else
				{
					dataGridItems.DisplayLayout.Bands[0].Columns["JobID"].ExcludeFromColumnChooser = ExcludeFromColumnChooser.True;
				}
				if (useJobCosting)
				{
					dataGridItems.DisplayLayout.Bands[0].Columns["CostCategoryID"].ExcludeFromColumnChooser = ExcludeFromColumnChooser.False;
				}
				else
				{
				...
				color = (dataGridItems.DisplayLayout.Bands[0].Columns["Attribute3"].CellAppearance.ForeColor = Color.Black);
				color4 = (cellAppearance3.ForeColor = (cellAppearance4.ForeColor = color));
		...
		public void LoadData(string voucherID)
		{
			try
			{
				...
				{
					currentData = Factory.JobMaterialRequisitionSystem.GetJobMaterialRequisitionByID(SystemDocID, voucherID);
					if (currentData != null)
					{
						FillData();
						IsNewRecord = false;
		...
		private void FillData()
		{
			try
			{
				...
				{
					DataRow dataRow = currentData.Tables["Job_Material_Requisition"].Rows[0];
					...
					{
						dateTimePickerReqonDate.Value = DateTime.Parse(dataRow["ReqonDate"].ToString());
						dateTimePickerReqonDate.Checked = true;
					}
					else
					{
						dateTimePickerReqonDate.Checked = false;
					}
					DataTable dataTable = dataGridItems.DataSource as DataTable;
					dataTable.Rows.Clear();
					if (currentData.Tables.Contains("Job_Material_Requisition_Detail") && currentData.JobMaterialRequisitionDetailTable.Rows.Count != 0)
					{
						dataGridItems.BeginUpdate();
						foreach (DataRow row in currentData.Tables["Job_Material_Requisition_Detail"].Rows)
						{
							...
							if (currentData.Tables["Job_Material_Requisition_Detail"].Columns.Contains("ItemType"))
							{
								dataRow3["ItemType"] = row["ItemType"];
							}
							dataRow3["Description"] = row["Description"];
							dataRow3["Remarks"] = row["Remarks"];
							if (!string.IsNullOrEmpty(row["OnHand"].ToString()))
							{
								dataRow3["OnHand"] = row["OnHand"];
							}
						...
						if (currentData.Tables.Contains("Purchase_Order_Detail") && currentData.Tables["Purchase_Order_Detail"].Rows.Count > 0)
						{
							...
							foreach (DataRow row2 in new DataView(currentData.Tables["Purchase_Order_Detail"]).ToTable(true, "SysDocID", "VoucherID").Rows)
							{
								NameValue item = new NameValue(row2["VoucherID"].ToString(), row2["SysDocID"].ToString());
								checkedListBoxPOs.Items.Add(item);
							}
						}
						if (currentData.Tables.Contains("Purchase_Quote_Detail") && currentData.Tables["Purchase_Quote_Detail"].Rows.Count > 0)
						{
							...
							labelSelectedDocs.Visible = true;
							checkedListBoxPOs.Visible = true;
							_ = currentData.Tables["Purchase_Quote_Detail"].Rows[0];
							foreach (DataRow row3 in currentData.Tables["Purchase_Quote_Detail"].Rows)
							{
								NameValue item2 = new NameValue(row3["VoucherID"].ToString(), row3["SysDocID"].ToString());
								checkedListBoxPOs.Items.Add(item2);
							}
						}
						if (currentData.Tables.Contains("Job_Inventory_Issue_Detail") && currentData.Tables["Job_Inventory_Issue_Detail"].Rows.Count > 0)
						{
							labelSelectedDocs.Visible = true;
							checkedListBoxPOs.Visible = true;
							_ = currentData.Tables["Job_Inventory_Issue_Detail"].Rows[0];
							foreach (DataRow row4 in currentData.Tables["Purchase_Quote_Detail"].Rows)
							{
								...
								checkedListBoxPOs.Items.Add(item3);
							}
						}
		...
		private bool SaveData(bool clearAfter)
		{
			...
			try
			{
				bool flag2 = Factory.JobMaterialRequisitionSystem.CreateJobMaterialRequisition(currentData, !isNewRecord);
				if (flag2)
				{
					flag = true;
				}
		...
		{
			if (!IsNewRecord && !Global.IsUserAdmin && !AllowEditTransaction && Global.CurrentUser != Factory.SystemDocumentSystem.GetTransUserID("Job_Material_Requisition", "VoucherID", SystemDocID, textBoxVoucherNumber.Text))
			{
			...
			if (!IsNewRecord)
			{
				DataSet entityApprovalStatus = Factory.EntityDocSystem.GetEntityApprovalStatus(currentData, SysDocTypes.JobMaterialRequisition, Global.CurrentUser, includeApproveUser: false);
			...
			checked
			{
				...
				for (int i = 0; i < dataGridItems.Rows.Count; i++)
				{
					...
					if (useJobCosting && dataGridItems.Rows[i].Cells["JobID"].Value.ToString() == "" && IsJobMust)
					{
				...
				if (formManager.IsFieldDirty(textBoxVoucherNumber) && Factory.SystemDocumentSystem.ExistDocumentNumber("Job_Material_Requisition", "VoucherID", SystemDocID, textBoxVoucherNumber.Text))
				{
					ErrorHelper.WarningMessage(UIMessages.DocumentNumberInUse);
				...
				if (!IsNewRecord)
				{
					...
					string text2 = "";
					string text3 = "";
					DataSet dataSet = Factory.JobMaterialRequisitionSystem.AllowDelete(comboBoxSysDoc.SelectedID, textBoxVoucherNumber.Text);
					if (dataSet.Tables["PODetails"].Rows.Count > 0 || dataSet.Tables["IssueDetails"].Rows.Count > 0 || dataSet.Tables["PQDetails"].Rows.Count > 0)
					{
						flag2 = false;
						...
						if (dataSet.Tables["IssueDetails"].Rows.Count > 0)
						{
							text2 = dataSet.Tables["IssueDetails"].Rows[0]["VoucherID"].ToString();
						}
						if (dataSet.Tables["PQDetails"].Rows.Count > 0)
						{
							text3 = dataSet.Tables["PQDetails"].Rows[0]["VoucherID"].ToString();
						}
					}
					if (!flag2)
					{
						...
						{
							ErrorHelper.WarningMessage("Some items in this transaction has been already allocated in Issued " + text2 + ". You are not able to modify.");
						}
						else if (text3 != "")
						{
							ErrorHelper.WarningMessage("Some items in this transaction has been already allocated in Quotation " + text3 + ". You are not able to modify.");
						}
						return false;
					}
				}
		...
		private void ClearForm()
		{
			try
			{
				...
				checkedListBoxPOs.Visible = false;
				labelSelectedDocs.Visible = false;
				if (string.IsNullOrEmpty(Security.DefaultInventoryLocationID))
				{
					comboBoxLocation.Clear();
				}
				dateTimePickerDate.Value = DateTime.Now;
				dateTimePickerReqonDate.Value = DateTime.Now;
				dateTimePickerReqonDate.Checked = false;
		...
		private bool Delete()
		{
			try
			{
				...
				return Factory.JobMaterialRequisitionSystem.DeleteJobMaterialRequisition(SystemDocID, textBoxVoucherNumber.Text);
			}
			catch (Exception e)
			{
				ErrorHelper.ProcessError(e);
		...
		{
			string nextID = DatabaseHelper.GetNextID("Job_Material_Requisition", "VoucherID", textBoxVoucherNumber.Text, "SysDocID", SystemDocID);
			if (!(nextID == ""))
			{
				LoadData(nextID);
			}
		...
		{
			string previousID = DatabaseHelper.GetPreviousID("Job_Material_Requisition", "VoucherID", textBoxVoucherNumber.Text, "SysDocID", SystemDocID);
			if (!(previousID == ""))
			{
				LoadData(previousID);
			}
		...
		{
			string lastID = DatabaseHelper.GetLastID("Job_Material_Requisition", "VoucherID", "SysDocID", SystemDocID);
			if (!(lastID == ""))
			{
				LoadData(lastID);
			}
		...
		{
			string firstID = DatabaseHelper.GetFirstID("Job_Material_Requisition", "VoucherID", "SysDocID", SystemDocID);
			if (!(firstID == ""))
			{
				LoadData(firstID);
			}
		...
		private void toolStripButtonFind_Click(object sender, EventArgs e)
		{
			try
			{
				...
				{
					string text = Factory.DatabaseSystem.FindDocumentByNumber("Job_Material_Requisition", "VoucherID", SystemDocID, toolStripTextBoxFind.Text);
					if (text != "")
					{
						LoadData(text);
					}
		...
		private void JournalLeavesForm_Load(object sender, EventArgs e)
		{
			try
			{
				...
				if (!string.IsNullOrEmpty(Security.DefaultInventoryLocationID))
				{
					comboBoxLocation.SelectedID = Security.DefaultInventoryLocationID;
				}
				if (!base.IsDisposed)
				{
					IsNewRecord = true;
		...
		private bool SaveDraft()
		{
			try
			{
				if (GetData())
				{
					...
					{
						return Global.CompanySettings.SaveTransactionDraft(currentData, enterNameDialog.EnteredName, SysDocTypes.JobMaterialRequisition);
					}
				}
				return false;
			}
		...
		private bool LoadDraft()
		{
			try
			{
				DataSet settingsList = Factory.SettingSystem.GetSettingsList("", 77.ToString());
				...
				if (selectDocumentDialog.ShowDialog(this) == DialogResult.OK)
				{
					...
					DataSet dataSet = Global.CompanySettings.LoadTransactionDraft(key, SysDocTypes.JobMaterialRequisition);
					currentData = (dataSet as JobMaterialRequisitionData);
					FillData();
				}
				return true;
			}
		...
		private void Print(bool isPrint, bool showPrintDialog, bool saveChanges)
		{
			try
			{
				if (!(IsDirty && saveChanges) || (ErrorHelper.QuestionMessage(MessageBoxButtons.YesNo, "You must save the document before printing.", "Do you want to save?") == DialogResult.Yes && SaveData(clearAfter: false)))
				{
					...
					{
						DataSet jobMaterialRequisitionToPrint = Factory.JobMaterialRequisitionSystem.GetJobMaterialRequisitionToPrint(selectedID, text);
						...
						{
							PrintHelper.PrintDocument(jobMaterialRequisitionToPrint, selectedID, "Project Material Requisition", SysDocTypes.JobMaterialRequisition, isPrint, showPrintDialog);
						}
					}
				}
			}
		...
		{
			new FormHelper().EditSysDoc(comboBoxSysDoc.SelectedID, SysDocTypes.JobMaterialRequisition);
		}
		...
		private void changeRequestStatusToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			updatePOStatusDialog.SetDocument(SysDocTypes.JobMaterialRequisition, comboBoxSysDoc.SelectedID, textBoxVoucherNumber.Text);
			updatePOStatusDialog.ShowDialog(this);
		}

		private void toolStripButtonOpenList_Click(object sender, EventArgs e)
		{
			FormActivator.BringFormToFront(FormActivator.JobMaterialRequistionListFormObj);
		}
		...
		private void JobMaterialRequisitionForm_Load(object sender, EventArgs e)
		{
		}

		private void toolStripButtonApproval_Click(object sender, EventArgs e)
		{
			new ViewApprovalDetailDialog().ShowApprovalDetail(1, 77, textBoxVoucherNumber.Text, comboBoxSysDoc.SelectedID);
		}
		...
		public void ShowForApproval(string sysDocID, string voucherID, int approvalTaskID)
		{
			...
			formManager.ShowApprovalPanel(approvalTaskID, "Job_Material_Requisition", "VoucherID");
		}
		...
		private void loadFromMaterialEstimationToolStripMenuItem_Click(object sender, EventArgs e)
		{
			if (!IsNewRecord)
			{
			...
			foreach (UltraGridRow selectedRow in selectDocumentDialog.SelectedRows)
			{
				...
				comboBoxLocation.SelectedID = dataRow["LocationID"].ToString();
				textBoxRequestedBy.Text = dataRow["RequestedBy"].ToString();
				dataSet.Merge(jobMaterialEstimateByJobIDCostCategoryID.JobMaterialEstimateDetailTable, preserveChanges: true, MissingSchemaAction.Add);
			}
			...
			if (selectItemRowsDialog.ShowDialog() == DialogResult.OK)
			{
				...
				{
					foreach (UltraGridRow selectedRow2 in selectItemRowsDialog.SelectedRows)
					{
						DataRow dataRow2 = dataTable.NewRow();
						dataRow2["JOBID"] = selectedRow2.Cells["JobID"].Value;
						dataRow2["Item Code"] = selectedRow2.Cells["ProductID"].Value;
						dataRow2["CostCategoryID"] = selectedRow2.Cells["CostCategoryID"].Value;
						dataRow2["Description"] = selectedRow2.Cells["Description"].Value;
						dataRow2["Unit"] = selectedRow2.Cells["UnitID"].Value;
						dataRow2["Quantity"] = selectedRow2.Cells["Quantity"].Value;
						if (comboBoxLocation.SelectedID != "")
						{
							DateTime date = new DateTime(dateTimePickerDate.Value.Year, dateTimePickerDate.Value.Month, dateTimePickerDate.Value.Day, 11, 59, 59);
							DataSet productQuantityAndCostAsOfDate = Factory.ProductSystem.GetProductQuantityAndCostAsOfDate(dataRow2["Item Code"].ToString(), comboBoxLocation.SelectedID, date);
							DataRow dataRow3 = null;
							if (productQuantityAndCostAsOfDate != null && productQuantityAndCostAsOfDate.Tables.Count > 0 && productQuantityAndCostAsOfDate.Tables[0].Rows.Count > 0)
							{
								dataRow3 = productQuantityAndCostAsOfDate.Tables[0].Rows[0];
							}
							if (dataRow3 != null)
							{
								dataRow2["Onhand"] = dataRow3["Quantity"].ToString();
							}
							else
							{
								dataRow2["Onhand"] = 0;
		...
		private void InitializeComponent()
		{
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Micromind.ClientUI.WindowsForms.DataEntries.Projects.JobMaterialRequisitionForm));
			...
			loadFromMaterialEstimationToolStripMenuItem = new System.Windows.Forms.ToolStripMenuItem();
			...
			dateTimePickerReqonDate = new System.Windows.Forms.DateTimePicker();
			...
			toolStripButton1.DropDownItems.AddRange(new System.Windows.Forms.ToolStripItem[6]
			{
				saveADraftToolStripMenuItem,
				loadDraftToolStripMenuItem,
				toolStripSeparator4,
				changeRequestStatusToolStripMenuItem,
				loadFromMaterialEstimationToolStripMenuItem,
			...
			saveADraftToolStripMenuItem.Size = new System.Drawing.Size(245, 22);
			...
			changeRequestStatusToolStripMenuItem.Size = new System.Drawing.Size(245, 22);
			...
			loadFromMaterialEstimationToolStripMenuItem.Name = "loadFromMaterialEstimationToolStripMenuItem";
			loadFromMaterialEstimationToolStripMenuItem.Size = new System.Drawing.Size(245, 22);
			loadFromMaterialEstimationToolStripMenuItem.Text = "Load from Material Estimation";
			loadFromMaterialEstimationToolStripMenuItem.Click += new System.EventHandler(loadFromMaterialEstimationToolStripMenuItem_Click);
			...
			textBoxRef1.Location = new System.Drawing.Point(78, 47);
			...
			linkLabelVoucherNumber.Location = new System.Drawing.Point(208, 5);
			...
			panelDetails.Controls.Add(dateTimePickerReqonDate);
			...
			label5.Location = new System.Drawing.Point(472, 28);
			label5.Name = "label5";
			label5.Size = new System.Drawing.Size(68, 13);
			label5.TabIndex = 131;
			label5.Text = "Required on:\r\n";
			dateTimePickerReqonDate.Format = System.Windows.Forms.DateTimePickerFormat.Short;
			dateTimePickerReqonDate.Location = new System.Drawing.Point(543, 25);
			dateTimePickerReqonDate.Name = "dateTimePickerReqonDate";
			dateTimePickerReqonDate.ShowCheckBox = true;
			dateTimePickerReqonDate.Size = new System.Drawing.Size(109, 20);
			dateTimePickerReqonDate.TabIndex = 6;
			textBoxRef2.Location = new System.Drawing.Point(321, 47);
			...
			label4.Location = new System.Drawing.Point(239, 50);
			label4.Name = "label4";
			label4.Size = new System.Drawing.Size(69, 13);
			label4.TabIndex = 129;
			label4.Text = "Reference 2:";
			label2.AutoSize = true;
			label2.Location = new System.Drawing.Point(232, 29);
			label2.Name = "label2";
			label2.Size = new System.Drawing.Size(77, 13);
			label2.TabIndex = 127;
			label2.Text = "Requested By:";
			textBoxRequestedBy.Location = new System.Drawing.Point(320, 25);
			...
			labelVoided.Location = new System.Drawing.Point(16, 297);
			labelVoided.Name = "labelVoided";
			labelVoided.Size = new System.Drawing.Size(735, 81);
			...
			label3.Location = new System.Drawing.Point(19, 500);
			label3.Name = "label3";
			label3.Size = new System.Drawing.Size(33, 13);
			label3.TabIndex = 127;
			label3.Text = "Note:";
			textBoxNote.Anchor = (System.Windows.Forms.AnchorStyles.Bottom | System.Windows.Forms.AnchorStyles.Left);
			textBoxNote.Location = new System.Drawing.Point(78, 486);
			textBoxNote.MaxLength = 4000;
			textBoxNote.Multiline = true;
			textBoxNote.Name = "textBoxNote";
			textBoxNote.ScrollBars = System.Windows.Forms.ScrollBars.Vertical;
			textBoxNote.Size = new System.Drawing.Size(308, 76);
			textBoxNote.TabIndex = 126;
			...
			labelSelectedDocs.Location = new System.Drawing.Point(405, 474);
			labelSelectedDocs.Name = "labelSelectedDocs";
			labelSelectedDocs.Size = new System.Drawing.Size(117, 13);
			labelSelectedDocs.TabIndex = 135;
			labelSelectedDocs.Text = "Processed Documents:";
			labelSelectedDocs.Visible = false;
			...
			checkedListBoxPOs.Location = new System.Drawing.Point(405, 489);
			checkedListBoxPOs.Name = "checkedListBoxPOs";
			checkedListBoxPOs.Size = new System.Drawing.Size(155, 69);
			checkedListBoxPOs.TabIndex = 136;
			checkedListBoxPOs.Visible = false;
			...
			productPhotoViewer.Location = new System.Drawing.Point(44, 173);
			...
			comboBoxGridProductUnit.DisplayLayout.Appearance = appearance5;
			...
			comboBoxGridProductUnit.DisplayLayout.GroupByBox.Appearance = appearance6;
			appearance7.ForeColor = System.Drawing.SystemColors.GrayText;
			comboBoxGridProductUnit.DisplayLayout.GroupByBox.BandLabelAppearance = appearance7;
			...
			comboBoxGridProductUnit.DisplayLayout.GroupByBox.PromptAppearance = appearance8;
			...
			comboBoxGridProductUnit.DisplayLayout.Override.ActiveCellAppearance = appearance9;
			appearance10.BackColor = System.Drawing.SystemColors.Highlight;
			appearance10.ForeColor = System.Drawing.SystemColors.HighlightText;
			comboBoxGridProductUnit.DisplayLayout.Override.ActiveRowAppearance = appearance10;
			...
			comboBoxGridProductUnit.DisplayLayout.Override.CardAreaAppearance = appearance11;
			appearance12.BorderColor = System.Drawing.Color.Silver;
			appearance12.TextTrimming = Infragistics.Win.TextTrimming.EllipsisCharacter;
			comboBoxGridProductUnit.DisplayLayout.Override.CellAppearance = appearance12;
			...
			comboBoxGridProductUnit.DisplayLayout.Override.GroupByRowAppearance = appearance13;
			appearance14.TextHAlignAsString = "Left";
			comboBoxGridProductUnit.DisplayLayout.Override.HeaderAppearance = appearance14;
			...
			comboBoxGridProductUnit.DisplayLayout.Override.RowAppearance = appearance15;
			comboBoxGridProductUnit.DisplayLayout.Override.RowSelectors = Infragistics.Win.DefaultableBoolean.False;
			appearance16.BackColor = System.Drawing.SystemColors.ControlLight;
			comboBoxGridProductUnit.DisplayLayout.Override.TemplateAddRowAppearance = appearance16;
			...
			comboBoxGridItem.DisplayLayout.Appearance = appearance17;
			...
			comboBoxGridItem.DisplayLayout.GroupByBox.Appearance = appearance18;
			appearance19.ForeColor = System.Drawing.SystemColors.GrayText;
			comboBoxGridItem.DisplayLayout.GroupByBox.BandLabelAppearance = appearance19;
			...
			comboBoxGridItem.DisplayLayout.GroupByBox.PromptAppearance = appearance20;
			...
			comboBoxGridItem.DisplayLayout.Override.ActiveCellAppearance = appearance21;
			appearance22.BackColor = System.Drawing.SystemColors.Highlight;
			appearance22.ForeColor = System.Drawing.SystemColors.HighlightText;
			comboBoxGridItem.DisplayLayout.Override.ActiveRowAppearance = appearance22;
			...
			comboBoxGridItem.DisplayLayout.Override.CardAreaAppearance = appearance23;
			appearance24.BorderColor = System.Drawing.Color.Silver;
			appearance24.TextTrimming = Infragistics.Win.TextTrimming.EllipsisCharacter;
			comboBoxGridItem.DisplayLayout.Override.CellAppearance = appearance24;
			...
			comboBoxGridItem.DisplayLayout.Override.GroupByRowAppearance = appearance25;
			appearance26.TextHAlignAsString = "Left";
			comboBoxGridItem.DisplayLayout.Override.HeaderAppearance = appearance26;
			...
			comboBoxGridItem.DisplayLayout.Override.RowAppearance = appearance27;
			comboBoxGridItem.DisplayLayout.Override.RowSelectors = Infragistics.Win.DefaultableBoolean.False;
			appearance28.BackColor = System.Drawing.SystemColors.ControlLight;
			comboBoxGridItem.DisplayLayout.Override.TemplateAddRowAppearance = appearance28;
			...
			comboBoxSysDoc.Location = new System.Drawing.Point(78, 2);
			...
			mmLabel2.Location = new System.Drawing.Point(9, 26);
			...
			comboBoxLocation.DisplayLayout.Appearance = appearance41;
			...
			comboBoxLocation.DisplayLayout.GroupByBox.Appearance = appearance42;
			appearance43.ForeColor = System.Drawing.SystemColors.GrayText;
			comboBoxLocation.DisplayLayout.GroupByBox.BandLabelAppearance = appearance43;
			...
			comboBoxLocation.DisplayLayout.GroupByBox.PromptAppearance = appearance44;
			...
			comboBoxLocation.DisplayLayout.Override.ActiveCellAppearance = appearance45;
			...
			comboBoxLocation.DisplayLayout.Override.ActiveRowAppearance = appearance46;
			...
			comboBoxLocation.DisplayLayout.Override.CardAreaAppearance = appearance47;
			appearance48.BorderColor = System.Drawing.Color.Silver;
			appearance48.TextTrimming = Infragistics.Win.TextTrimming.EllipsisCharacter;
			comboBoxLocation.DisplayLayout.Override.CellAppearance = appearance48;
			...
			comboBoxLocation.DisplayLayout.Override.GroupByRowAppearance = appearance49;
			appearance50.TextHAlignAsString = "Left";
			comboBoxLocation.DisplayLayout.Override.HeaderAppearance = appearance50;
			...
			comboBoxLocation.DisplayLayout.Override.RowAppearance = appearance51;
			...
			comboBoxLocation.DisplayLayout.Override.TemplateAddRowAppearance = appearance52;
			...
			comboBoxLocation.Location = new System.Drawing.Point(78, 24);
			...
			dataGridItems.Location = new System.Drawing.Point(11, 123);
			...
			dataGridItems.Size = new System.Drawing.Size(749, 343);
			dataGridItems.TabIndex = 8;
			...
			comboBoxSpecification.DisplayLayout.Appearance = appearance77;
			...
			comboBoxSpecification.DisplayLayout.GroupByBox.Appearance = appearance78;
			appearance79.ForeColor = System.Drawing.SystemColors.GrayText;
			comboBoxSpecification.DisplayLayout.GroupByBox.BandLabelAppearance = appearance79;
			...
			comboBoxSpecification.DisplayLayout.GroupByBox.PromptAppearance = appearance80;
			...
			comboBoxSpecification.DisplayLayout.Override.ActiveCellAppearance = appearance81;
			...
			comboBoxSpecification.DisplayLayout.Override.ActiveRowAppearance = appearance82;
			...
			comboBoxSpecification.DisplayLayout.Override.CardAreaAppearance = appearance83;
			...
			comboBoxSpecification.DisplayLayout.Override.CellAppearance = appearance84;
			...
			comboBoxSpecification.DisplayLayout.Override.GroupByRowAppearance = appearance85;
			appearance86.TextHAlignAsString = "Left";
			comboBoxSpecification.DisplayLayout.Override.HeaderAppearance = appearance86;
			...
			comboBoxSpecification.DisplayLayout.Override.RowAppearance = appearance87;
			...
			comboBoxSpecification.DisplayLayout.Override.TemplateAddRowAppearance = appearance88;
			...
			comboBoxSpecification.Location = new System.Drawing.Point(364, 229);
			...
			comboBoxStyle.DisplayLayout.Appearance = appearance89;
			...
			comboBoxStyle.DisplayLayout.GroupByBox.Appearance = appearance90;
			appearance91.ForeColor = System.Drawing.SystemColors.GrayText;
			comboBoxStyle.DisplayLayout.GroupByBox.BandLabelAppearance = appearance91;
			...
			comboBoxStyle.DisplayLayout.GroupByBox.PromptAppearance = appearance92;
			...
			comboBoxStyle.DisplayLayout.Override.ActiveCellAppearance = appearance93;
			...
			comboBoxStyle.DisplayLayout.Override.ActiveRowAppearance = appearance94;
			...
			comboBoxStyle.DisplayLayout.Override.CardAreaAppearance = appearance95;
			...
			comboBoxStyle.DisplayLayout.Override.CellAppearance = appearance96;
			...
			comboBoxStyle.DisplayLayout.Override.GroupByRowAppearance = appearance97;
			appearance98.TextHAlignAsString = "Left";
			comboBoxStyle.DisplayLayout.Override.HeaderAppearance = appearance98;
			...
			comboBoxStyle.DisplayLayout.Override.RowAppearance = appearance99;
			...
			comboBoxStyle.DisplayLayout.Override.TemplateAddRowAppearance = appearance100;
			...
			comboBoxStyle.Location = new System.Drawing.Point(498, 229);
			...
			base.ClientSize = new System.Drawing.Size(774, 623);
			...
			MinimumSize = new System.Drawing.Size(648, 366);
			base.Name = "JobMaterialRequisitionForm";
			Text = "Material Requisition Entry";
			base.Load += new System.EventHandler(JobMaterialRequisitionForm_Load);
