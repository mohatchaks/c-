...
{
	public class JobBOMDetailsForm : Form, IForm
	{
		private bool canAccessCost = true;

		private JobBOMData currentData;

		private const string TABLENAME_CONST = "Job_BOM";

		private const string IDFIELD_CONST = "JobBOMID";
		...
		{
			base.Load += JobBOMDetailsForm_Load;
		...
		private void dataEntryGridBOM_AfterCellUpdate(object sender, CellEventArgs e)
		{
			...
			if (dataEntryGridBOM.ActiveRow != null)
			{
				...
				dataEntryGridBOM.ActiveRow.Cells["Cost"].Value = Factory.ProductSystem.GetJobBOMProductPurchasePrice(comboBoxGridBOMItem.SelectedID);
				dataEntryGridBOM.ActiveRow.Cells["Labour Cost"].Value = Factory.ProductSystem.GetJobBOMLabourCost(comboBoxGridBOMItem.SelectedID);
		...
		private bool GetData()
		{
			try
			{
				...
				{
					currentData = new JobBOMData();
				}
				DataRow dataRow = (!isNewRecord) ? currentData.JobBOMTable.Rows[0] : currentData.JobBOMTable.NewRow();
				dataRow.BeginEdit();
				if (isNewRecord)
				{
					dataRow["JobBOMID"] = textBoxCode.Text.Trim();
				}
				else
				{
					dataRow["JobBOMID"] = textBoxCode.Text;
				}
				...
				if (isNewRecord)
				{
					currentData.JobBOMTable.Rows.Add(dataRow);
				}
				currentData.JobBOMDetailTable.Rows.Clear();
				foreach (UltraGridRow row in dataEntryGridBOM.Rows)
				{
					DataRow dataRow2 = currentData.JobBOMDetailTable.NewRow();
					dataRow2["JobBOMID"] = textBoxCode.Text;
					...
					if (row.Cells["Labour Cost"].Value.ToString() != null)
					{
						dataRow2["LabourCost"] = row.Cells["Labour Cost"].Value.ToString();
					}
					else
					{
						dataRow2["LabourCost"] = 0;
					}
					dataRow2["Description"] = row.Cells["Description"].Value.ToString();
					dataRow2["Quantity"] = row.Cells["Quantity"].Value.ToString();
					currentData.JobBOMDetailTable.Rows.Add(dataRow2);
				}
				return true;
			}
			catch (Exception e)
		...
		public void LoadData(string id)
		{
			try
			{
				...
				{
					currentData = Factory.JobBOMSystem.GetJobBOMByID(id);
		...
		private void FillData()
		{
			try
			{
				...
				if (currentData != null && currentData.Tables.Count != 0 && currentData.Tables[0].Rows.Count != 0)
				{
					...
					textBoxCode.Text = dataRow["JobBOMID"].ToString();
					...
					foreach (DataRow row in currentData.JobBOMDetailTable.Rows)
					{
						...
						dataRow3["Labour Cost"] = row["LabourCost"];
		...
		private bool SaveData()
		{
			...
			try
			{
				bool flag = (!isNewRecord) ? Factory.JobBOMSystem.UpdateJobBOM(currentData) : Factory.JobBOMSystem.CreateJobBOM(currentData);
				if (flag)
				{
		...
		private bool ValidateData()
		{
			...
			if (!IsNewRecord && !Global.IsUserAdmin && !AllowEditCard && Global.CurrentUser != Factory.SystemDocumentSystem.GetCardUserID("Job_BOM", "JobBOMID", textBoxCode.Text))
			{
			...
			if (isNewRecord && Factory.DatabaseSystem.ExistFieldValue("Job_BOM", "JobBOMID", textBoxCode.Text.Trim()))
			{
		...
		{
			textBoxCode.Text = PublicFunctions.GetNextCardNumber("Job_BOM", "JobBOMID");
			textBoxName.Clear();
			textBoxNote.Clear();
			checkBoxInactive.Checked = false;
		...
		{
			LoadData(DatabaseHelper.GetNextID("Job_BOM", "JobBOMID", textBoxCode.Text));
		}

		private void toolStripButtonPrevious_Click(object sender, EventArgs e)
		{
			LoadData(DatabaseHelper.GetPreviousID("Job_BOM", "JobBOMID", textBoxCode.Text));
		}

		private void toolStripButtonLast_Click(object sender, EventArgs e)
		{
			LoadData(DatabaseHelper.GetLastID("Job_BOM", "JobBOMID"));
		}

		private void toolStripButtonFirst_Click(object sender, EventArgs e)
		{
			LoadData(DatabaseHelper.GetFirstID("Job_BOM", "JobBOMID"));
		}
		...
		private void toolStripButtonFind_Click(object sender, EventArgs e)
		{
			try
			{
				...
				else if (Factory.DatabaseSystem.ExistFieldValue("Job_BOM", "JobBOMID", toolStripTextBoxFind.Text.Trim()))
				{
		...
		private void JobBOMDetailsForm_Load(object sender, EventArgs e)
		{
		}
		...
		private void SetupBOMGrid()
		{
			...
			dataTable.Columns.Add("Labour Cost", typeof(decimal));
			...
			dataEntryGridBOM.DisplayLayout.Bands[0].Columns["Labour Cost"].CellActivation = Activation.AllowEdit;
			dataEntryGridBOM.DisplayLayout.Bands[0].Columns["Labour Cost"].CellAppearance.TextHAlign = HAlign.Right;
			dataEntryGridBOM.DisplayLayout.Bands[0].Columns["Labour Cost"].Format = Format.GridAmountFormat;
		...
		{
			new FormHelper().ShowList(DataComboType.JobBOM);
		}
		...
		private void InitializeComponent()
		{
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Micromind.ClientUI.WindowsForms.DataEntries.Projects.JobBOMDetailsForm));
