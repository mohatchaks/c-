...
{
	public class PurchaseOrderNonInvForm : Form, IForm, IWorkFlowForm
	{
		...
		private PurchaseOrderNIData currentData;

		private const string TABLENAME_CONST = "Purchase_Order_NonInv";
		...
		public PurchaseOrderNonInvForm()
		{
			InitializeComponent();
			AddEvents();
		...
		private void docStatusLabel_LinkClicked(object sender, EventArgs e)
		{
			...
			if (control != null)
			{
				formHelper.EditTransaction(TransactionListType.PurchaseInvoiceNI, control.Tag.ToString(), control.Text);
			}
		}
		...
		private bool GetData()
		{
			try
			{
				...
				{
					currentData = new PurchaseOrderNIData();
				}
		...
		public void LoadData(string voucherID)
		{
			try
			{
				...
				{
					currentData = Factory.PurchaseOrderNISystem.GetPurchaseOrderByID(SystemDocID, voucherID);
					if (currentData != null)
					{
						FillData();
						IsNewRecord = false;
		...
		private void FillData()
		{
			try
			{
				...
				{
					DataRow dataRow = currentData.Tables["Purchase_Order_NonInv"].Rows[0];
					...
					if (orderStatusEnum == OrderStatusEnum.Open)
					{
					...
					{
						docStatusLabel.DocumentNumber = currentData.Tables["Purchase_Order_NonInv"].Rows[0]["PKVoucherID"].ToString();
						docStatusLabel.Tag = currentData.Tables["Purchase_Order_NonInv"].Rows[0]["PKSysDocID"].ToString();
					}
					...
					if (currentData.Tables.Contains("Purchase_Order_NonInv_Detail") && currentData.PurchaseOrderDetailTable.Rows.Count != 0)
					{
						foreach (DataRow row in currentData.Tables["Purchase_Order_NonInv_Detail"].Rows)
						{
		...
		private bool SaveData(bool clearAfter)
		{
			...
			try
			{
				if (Factory.PurchaseOrderNISystem.CreatePurchaseOrder(currentData, !isNewRecord))
				{
					flag = true;
				}
				bool flag2 = true;
		...
		public void ShowForApproval(string sysDocID, string voucherID, int approvalTaskID)
		{
			...
			formManager.ShowApprovalPanel(approvalTaskID, "Purchase_Order_NonInv", "VoucherID");
		}
		...
		private bool ValidateData()
		{
			...
			if (!IsNewRecord && !Global.IsUserAdmin && !AllowEditTransaction && Global.CurrentUser != Factory.SystemDocumentSystem.GetTransUserID("Purchase_Order_NonInv", "VoucherID", SystemDocID, textBoxVoucherNumber.Text))
			{
			...
			if (!IsNewRecord)
			{
				DataSet entityApprovalStatus = Factory.EntityDocSystem.GetEntityApprovalStatus(currentData, SysDocTypes.PurchaseOrderNI, Global.CurrentUser, includeApproveUser: false);
			...
			checked
			{
				...
				if (!IsNewRecord && !Factory.PurchaseOrderNISystem.CanUpdate(comboBoxSysDoc.SelectedID, textBoxVoucherNumber.Text))
				{
				...
				if (formManager.IsFieldDirty(textBoxVoucherNumber) && Factory.SystemDocumentSystem.ExistDocumentNumber("Purchase_Order_NonInv", "VoucherID", SystemDocID, textBoxVoucherNumber.Text))
				{
					ErrorHelper.WarningMessage(UIMessages.DocumentNumberInUse);
		...
		private bool Delete()
		{
			try
			{
				...
				if (!Factory.PurchaseOrderNISystem.CanUpdate(comboBoxSysDoc.SelectedID, textBoxVoucherNumber.Text))
				{
				...
				return Factory.PurchaseOrderNISystem.DeletePurchaseOrder(SystemDocID, textBoxVoucherNumber.Text);
			}
			catch (CompanyException ex)
			{
				if (ex.Number == 1037)
		...
		{
			string nextID = DatabaseHelper.GetNextID("Purchase_Order_NonInv", "VoucherID", textBoxVoucherNumber.Text, "SysDocID", SystemDocID);
			if (!(nextID == ""))
			{
				LoadData(nextID);
			}
		...
		{
			string previousID = DatabaseHelper.GetPreviousID("Purchase_Order_NonInv", "VoucherID", textBoxVoucherNumber.Text, "SysDocID", SystemDocID);
			if (!(previousID == ""))
			{
				LoadData(previousID);
			}
		...
		{
			string lastID = DatabaseHelper.GetLastID("Purchase_Order_NonInv", "VoucherID", "SysDocID", SystemDocID);
			if (!(lastID == ""))
			{
				LoadData(lastID);
			}
		...
		{
			string firstID = DatabaseHelper.GetFirstID("Purchase_Order_NonInv", "VoucherID", "SysDocID", SystemDocID);
			if (!(firstID == ""))
			{
				LoadData(firstID);
			}
		...
		private void toolStripButtonFind_Click(object sender, EventArgs e)
		{
			try
			{
				...
				{
					string text = Factory.DatabaseSystem.FindDocumentByNumber("Purchase_Order_NonInv", "VoucherID", SystemDocID, toolStripTextBoxFind.Text);
					if (text != "")
					{
						LoadData(text);
					}
		...
		private void Form_Load(object sender, EventArgs e)
		{
			checked
			{
				try
				{
					...
					createFromMRToolStripMenuItem.Visible = useJobCosting;
					comboBoxSysDoc.FilterByType(SysDocTypes.PurchaseOrderNI);
					SetSecurity();
					if (!base.IsDisposed)
					{
						IsNewRecord = true;
		...
		private bool Void(bool isVoid)
		{
			try
			{
				...
				return Factory.PurchaseOrderNISystem.VoidPurchaseOrder(SystemDocID, textBoxVoucherNumber.Text, isVoid);
			}
			catch (CompanyException ex)
			{
				if (ex.Number == 1037)
		...
		{
			new FormHelper().EditSysDoc(comboBoxSysDoc.SelectedID, SysDocTypes.PurchaseOrderNI);
		}
		...
		private void Print(bool isPrint, bool showPrintDialog, bool saveChanges)
		{
			try
			{
				if (!(IsDirty && saveChanges) || (ErrorHelper.QuestionMessage(MessageBoxButtons.YesNo, "You must save the document before printing.", "Do you want to save?") == DialogResult.Yes && SaveData(clearAfter: false)))
				{
					string selectedID = comboBoxSysDoc.SelectedID;
					string text = textBoxVoucherNumber.Text;
					DataSet purchaseOrderToPrint = Factory.PurchaseOrderNISystem.GetPurchaseOrderToPrint(selectedID, text);
					...
					{
						PrintHelper.PrintDocument(purchaseOrderToPrint, selectedID, "Purchase Order NI", SysDocTypes.PurchaseOrderNI, isPrint, showPrintDialog);
					}
				}
			}
			catch (Exception e)
		...
		{
			FormActivator.BringFormToFront(FormActivator.PurchaseOrderNIListFormObj);
		}
		...
		private void closeOrderToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			updatePOStatusDialog.SetDocument(SysDocTypes.PurchaseOrderNI, comboBoxSysDoc.SelectedID, textBoxVoucherNumber.Text);
			...
			if (purchaseQuoteByID.Tables.Contains("Purchase_Quote_Detail") && purchaseQuoteByID.PurchaseQuoteDetailTable.Rows.Count != 0)
			{
				foreach (DataRow row in purchaseQuoteByID.Tables["Purchase_Quote_Detail"].Rows)
				{
					...
					if (itemTypes == ItemTypes.NonInventory || itemTypes == ItemTypes.Service)
					{
						...
						if (useJobCosting && !string.IsNullOrEmpty(row["JobID"].ToString()))
						{
							dataRow3["Job"] = row["JobID"].ToString();
						}
						dataRow3["TaxGroupID"] = row["TaxGroupID"];
						if (row["TaxOption"] != DBNull.Value)
						{
		...
		private void InitializeComponent()
		{
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Micromind.ClientUI.WindowsForms.DataEntries.Vendors.PurchaseOrderNonInvForm));
			...
			textBoxvendorRefNo.Location = new System.Drawing.Point(478, 45);
			...
			labelcostcategory.Location = new System.Drawing.Point(608, 96);
			...
			labelJob.Location = new System.Drawing.Point(436, 95);
			...
			comboBoxCostCategory.Location = new System.Drawing.Point(688, 93);
			...
			comboBoxJob.Location = new System.Drawing.Point(478, 92);
			comboBoxJob.MaxDropDownItems = 12;
			comboBoxJob.Name = "comboBoxJob";
			comboBoxJob.ShowInactiveItems = false;
			comboBoxJob.ShowQuickAdd = true;
			comboBoxJob.Size = new System.Drawing.Size(103, 20);
			...
			buttonSelectPL.Location = new System.Drawing.Point(407, 88);
			buttonSelectPL.Name = "buttonSelectPL";
			buttonSelectPL.Size = new System.Drawing.Size(25, 24);
			buttonSelectPL.TabIndex = 183;
			buttonSelectPL.Text = "...";
			buttonSelectPL.UseVisualStyleBackColor = false;
			buttonSelectPL.Click += new System.EventHandler(buttonSelectPL_Click);
			textBoxBOLNo.BackColor = System.Drawing.Color.WhiteSmoke;
			textBoxBOLNo.Location = new System.Drawing.Point(268, 90);
			textBoxBOLNo.Name = "textBoxBOLNo";
			textBoxBOLNo.ReadOnly = true;
			textBoxBOLNo.Size = new System.Drawing.Size(134, 20);
			textBoxBOLNo.TabIndex = 15;
			textBoxBOLNo.TabStop = false;
			label4.AutoSize = true;
			label4.Location = new System.Drawing.Point(211, 93);
			label4.Name = "label4";
			...
			dateTimePickerDueDate.Location = new System.Drawing.Point(478, 68);
			dateTimePickerDueDate.Name = "dateTimePickerDueDate";
			dateTimePickerDueDate.Size = new System.Drawing.Size(96, 20);
			...
			linkLabelTax.Appearance = appearance93;
			...
			linkLabelTax.VisitedLinkAppearance = appearance94;
			...
			docStatusLabel.Location = new System.Drawing.Point(426, 507);
			...
			base.Name = "PurchaseOrderNonInvForm";
			Text = "Purchase Order(Non Inventory)";
