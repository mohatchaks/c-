...
{
	public class UserGroupAssignForm : Form, IForm
	{
		...
		{
			base.Load += UserGroupAssignForm_Load;
		...
		public void LoadData(string id)
		{
			try
			{
				if (!(id.Trim() == "") && CanClose())
				{
					currentData = Factory.UserGroupSystem.GetGroupsByUser(id.Trim());
		...
		private void FillData()
		{
			if (currentData != null && currentData.Tables.Count != 0 && currentData.Tables[0].Rows.Count != 0)
			{
				...
				{
					NameValue item = new NameValue(row["GroupName"].ToString(), row["GroupID"].ToString());
					bool result = false;
		...
		private bool SaveData()
		{
			...
			default:
				...
				{
					bool flag = true;
					flag = Factory.UserGroupSystem.AssignGroupsToUser(comboBoxUser.SelectedID, (UserGroupData)currentData);
					if (!flag)
					{
		...
		private void UserGroupAssignForm_Load(object sender, EventArgs e)
		{
			try
			{
		...
		private void InitializeComponent()
		{
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Micromind.ClientUI.Configurations.UserGroupAssignForm));
