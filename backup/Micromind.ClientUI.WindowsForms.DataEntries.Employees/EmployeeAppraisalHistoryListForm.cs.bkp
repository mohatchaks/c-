...
{
	public class EmployeeAppraisalHistoryListForm : Form, IDataForm, IDataList, IExternalReport
	{
		...
		public EmployeeAppraisalHistoryListForm()
		{
			IsCRMList = false;
			InitializeComponent();
		...
		{
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Micromind.ClientUI.WindowsForms.DataEntries.Employees.EmployeeAppraisalHistoryListForm));
			...
			base.Name = "EmployeeAppraisalHistoryListForm";
			Text = "Employee Activity History";
		...
		private void LoadData()
		{
			if (Factory.IsDBConnected)
			{
				try
				{
					...
					employeeData = Factory.EmployeeActivitySystem.GetEmployeeAppraisalHistory(employeeID);
		...
		private void OpenForEdit()
		{
			...
			if (!(selectedID == "") && !(selectedSysDocID == ""))
			{
				FormActivator.BringFormToFront(FormActivator.EmployeeGeneralActivityFormObj);
				FormActivator.EmployeeGeneralActivityFormObj.EditDocument(selectedSysDocID, selectedID);
			}
		}
		...
		private string GetSelectedID()
		{
			...
			return dataGridList.ActiveRow.Cells["Doc Number"].Text.ToString();
		}
		...
		private string GetSelectedSysDocID()
		{
			...
			return dataGridList.ActiveRow.Cells["Doc ID"].Text.ToString();
		}
			...
			if (!isReadOnly && screenRight.Delete)
			{
				...
				if (selectedID != "")
				{
					try
					{
						...
						{
							PublicFunctions.StartWaiting(this);
							if (Factory.CustomerInsuranceSystem.DeleteCustomerInsurance(selectedID))
							{
								try
								{
