...
{
	public class POSCashRegisterPaymentMethodsForm : Form, IForm
	{
		...
		public POSCashRegisterPaymentMethodsForm()
		{
			InitializeComponent();
		...
		{
			if (comboBoxGridPaymentMethod.SelectedPaymentType == PaymentMethodTypes.Check)
			{
				ErrorHelper.InformationMessage("Cheque payments are not supported for Point of Sale.");
				e.Cancel = true;
			}
			if (e.Cell.Column.Key == "Payment Method" && dataGridItems.ExistCellValue("Payment Method", e.NewValue.ToString()) >= 0)
			{
				ErrorHelper.InformationMessage("This payment method is already added. You cannot enter duplicate payment method.");
				e.Cancel = true;
			}
		}
		...
		private void dataGridItems_AfterCellUpdate(object sender, CellEventArgs e)
		{
			...
			if (e.Cell.Column.Key == "Payment Method")
			{
				dataGridItems.ActiveRow.Cells["Display Name"].Value = comboBoxGridPaymentMethod.SelectedName;
				dataGridItems.ActiveRow.Cells["MethodType"].Value = checked((byte)comboBoxGridPaymentMethod.SelectedPaymentType);
			}
		}
		...
		private bool GetData()
		{
			try
			{
				...
				currentData.PaymentMethodTable.Rows.Clear();
				foreach (UltraGridRow row in dataGridItems.Rows)
				{
					DataRow dataRow = currentData.PaymentMethodTable.NewRow();
					dataRow.BeginEdit();
					dataRow["CashRegisterID"] = comboBoxCashRegister.SelectedID;
					dataRow["PaymentMethodID"] = row.Cells["Payment Method"].Value.ToString();
					dataRow["AccountID"] = row.Cells["Account"].Value.ToString();
					dataRow["DisplayName"] = row.Cells["Display Name"].Value.ToString();
					dataRow["Inactive"] = row.Cells["I"].Value.ToString();
		...
		public void LoadData(string registerID)
		{
			try
			{
				...
				{
					DataSet paymentMethodsList = Factory.POSCashRegisterSystem.GetPaymentMethodsList(registerID);
					comboBoxCashRegister.SelectedID = registerID;
					FillData(paymentMethodsList);
					IsNewRecord = false;
		...
		private void FillData(DataSet data)
		{
			if (data != null && data.Tables.Count != 0)
			{
				...
				foreach (DataRow row in data.Tables["POS_CashRegister_PaymentMethod"].Rows)
				{
					DataRow dataRow2 = dataTable.NewRow();
					comboBoxCashRegister.SelectedID = data.Tables["POS_CashRegister_PaymentMethod"].Rows[0]["CashRegisterID"].ToString();
					dataRow2["Account"] = row["AccountID"];
					dataRow2["Account Name"] = row["AccountName"];
					dataRow2["Payment Method"] = row["PaymentMethodID"];
					dataRow2["Display Name"] = row["DisplayName"];
					dataRow2["I"] = bool.Parse(row["Inactive"].ToString());
					dataRow2.EndEdit();
		...
		private void SetupGrid()
		{
			try
			{
				...
				dataTable.Columns.Add("I", typeof(bool));
				...
				dataGridItems.DisplayLayout.Bands[0].Columns["I"].Style = Infragistics.Win.UltraWinGrid.ColumnStyle.CheckBox;
				dataGridItems.DisplayLayout.Bands[0].Columns["I"].DefaultCellValue = false;
				dataGridItems.DisplayLayout.Bands[0].Columns["I"].MaxWidth = 18;
				dataGridItems.DisplayLayout.Bands[0].Columns["MethodType"].Hidden = true;
				...
				dataGridItems.DisplayLayout.Bands[0].Columns["Payment Method"].CharacterCasing = CharacterCasing.Upper;
				dataGridItems.DisplayLayout.Bands[0].Columns["Payment Method"].MaxLength = 64;
				dataGridItems.DisplayLayout.Bands[0].Columns["Payment Method"].InvalidValueBehavior = InvalidValueBehavior.RetainValueAndFocus;
				dataGridItems.DisplayLayout.Bands[0].Columns["Payment Method"].Style = Infragistics.Win.UltraWinGrid.ColumnStyle.DropDownValidate;
				dataGridItems.DisplayLayout.Bands[0].Columns["Payment Method"].ValueList = comboBoxGridPaymentMethod;
		...
		private bool SaveData()
		{
			...
			try
			{
				bool flag = Factory.POSCashRegisterSystem.InsertUpdatePOSCashRegisterPaymentMethods(currentData);
				if (!flag)
				{
		...
		private void InitializeComponent()
		{
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Micromind.ClientUI.WindowsForms.DataEntries.POS.POSCashRegisterPaymentMethodsForm));
			...
			base.Name = "POSCashRegisterPaymentMethodsForm";
			Text = "POS Cash Register Payment Methods";
