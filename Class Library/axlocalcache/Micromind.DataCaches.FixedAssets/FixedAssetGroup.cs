...
{
	public sealed class FixedAssetGroup
	{
		...
		static FixedAssetGroup()
		{
			FixedAssetGroup.Refereshed = null;
			FixedAssetGroup.CacheItemUpdated = null;
			assets = null;
			syncRoot = new object();
			dateTimeStamp = DateTime.MinValue;
			ChangeEvents.FixedAssetGroupChanged += ChangeEvents_AssetChanged;
		}
		...
		public static DataSet GetAssetsGroup(bool isReferesh)
		{
				...
				if ((assets == null) | isReferesh)
				{
					lock (syncRoot)
					{
						if ((assets == null) | isReferesh)
						{
							if (assets != null && isReferesh)
							{
								DateTime tableLastDateTimeStamp = Factory.DatabaseSystem.GetTableLastDateTimeStamp("FixedAsset_Group");
		...
		{
			if (FixedAssetGroup.Refereshed != null)
			{
				FixedAssetGroup.Refereshed();
			}
		}

		private static void OnCacheItemUpdated(object sender, int id, ChangeTypes changeType)
		{
			if (FixedAssetGroup.CacheItemUpdated != null)
			{
				FixedAssetGroup.CacheItemUpdated(sender, new ChangeEventArg(id, changeType));
			}
		}
		...
		private static void LoadChangedItem(int id, ChangeTypes changeType)
		{
			if (assets != null)
			{
				...
				try
				{
					...
					{
						DataRow[] array = assets.Tables["FixedAsset_Group"].Select("AssetGroupID=" + id.ToString());
						if (array.Length != 0)
						{
							assets.Tables["FixedAsset_Group"].Rows.Remove(array[0]);
						}
						assets.Merge(dataSet, preserveChanges: true, MissingSchemaAction.Add);
						_ = dataSet.Tables["FixedAsset_Group"].Rows[0];
