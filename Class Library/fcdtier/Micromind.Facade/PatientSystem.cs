...
{
	public sealed class PatientSystem : MarshalByRefObject, IPatientSystem, IDisposable
	{
		private Config config;

		public PatientSystem(Config config)
		{
			this.config = config;
		}
		...
		public bool CreateCustomer(PatientData data)
		{
			return new Patient(config).InsertUpdateCustomer(data, isUpdate: false);
		}

		public bool UpdateCustomer(PatientData data)
		{
			return UpdateCustomer(data, checkConcurrency: false);
		}

		public bool UpdateCustomer(PatientData data, bool checkConcurrency)
		{
			return new Patient(config).InsertUpdateCustomer(data, isUpdate: true);
		}

		public PatientData GetCustomer()
		{
			using (Patient patient = new Patient(config))
			{
				return patient.GetCustomer();
			}
		}
		...
		public bool DeleteCustomer(string groupID)
		{
			using (Patient patient = new Patient(config))
			{
				return patient.DeleteCustomer(groupID);
			}
		}
		...
		public PatientData GetCustomerByID(string id)
		{
			using (Patient patient = new Patient(config))
			{
				return patient.GetCustomerByID(id);
			}
		}
		...
		public DataSet GetCustomerList()
		{
			using (Patient patient = new Patient(config))
			{
				return patient.GetCustomerList();
			}
		}
		...
		public byte[] GetPatientThumbnailImage(string employeeID)
		{
			return GetPatientThumbnailImage(employeeID, 128, 128);
		}

		public byte[] GetPatientThumbnailImage(string patientID, int width, int height)
		{
			using (Patient patient = new Patient(config))
			{
				byte[] patientThumbnailImage = patient.GetPatientThumbnailImage(patientID);
				if (patientThumbnailImage == null)
				{
					return null;
				}
				return CreateThumbnail(patientThumbnailImage, width, height);
			}
		}
		...
		public bool AddPatientPhoto(string patientID, byte[] pictureByte)
		{
			using (Patient patient = new Patient(config))
			{
				return patient.AddPatientPhoto(patientID, pictureByte);
			}
		}
		...
		public bool RemovePatientPhoto(string patientID)
		{
			using (Patient patient = new Patient(config))
			{
				return patient.RemovePatientPhoto(patientID);
			}
		}

		public DataSet GetCustomerDataToPrint(string fromCustomerID, string toCustomerID)
		{
			using (Patient patient = new Patient(config))
			{
				return patient.GetCustomerDataToPrint(fromCustomerID, toCustomerID);
			}
		}
	}
}
