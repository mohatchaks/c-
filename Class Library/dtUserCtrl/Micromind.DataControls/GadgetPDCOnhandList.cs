...
{
	public class GadgetPDCOnhandList : UserControl, IGadget
	{
		...
		public GadgetTypes GadgetType => GadgetTypes.PDCOnhandList;
		...
		public string Description
		{
			get
			{
				return "List of received PDCs that are not yet matured.";
			}
			set
			{
			}
			...
			try
			{
				if (!isDataLoaded || isRefresh)
				{
					...
					if (dataSet.Tables.Count > 0)
					{
						...
						dataTable.Columns.Remove("SysDocID");
						dataTable.Columns.Remove("VoucherID");
						dataTable.Columns.Remove("BankID");
						dataTable.Columns.Remove("Currency");
						dataTable.Columns.Remove("Bank Name");
						dataTable.Columns.Remove("PayeeType");
		...
		private void backgroundWorker_DoWork(object sender, DoWorkEventArgs e)
		{
			try
			{
				...
				DataSet dataSet = (DataSet)(e.Result = Factory.ReceivedChequeSystemAsync.GetChequesToDepositList(today));
			}
			catch (Exception e2)
			{
				ErrorHelper.ProcessError(e2);
