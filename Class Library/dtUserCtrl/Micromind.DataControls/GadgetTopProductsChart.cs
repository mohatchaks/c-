...
{
	public class GadgetTopProductsChart : UserControl, IGadget
	{
		...
		public string GadgetTitle
		{
			get
			{
				return "Top 10 Product Category";
			}
			set
			{
			}
		...
		public GadgetTypes GadgetType => GadgetTypes.TopProductsChart;
		...
		public string Description
		{
			get
			{
				return "Top products by sales for a selected period.";
			}
			set
			{
			}
		...
		private void backgroundWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
		{
			try
			{
				if (!isDataLoaded || isRefresh)
				{
					...
					{
						chartControl.SetData(dataSet.Tables[0], "Sales", "CategoryName");
					}
					else
					{
						chartControl.Clear();
		...
		private void backgroundWorker_DoWork(object sender, DoWorkEventArgs e)
		{
			try
			{
				...
				DataSet dataSet = (DataSet)(e.Result = Factory.ProductSystemAsync.GetTopProducts(fromDate, toDate, 10));
			}
			catch (Exception e2)
			{
				ErrorHelper.ProcessError(e2);
		...
		private void InitializeComponent()
		{
			...
			comboBoxPeriod.Location = new System.Drawing.Point(49, 1);
			...
			comboData5.Tag = null;
			comboBoxPeriod.Properties.Items.AddRange(new object[4]
			{
				comboData2,
				comboData3,
				comboData4,
			...
			base.Name = "GadgetTopProductsChart";
