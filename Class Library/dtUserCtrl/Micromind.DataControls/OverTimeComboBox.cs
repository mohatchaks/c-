...
{
	public class OverTimeComboBox : MultiColumnComboBox
	{
		...
		public string SelectedRate
		{
			get
			{
				bool.TryParse(GetSelectedCellValue("IsFixed").ToString(), out isFixed);
				if (isFixed)
				{
					return GetSelectedCellValue("FixedAmount").ToString();
				}
				return GetSelectedCellValue("Factor").ToString();
			}
		}

		public OverTimeComboBox()
		{
			InitializeComponent();
			base.ComboType = DataComboType.OverTime;
			TABLENAME_FIELD = "Employee_OverTime";
			ID_FIELD = "OverTimeID";
			NAME_FIELD = "OverTimeName";
		}
		...
		{
			return CombosData.GetOverTimeList(isReferesh);
		}
