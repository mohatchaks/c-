...
{
	public class GadgetReorderItems : UserControl, IGadget
	{
		...
		public GadgetTypes GadgetType => GadgetTypes.ItemsReorderList;
		...
		public string Description
		{
			get
			{
				return "List of items that have balance less than reorder level.";
			}
			set
			{
			}
		...
		public GadgetCategories Category
		{
			get
			{
				return GadgetCategories.Inventory;
			}
			set
			{
			}
		...
		private void backgroundWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
		{
			try
			{
				if (!isDataLoaded || isRefresh)
				{
					...
					{
						FormatInfo displayFormat = columnView.Columns["Reorder"].DisplayFormat;
						displayFormat.FormatType = FormatType.Numeric;
						displayFormat.FormatString = "#,##0";
						FormatInfo displayFormat2 = columnView.Columns["Onhand"].DisplayFormat;
						displayFormat2.FormatType = FormatType.Numeric;
						displayFormat2.FormatString = "#,##0";
					}
		...
		private void backgroundWorker_DoWork(object sender, DoWorkEventArgs e)
		{
			try
			{
				DataSet dataSet = (DataSet)(e.Result = Factory.ProductSystemAsync.GetProductsToReorder());
			}
			catch (Exception e2)
			{
				ErrorHelper.ProcessError(e2);
