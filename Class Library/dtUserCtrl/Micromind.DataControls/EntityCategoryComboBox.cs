...
{
	public class EntityCategoryComboBox : MultiColumnComboBox
	{
		...
		private EntityTypesEnum entityListType = EntityTypesEnum.Customers;
		...
		public EntityTypesEnum EntityListType
		{
			get
			{
		...
		public EntityCategoryComboBox()
		{
			InitializeComponent();
			base.ComboType = DataComboType.EntityCategory;
			TABLENAME_FIELD = "Entity_Category";
			ID_FIELD = "CategoryID";
			NAME_FIELD = "CategoryName";
			ID2_FIELD = "EntityType";
		}
		...
		{
			return CombosData.GetEntityCategoryList(isReferesh);
		}
		...
		public void LoadData(bool isReferesh)
		{
			try
			{
				DataView data = new DataView(GetData(isReferesh).Tables[0], "EntityType = " + (byte)EntityListType, "Code", DataViewRowState.CurrentRows);
				FillData(data);
				if (isSingleColumn)
				{
					base.DisplayMember = "Name";
