...
{
	public class LocationComboBox : MultiColumnComboBox
	{
		...
		public void LoadData(bool isReferesh)
		{
			...
			try
			{
				...
				dataSet = dataSet2;
				object fieldValue = Factory.DatabaseSystem.GetFieldValue("User_Location_Detail", "LocationID", "UserID", Global.CurrentUser.ToString().ToUpper());
				if (fieldValue != null)
				{
					fieldValue.ToString();
					dataSet2.Clear();
					dataSet2 = Factory.LocationSystem.GetUserLocationComboList(Global.CurrentUser.ToString().ToUpper());
					dataSet = dataSet2;
				}
