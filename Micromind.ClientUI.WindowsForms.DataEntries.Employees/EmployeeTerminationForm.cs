...
{
	public class EmployeeTerminationForm : Form, IForm
	{
		private EmployeeActivityData currentData;

		private const string TABLENAME_CONST = "Employee_Termination";
		...
		public EmployeeTerminationForm()
		{
			InitializeComponent();
			AddEvents();
		}
		...
		{
			base.Load += EmployeeTerminationForm_Load;
		}
		...
		private bool GetData()
		{
			try
			{
				...
				{
					currentData = new EmployeeActivityData(EmployeeActivityTypes.Termination);
				}
				...
				dataRow = ((!isNewRecord) ? currentData.EmployeeTerminationTable.Rows[0] : currentData.EmployeeTerminationTable.NewRow());
				dataRow["RequestedBy"] = textBoxRequestedBy.Text;
				if (comboBoxType.SelectedIndex > 0)
				{
					dataRow["TerminationType"] = 'R';
				}
				else
				{
					dataRow["TerminationType"] = 'T';
				}
				dataRow.EndEdit();
				if (isNewRecord)
				{
					currentData.EmployeeTerminationTable.Rows.Add(dataRow);
				}
				return true;
			}
			catch (Exception e)
		...
		private void FillData()
		{
			if (currentData != null && currentData.Tables.Count != 0 && currentData.Tables[0].Rows.Count != 0)
			{
				...
				dataRow = currentData.Tables["Employee_Termination"].Rows[0];
				textBoxRequestedBy.Text = dataRow["RequestedBy"].ToString();
				if (dataRow["TerminationType"].ToString() == "R")
				{
		...
		private bool SaveData()
		{
			...
			try
			{
				bool flag = (!isNewRecord) ? Factory.EmployeeActivitySystem.CreateEmployeeActivity(currentData, EmployeeActivityTypes.Termination, isUpdate: true) : Factory.EmployeeActivitySystem.CreateEmployeeActivity(currentData, EmployeeActivityTypes.Termination, isUpdate: false);
				if (!flag)
				{
		...
		{
			int result = -1;
			int.TryParse(DatabaseHelper.GetNextID("Employee_Termination", "ActivityID", textBoxCode.Text), out result);
			if (result > 0)
			{
				LoadData(result);
			}
		...
		{
			int result = -1;
			int.TryParse(DatabaseHelper.GetPreviousID("Employee_Termination", "ActivityID", textBoxCode.Text), out result);
			if (result > 0)
			{
				LoadData(result);
			}
		...
		{
			int result = -1;
			int.TryParse(DatabaseHelper.GetLastID("Employee_Termination", "ActivityID"), out result);
			if (result > 0)
			{
				LoadData(result);
			}
		...
		{
			int result = -1;
			int.TryParse(DatabaseHelper.GetFirstID("Employee_Termination", "ActivityID"), out result);
			if (result > 0)
			{
				LoadData(result);
			}
		...
		private void EmployeeTerminationForm_Load(object sender, EventArgs e)
		{
			try
			{
				comboBoxType.Items.Add("Terminate");
				comboBoxType.Items.Add("Resign");
				comboBoxType.SelectedIndex = 0;
				SetSecurity();
				if (!base.IsDisposed)
				{
		...
		private void InitializeComponent()
		{
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Micromind.ClientUI.WindowsForms.DataEntries.Employees.EmployeeTerminationForm));
			...
			mmLabel12.Location = new System.Drawing.Point(228, 155);
			mmLabel12.Name = "mmLabel12";
			mmLabel12.PenWidth = 1f;
			mmLabel12.ShowBorder = false;
			mmLabel12.Size = new System.Drawing.Size(108, 13);
			mmLabel12.TabIndex = 0;
			mmLabel12.Text = "Termination Date:";
			...
			dateTimePickerDate.Value = new System.DateTime(2015, 5, 20, 11, 56, 43, 786);
			comboBoxType.DropDownStyle = System.Windows.Forms.ComboBoxStyle.DropDownList;
			comboBoxType.FormattingEnabled = true;
			comboBoxType.Location = new System.Drawing.Point(112, 151);
			comboBoxType.Name = "comboBoxType";
			comboBoxType.Size = new System.Drawing.Size(110, 21);
			...
			ultraFormattedLinkLabel6.Location = new System.Drawing.Point(15, 34);
			...
			mmLabel6.Location = new System.Drawing.Point(11, 156);
			...
			base.Name = "EmployeeTerminationForm";
			Text = "Employee Termination Entry";
