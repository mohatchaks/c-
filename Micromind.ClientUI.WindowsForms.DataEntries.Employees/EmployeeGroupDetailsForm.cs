...
{
	public class EmployeeGroupDetailsForm : Form, IForm
	{
		private EmployeeGroupData currentData;

		private const string TABLENAME_CONST = "Employee_Group";
		...
		public EmployeeGroupDetailsForm()
		{
			InitializeComponent();
			AddEvents();
		}
		...
		{
			base.Load += EmployeeGroupDetailsForm_Load;
		}
		...
		private bool GetData()
		{
			try
			{
				...
				{
					currentData = new EmployeeGroupData();
				}
				DataRow dataRow = (!isNewRecord) ? currentData.EmployeeGroupTable.Rows[0] : currentData.EmployeeGroupTable.NewRow();
				dataRow.BeginEdit();
				...
				if (isNewRecord)
				{
					currentData.EmployeeGroupTable.Rows.Add(dataRow);
				}
				return true;
			}
			catch (Exception e)
		...
		public void LoadData(string id)
		{
			try
			{
				...
				{
					currentData = Factory.EmployeeGroupSystem.GetEmployeeGroupByID(id.Trim());
		...
		private bool SaveData()
		{
			...
			try
			{
				...
				if (isNewRecord)
				{
					flag = Factory.EmployeeGroupSystem.CreateEmployeeGroup(currentData);
					if (flag)
					{
						ComboDataHelper.SetRefreshStatus(DataComboType.EmployeeGroup, needRefresh: true);
					}
				}
				else
				{
					flag = Factory.EmployeeGroupSystem.UpdateEmployeeGroup(currentData);
				}
				if (!flag)
				{
		...
		private bool ValidateData()
		{
			...
			if (isNewRecord && Factory.DatabaseSystem.ExistFieldValue("Employee_Group", "GroupID", textBoxCode.Text.Trim()))
			{
		...
		{
			textBoxCode.Text = PublicFunctions.GetNextCardNumber("Employee_Group", "GroupID");
			textBoxName.Clear();
			textBoxNote.Clear();
		...
		private void EmployeeGroupGroupDetailsForm_Validating(object sender, CancelEventArgs e)
		{
		}

		private void EmployeeGroupGroupDetailsForm_Validated(object sender, EventArgs e)
		{
		}
		...
		private bool Delete()
		{
			try
			{
				...
				return Factory.EmployeeGroupSystem.DeleteEmployeeGroup(textBoxCode.Text);
			}
			catch (Exception e)
			{
				ErrorHelper.ProcessError(e);
		...
		{
			LoadData(DatabaseHelper.GetNextID("Employee_Group", "GroupID", textBoxCode.Text));
		}

		private void toolStripButtonPrevious_Click(object sender, EventArgs e)
		{
			LoadData(DatabaseHelper.GetPreviousID("Employee_Group", "GroupID", textBoxCode.Text));
		}

		private void toolStripButtonLast_Click(object sender, EventArgs e)
		{
			LoadData(DatabaseHelper.GetLastID("Employee_Group", "GroupID"));
		}

		private void toolStripButtonFirst_Click(object sender, EventArgs e)
		{
			LoadData(DatabaseHelper.GetFirstID("Employee_Group", "GroupID"));
		}
		...
		private void toolStripButtonFind_Click(object sender, EventArgs e)
		{
			try
			{
				...
				else if (Factory.DatabaseSystem.ExistFieldValue("Employee_Group", "GroupID", toolStripTextBoxFind.Text.Trim()))
				{
		...
		private void EmployeeGroupDetailsForm_Load(object sender, EventArgs e)
		{
			try
			{
				SetSecurity();
		...
		{
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Micromind.ClientUI.WindowsForms.DataEntries.Employees.EmployeeGroupDetailsForm));
			...
			base.Name = "EmployeeGroupDetailsForm";
			Text = "Employee Group";
