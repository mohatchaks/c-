...
{
	public class PriceListDetailsForm : Form, IForm
	{
		private bool allowEdit = true;

		private PriceListData currentData;

		private const string TABLENAME_CONST = "Price_List";

		private const string IDFIELD_CONST = "VoucherID";

		private const string CUSTOMERIDFIELD_CONST = "CustomerID";
		...
		private void comboBoxCustomer_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			try
			{
				if (comboBoxCustomer.SelectedID != string.Empty)
				{
		...
		private bool GetData()
		{
			try
			{
				...
				DataRow dataRow = (!isNewRecord) ? currentData.PriceListTable.Rows[0] : currentData.PriceListTable.NewRow();
				dataRow["TransactionDate"] = dateTimePickerDate.Value;
				dataRow["ValidDateFrom"] = dateTimePickerValidFrom.Value;
				dataRow["ValidDateTo"] = dateTimePickerValidTo.Value;
				...
				if (IsNewRecord)
				{
					currentData.PriceListTable.Rows.Add(dataRow);
				}
				foreach (UltraGridColumn column in dataGridItems.DisplayLayout.Bands[0].Columns)
				{
					if (!currentData.PriceListDetailTable.Columns.Contains(column.Key))
					{
						currentData.PriceListDetailTable.Columns.Add(column.Key, column.DataType);
					}
				}
				currentData.PriceListDetailTable.Rows.Clear();
				foreach (UltraGridRow row in dataGridItems.Rows)
				{
					DataRow dataRow2 = currentData.PriceListDetailTable.NewRow();
					dataRow2.BeginEdit();
					dataRow2["ProductID"] = row.Cells["Item Code"].Value.ToString();
					dataRow2["CustomerProductID"] = row.Cells["Customer Item Code"].Value.ToString();
					...
					dataRow2.EndEdit();
					currentData.PriceListDetailTable.Rows.Add(dataRow2);
				}
				return true;
			}
			catch (Exception e)
		...
		private void SetupGrid()
		{
			try
			{
				...
				dataTable.Columns.Add("Customer Item Code");
				...
				dataGridItems.DisplayLayout.Bands[0].Columns["Customer Item Code"].CharacterCasing = CharacterCasing.Upper;
				dataGridItems.DisplayLayout.Bands[0].Columns["Customer Item Code"].MaxLength = 64;
				dataGridItems.DisplayLayout.Bands[0].Columns["Customer Item Code"].Width = 150;
				dataGridItems.DisplayLayout.Bands[0].Columns["Customer Item Code"].InvalidValueBehavior = InvalidValueBehavior.RetainValueAndFocus;
		...
		public void LoadData(string voucherID)
		{
			try
			{
				...
				{
					currentData = Factory.PriceListSystem.GetPriceListByID(SystemDocID, voucherID);
					if (currentData == null)
					{
						ClearForm();
						textBoxCustomerName.Text = comboBoxCustomer.SelectedName;
		...
		private void FillData()
		{
			try
			{
				...
				{
					DataRow dataRow = currentData.Tables["Price_List"].Rows[0];
					...
					if (currentData.Tables.Contains("Price_List_Detail") && currentData.PriceListDetailTable.Rows.Count != 0)
					{
						foreach (DataRow row in currentData.Tables["Price_List_Detail"].Rows)
						{
							DataRow dataRow3 = dataTable.NewRow();
							dataRow3["Item Code"] = row["ProductID"];
							dataRow3["Customer Item Code"] = row["CustomerProductID"];
		...
		private bool SaveData(bool clearAfter)
		{
			...
			try
			{
				bool flag = Factory.PriceListSystem.CreatePriceList(currentData, !isNewRecord);
				if (!flag)
				{
		...
		private bool ValidateData()
		{
			...
			if (formManager.IsFieldDirty(textBoxVoucherNumber) && Factory.SystemDocumentSystem.ExistDocumentNumber("Price_List", "VoucherID", SystemDocID, textBoxVoucherNumber.Text))
			{
				ErrorHelper.WarningMessage(UIMessages.DocumentNumberInUse);
		...
		private bool Delete()
		{
			try
			{
				...
				return Factory.PriceListSystem.DeletePriceList(SystemDocID, textBoxVoucherNumber.Text);
			}
			catch (Exception e)
			{
				ErrorHelper.ProcessError(e);
		...
		{
			string nextID = DatabaseHelper.GetNextID("Price_List", "VoucherID", textBoxVoucherNumber.Text, "SysDocID", SystemDocID);
			if (!(nextID == ""))
			{
				LoadData(nextID);
			}
		...
		{
			string previousID = DatabaseHelper.GetPreviousID("Price_List", "VoucherID", textBoxVoucherNumber.Text, "SysDocID", SystemDocID);
			if (!(previousID == ""))
			{
				LoadData(previousID);
			}
		...
		{
			string lastID = DatabaseHelper.GetLastID("Price_List", "VoucherID", "SysDocID", SystemDocID);
			if (!(lastID == ""))
			{
				LoadData(lastID);
			}
		...
		{
			string firstID = DatabaseHelper.GetFirstID("Price_List", "VoucherID", "SysDocID", SystemDocID);
			if (!(firstID == ""))
			{
				LoadData(firstID);
			}
		...
		private void toolStripButtonFind_Click(object sender, EventArgs e)
		{
			try
			{
				...
				{
					string text = Factory.DatabaseSystem.FindDocumentByNumber("Price_List", "VoucherID", SystemDocID, toolStripTextBoxFind.Text);
					if (text != "")
					{
						LoadData(text);
					}
		...
		private void Form_Load(object sender, EventArgs e)
		{
			checked
			{
				try
				{
					...
					comboBoxSysDoc.FilterByType(SysDocTypes.PriceList);
					SetSecurity();
					if (!base.IsDisposed)
					{
						IsNewRecord = true;
		...
		{
			new FormHelper().EditSysDoc(comboBoxSysDoc.SelectedID, SysDocTypes.PriceList);
		}
		...
		private void Print(bool isPrint, bool showPrintDialog, bool saveChanges)
		{
			try
			{
				if (!(IsDirty && saveChanges) || (ErrorHelper.QuestionMessage(MessageBoxButtons.YesNo, "You must save the document before printing.", "Do you want to save?") == DialogResult.Yes && SaveData(clearAfter: false)))
				{
					...
					string text = textBoxVoucherNumber.Text;
					DataSet priceListToPrint = Factory.PriceListSystem.GetPriceListToPrint(selectedID, text);
					if (priceListToPrint == null || priceListToPrint.Tables.Count == 0)
					{
						ErrorHelper.ErrorMessage("Cannot print the document.", "Document not found.");
					}
					else
					{
						PrintHelper.PrintDocument(priceListToPrint, selectedID, "Price List", SysDocTypes.PriceList, isPrint, showPrintDialog);
					}
				}
			}
			catch (Exception e)
		...
		private void buttonSelectDocument_Click(object sender, EventArgs e)
		{
			...
			DataSet priceListByCustomerID = Factory.PriceListSystem.GetPriceListByCustomerID(comboBoxCustomer.SelectedID);
			SelectDocumentDialog selectDocumentDialog = new SelectDocumentDialog();
			selectDocumentDialog.DataSource = priceListByCustomerID;
			selectDocumentDialog.Text = "Select Price List";
			if (selectDocumentDialog.ShowDialog(this) != DialogResult.OK)
			{
				return;
			...
			string sysDocID = selectDocumentDialog.SelectedRow.Cells["Doc ID"].Value.ToString();
			string voucherID = selectDocumentDialog.SelectedRow.Cells["Number"].Value.ToString();
			currentData = Factory.PriceListSystem.GetPriceListByID(sysDocID, voucherID);
			if (currentData == null)
			{
				ClearForm();
				IsNewRecord = true;
			...
			if (currentData.Tables.Contains("Price_List_Detail") && currentData.PriceListDetailTable.Rows.Count != 0)
			{
				foreach (DataRow row in currentData.Tables["Price_List_Detail"].Rows)
				{
					DataRow dataRow2 = dataTable.NewRow();
					dataRow2["Item Code"] = row["ProductID"];
					dataRow2["Customer Item Code"] = row["CustomerProductID"];
		...
		private void InitializeComponent()
		{
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Micromind.ClientUI.WindowsForms.DataEntries.Customers.PriceListDetailsForm));
			...
			checkBoxApplicableToChild.TabIndex = 143;
			...
			buttonSelectDocument.TabIndex = 142;
			...
			dateTimePickerValidFrom.TabIndex = 6;
			...
			comboBoxSalesperson.DisplayLayout.Appearance = appearance3;
			...
			comboBoxSalesperson.DisplayLayout.GroupByBox.Appearance = appearance4;
			appearance5.ForeColor = System.Drawing.SystemColors.GrayText;
			comboBoxSalesperson.DisplayLayout.GroupByBox.BandLabelAppearance = appearance5;
			...
			comboBoxSalesperson.DisplayLayout.GroupByBox.PromptAppearance = appearance6;
			...
			comboBoxSalesperson.DisplayLayout.Override.ActiveCellAppearance = appearance7;
			appearance8.BackColor = System.Drawing.SystemColors.Highlight;
			appearance8.ForeColor = System.Drawing.SystemColors.HighlightText;
			comboBoxSalesperson.DisplayLayout.Override.ActiveRowAppearance = appearance8;
			...
			comboBoxSalesperson.DisplayLayout.Override.CardAreaAppearance = appearance9;
			appearance10.BorderColor = System.Drawing.Color.Silver;
			appearance10.TextTrimming = Infragistics.Win.TextTrimming.EllipsisCharacter;
			comboBoxSalesperson.DisplayLayout.Override.CellAppearance = appearance10;
			...
			comboBoxSalesperson.DisplayLayout.Override.GroupByRowAppearance = appearance11;
			appearance12.TextHAlignAsString = "Left";
			comboBoxSalesperson.DisplayLayout.Override.HeaderAppearance = appearance12;
			...
			comboBoxSalesperson.DisplayLayout.Override.RowAppearance = appearance13;
			comboBoxSalesperson.DisplayLayout.Override.RowSelectors = Infragistics.Win.DefaultableBoolean.False;
			appearance14.BackColor = System.Drawing.SystemColors.ControlLight;
			comboBoxSalesperson.DisplayLayout.Override.TemplateAddRowAppearance = appearance14;
			...
			comboBoxSalesperson.Location = new System.Drawing.Point(420, 45);
			...
			comboBoxSalesperson.Size = new System.Drawing.Size(215, 20);
			comboBoxSalesperson.TabIndex = 8;
			...
			ultraFormattedLinkLabel6.Location = new System.Drawing.Point(348, 48);
			...
			comboBoxCustomer.DisplayLayout.Appearance = appearance19;
			...
			comboBoxCustomer.DisplayLayout.GroupByBox.Appearance = appearance20;
			appearance21.ForeColor = System.Drawing.SystemColors.GrayText;
			comboBoxCustomer.DisplayLayout.GroupByBox.BandLabelAppearance = appearance21;
			...
			comboBoxCustomer.DisplayLayout.GroupByBox.PromptAppearance = appearance22;
			...
			comboBoxCustomer.DisplayLayout.Override.ActiveCellAppearance = appearance23;
			...
			comboBoxCustomer.DisplayLayout.Override.ActiveRowAppearance = appearance24;
			...
			comboBoxCustomer.DisplayLayout.Override.CardAreaAppearance = appearance25;
			appearance26.BorderColor = System.Drawing.Color.Silver;
			appearance26.TextTrimming = Infragistics.Win.TextTrimming.EllipsisCharacter;
			comboBoxCustomer.DisplayLayout.Override.CellAppearance = appearance26;
			...
			comboBoxCustomer.DisplayLayout.Override.GroupByRowAppearance = appearance27;
			appearance28.TextHAlignAsString = "Left";
			comboBoxCustomer.DisplayLayout.Override.HeaderAppearance = appearance28;
			...
			comboBoxCustomer.DisplayLayout.Override.RowAppearance = appearance29;
			comboBoxCustomer.DisplayLayout.Override.RowSelectors = Infragistics.Win.DefaultableBoolean.False;
			appearance30.BackColor = System.Drawing.SystemColors.ControlLight;
			comboBoxCustomer.DisplayLayout.Override.TemplateAddRowAppearance = appearance30;
			...
			base.Name = "PriceListDetailsForm";
			Text = "Price List";
