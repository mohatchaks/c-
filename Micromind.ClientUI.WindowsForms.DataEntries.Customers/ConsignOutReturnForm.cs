...
{
	public class ConsignOutReturnForm : Form, IForm
	{
		private bool allowEdit = true;

		private ConsignOutReturnData currentData;

		private const string TABLENAME_CONST = "ConsignOut_Return";
		...
		private bool GetData()
		{
			try
			{
				...
				{
					currentData = new ConsignOutReturnData();
				}
				DataRow dataRow = (!isNewRecord) ? currentData.ConsignOutReturnTable.Rows[0] : currentData.ConsignOutReturnTable.NewRow();
				...
				if (IsNewRecord)
				{
					currentData.ConsignOutReturnTable.Rows.Add(dataRow);
				}
				currentData.Tables["Product_Lot_Issue_Detail"].Rows.Clear();
				currentData.ConsignOutReturnDetailTable.Rows.Clear();
				foreach (UltraGridRow row in dataGridItems.Rows)
				{
					...
					{
						DataRow dataRow2 = currentData.ConsignOutReturnDetailTable.NewRow();
						dataRow2.BeginEdit();
						...
						dataRow2.EndEdit();
						currentData.ConsignOutReturnDetailTable.Rows.Add(dataRow2);
						row.Cells["Location"].Value.ToString();
						string b = row.Cells["ConsignLocationID"].Value.ToString();
		...
		public void LoadData(string voucherID)
		{
			try
			{
				...
				{
					currentData = Factory.ConsignOutReturnSystem.GetConsignOutReturnByID(SystemDocID, voucherID);
					if (currentData != null)
					{
						FillData();
						IsNewRecord = false;
		...
		private void FillData()
		{
			try
			{
				...
				{
					DataRow dataRow = currentData.Tables["ConsignOut_Return"].Rows[0];
					...
					if (currentData.Tables.Contains("ConsignOut_Return_Detail") && currentData.ConsignOutReturnDetailTable.Rows.Count != 0)
					{
						foreach (DataRow row in currentData.Tables["ConsignOut_Return_Detail"].Rows)
						{
		...
		private bool SaveData(bool clearAfter)
		{
			...
			try
			{
				bool flag = Factory.ConsignOutReturnSystem.CreateConsignOutReturn(currentData, !isNewRecord);
				if (!flag)
				{
		...
		private bool ValidateData()
		{
			...
			if (!IsNewRecord && !Global.IsUserAdmin && !AllowEditTransaction && Global.CurrentUser != Factory.SystemDocumentSystem.GetTransUserID("ConsignOut_Return", "VoucherID", SystemDocID, textBoxVoucherNumber.Text))
			{
			...
			checked
			{
				...
				if (formManager.IsFieldDirty(textBoxVoucherNumber) && Factory.SystemDocumentSystem.ExistDocumentNumber("ConsignOut_Return", "VoucherID", SystemDocID, textBoxVoucherNumber.Text))
				{
					ErrorHelper.WarningMessage(UIMessages.DocumentNumberInUse);
		...
		private bool Delete()
		{
			try
			{
				...
				return Factory.ConsignOutReturnSystem.DeleteConsignOutReturn(SystemDocID, textBoxVoucherNumber.Text);
			}
			catch (Exception e)
			{
				ErrorHelper.ProcessError(e);
		...
		{
			string nextID = DatabaseHelper.GetNextID("ConsignOut_Return", "VoucherID", textBoxVoucherNumber.Text, "SysDocID", SystemDocID);
			if (!(nextID == ""))
			{
				LoadData(nextID);
			}
		...
		{
			string previousID = DatabaseHelper.GetPreviousID("ConsignOut_Return", "VoucherID", textBoxVoucherNumber.Text, "SysDocID", SystemDocID);
			if (!(previousID == ""))
			{
				LoadData(previousID);
			}
		...
		{
			string lastID = DatabaseHelper.GetLastID("ConsignOut_Return", "VoucherID", "SysDocID", SystemDocID);
			if (!(lastID == ""))
			{
				LoadData(lastID);
			}
		...
		{
			string firstID = DatabaseHelper.GetFirstID("ConsignOut_Return", "VoucherID", "SysDocID", SystemDocID);
			if (!(firstID == ""))
			{
				LoadData(firstID);
			}
		...
		private void toolStripButtonFind_Click(object sender, EventArgs e)
		{
			try
			{
				...
				{
					string text = Factory.DatabaseSystem.FindDocumentByNumber("ConsignOut_Return", "VoucherID", SystemDocID, toolStripTextBoxFind.Text);
					if (text != "")
					{
						LoadData(text);
					}
		...
		private void JournalLeavesForm_Load(object sender, EventArgs e)
		{
			try
			{
				...
				SetupGrid();
				comboBoxSysDoc.FilterByType(SysDocTypes.ConsignOutReturn);
				SetSecurity();
				if (!base.IsDisposed)
				{
					IsNewRecord = true;
		...
		private bool Void(bool isVoid)
		{
			try
			{
				...
				return Factory.ConsignOutReturnSystem.VoidConsignOutReturn(SystemDocID, textBoxVoucherNumber.Text, isVoid);
			}
			catch (Exception e)
			{
				ErrorHelper.ProcessError(e);
		...
		{
			new FormHelper().EditSysDoc(comboBoxSysDoc.SelectedID, SysDocTypes.ConsignOutReturn);
		}
		...
		private void Print(bool isPrint, bool showPrintDialog, bool saveChanges)
		{
			try
			{
				if (!(IsDirty && saveChanges) || (ErrorHelper.QuestionMessage(MessageBoxButtons.YesNo, "You must save the document before printing.", "Do you want to save?") == DialogResult.Yes && SaveData(clearAfter: false)))
				{
					...
					string text = textBoxVoucherNumber.Text;
					DataSet consignOutReturnToPrint = Factory.ConsignOutReturnSystem.GetConsignOutReturnToPrint(selectedID, text);
					if (consignOutReturnToPrint == null || consignOutReturnToPrint.Tables.Count == 0)
					{
						ErrorHelper.ErrorMessage("Cannot print the document.", "Document not found.");
					}
					else
					{
						PrintHelper.PrintDocument(consignOutReturnToPrint, selectedID, "ConsignOut Return", SysDocTypes.ConsignOutReturn, isPrint, showPrintDialog);
					}
				}
			}
			catch (Exception e)
		...
		{
			FormActivator.BringFormToFront(FormActivator.ConsignOutReturnListFormObj);
		}
		...
		private void toolStripButtonMultiPreview_Click(object sender, EventArgs e)
		{
			...
			attachementDetailsForm.SysDocType = SysDocTypes.ConsignOutReturn;
		...
		private void InitializeComponent()
		{
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Micromind.ClientUI.WindowsForms.DataEntries.Customers.ConsignOutReturnForm));
			...
			base.Name = "ConsignOutReturnForm";
			Text = "Consignment Out Return";
