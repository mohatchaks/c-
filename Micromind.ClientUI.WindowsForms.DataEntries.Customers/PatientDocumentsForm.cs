...
{
	public class PatientDocumentsForm : Form, IForm
	{
		private PatientDocumentData currentData;

		private const string TABLENAME_CONST = "Patient_Document";
		...
		private MMTextBox textBoxPatientName;
		...
		private PatientDocTypeComboBox comboBoxDocumentType;
		...
		private bool GetData()
		{
			try
			{
				...
				{
					currentData = new PatientDocumentData();
				}
				foreach (UltraGridRow row in dataGrid.Rows)
				{
					DataRow dataRow = currentData.PatientDocumentTable.NewRow();
					dataRow.BeginEdit();
					if (!(row.Cells["Document Number"].Value.ToString() == ""))
					{
						string text = (string)(dataRow["CustomerID"] = ((!isLoadingNewEmployee) ? comboBoxCustomer.SelectedID : comboBoxCustomer.OldValue));
						...
						dataRow.EndEdit();
						currentData.PatientDocumentTable.Rows.Add(dataRow);
					}
				}
				return true;
			}
		...
		public void LoadData(string patientID)
		{
			try
			{
				if (!(patientID.Trim() == ""))
				{
					if (comboBoxCustomer.Text != patientID)
					{
						comboBoxCustomer.SelectedID = patientID;
					}
					if (CanClose())
					{
						currentData = Factory.PatientDocumentSystem.GetPatientDocumentByID(patientID);
						if (currentData != null)
						{
							FillData();
							IsNewRecord = false;
		...
		{
			if (currentData != null && currentData.Tables.Count != 0 && currentData.Tables[0].Rows.Count != 0 && currentData.Tables.Contains("Patient_Document") && currentData.PatientDocumentTable.Rows.Count != 0)
			{
				DataTable dataTable = dataGrid.DataSource as DataTable;
				dataTable.Rows.Clear();
				foreach (DataRow row in currentData.Tables["Patient_Document"].Rows)
				{
		...
		private bool SaveData()
		{
			...
			try
			{
				...
				if (isNewRecord)
				{
					flag = Factory.PatientDocumentSystem.CreatePatientDocument(currentData);
				}
				else
				{
					flag = Factory.PatientDocumentSystem.UpdatePatientDocument(currentData);
		...
		private bool Delete()
		{
			try
			{
				...
				return Factory.EmployeeDocumentSystem.DeleteEmployeeDocument(comboBoxCustomer.SelectedID);
			}
			catch (Exception e)
			{
				ErrorHelper.ProcessError(e);
		...
		{
			string nextID = DatabaseHelper.GetNextID("Patient_Document", "CustomerID", comboBoxCustomer.SelectedID);
			if (!(nextID == ""))
			{
				comboBoxCustomer.SelectedID = nextID;
			}
		}

		private void toolStripButtonPrevious_Click(object sender, EventArgs e)
		{
			string previousID = DatabaseHelper.GetPreviousID("Patient_Document", "CustomerID", comboBoxCustomer.SelectedID);
			if (!(previousID == ""))
			{
				comboBoxCustomer.SelectedID = previousID;
			}
		}

		private void toolStripButtonLast_Click(object sender, EventArgs e)
		{
			string lastID = DatabaseHelper.GetLastID("Patient_Document", "CustomerID");
			if (!(lastID == ""))
			{
				comboBoxCustomer.SelectedID = lastID;
			}
		}

		private void toolStripButtonFirst_Click(object sender, EventArgs e)
		{
			string firstID = DatabaseHelper.GetFirstID("Patient_Document", "CustomerID");
			if (!(firstID == ""))
			{
				comboBoxCustomer.SelectedID = firstID;
			}
		}
		...
		private void toolStripButtonFind_Click(object sender, EventArgs e)
		{
			try
			{
				...
				else if (Factory.DatabaseSystem.ExistFieldValue("Patient_Document", "CustomerID", toolStripTextBoxFind.Text.Trim()))
				{
					string selectedID = toolStripTextBoxFind.Text.Trim();
					comboBoxCustomer.SelectedID = selectedID;
				}
				else
				{
		...
		{
			if (comboBoxCustomer.SelectedRow != null && comboBoxCustomer.SelectedID != "")
			{
				dataGrid.Enabled = true;
				textBoxPatientName.Text = comboBoxCustomer.SelectedRow.Cells["Name"].Text.ToString();
				...
				{
					comboBoxCustomer.SelectedID = comboBoxCustomer.OldValue;
				}
			}
			else
			{
		...
		{
			new FormHelper().EditPatient(comboBoxCustomer.SelectedID);
		}
		...
		private void InitializeComponent()
		{
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Micromind.ClientUI.WindowsForms.DataEntries.Customers.PatientDocumentsForm));
			...
			textBoxPatientName = new Micromind.UISupport.MMTextBox();
			...
			comboBoxDocumentType = new Micromind.DataControls.PatientDocTypeComboBox();
			...
			mmLabel2.TabIndex = 2;
			mmLabel2.Text = "Patient Name:";
			textBoxPatientName.BackColor = System.Drawing.Color.WhiteSmoke;
			textBoxPatientName.CustomReportFieldName = "";
			textBoxPatientName.CustomReportKey = "";
			textBoxPatientName.CustomReportValueType = 1;
			textBoxPatientName.Enabled = false;
			textBoxPatientName.IsComboTextBox = false;
			textBoxPatientName.IsModified = false;
			textBoxPatientName.Location = new System.Drawing.Point(135, 29);
			textBoxPatientName.MaxLength = 255;
			textBoxPatientName.Name = "textBoxPatientName";
			textBoxPatientName.ReadOnly = true;
			textBoxPatientName.Size = new System.Drawing.Size(229, 20);
			textBoxPatientName.TabIndex = 3;
			...
			nonDirtyPanel1.Controls.Add(comboBoxCustomer);
			nonDirtyPanel1.Controls.Add(LabelEmployee);
			nonDirtyPanel1.Controls.Add(textBoxPatientName);
			...
			comboBoxCustomer.DescriptionTextBox = textBoxPatientName;
			...
			comboBoxCustomer.Location = new System.Drawing.Point(135, 3);
			...
			comboBoxCustomer.Size = new System.Drawing.Size(229, 20);
			comboBoxCustomer.TabIndex = 164;
			comboBoxCustomer.UseFlatMode = Infragistics.Win.DefaultableBoolean.True;
			comboBoxCustomer.SelectedIndexChanged += new System.EventHandler(comboBoxCustomer_SelectedIndexChanged);
			...
			LabelEmployee.Size = new System.Drawing.Size(79, 15);
			...
			LabelEmployee.Value = "Patient Code:";
			...
			comboBoxDocumentType.Location = new System.Drawing.Point(553, 57);
			...
			base.Name = "PatientDocumentsForm";
