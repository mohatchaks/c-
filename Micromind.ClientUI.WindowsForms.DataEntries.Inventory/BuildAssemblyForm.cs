...
{
	public class BuildAssemblyForm : Form, IForm
	{
		private AssemblyBuildData currentData;

		private const string TABLENAME_CONST = "Assembly_Build";
		...
		private ProductComboBox comboBoxAssembly;
		...
		private UnitPriceTextBox textBoxUnitCost;
		...
		public BuildAssemblyForm()
		{
			...
			checked
			{
				...
				comboBoxAssembly.ShowOnlyAssemblyItems = true;
		...
		private void comboBoxBOM_SelectedIndexChanged(object sender, EventArgs e)
		{
			try
			{
				...
				decimal d = (textBoxQuantity.Text.Trim() != "") ? decimal.Parse(textBoxQuantity.Text) : 0m;
				foreach (DataRow row in bOMByID.Tables["BOM_Detail"].Rows)
				{
					...
					{
						num = int.Parse(row["Quantity"].ToString());
					}
					if (row["Cost"] != null && row["Cost"].ToString() != "")
					{
						num2 = int.Parse(row["Cost"].ToString());
					}
					dataRow2["Qty"] = (decimal)num * d;
					dataRow2["Quantity"] = num;
					dataRow2["Cost"] = num2.ToString();
					dataRow2.EndEdit();
		...
		private void comboBoxAssembly_SelectedIndexChanged(object sender, EventArgs e)
		{
			try
			{
				if (comboBoxAssembly.SelectedRow != null && comboBoxAssembly.SelectedRow.Cells.Exists("BOMID"))
				{
					comboBoxBOM.SelectedID = comboBoxAssembly.SelectedRow.Cells["BOMID"].Value.ToString();
				}
				else
				{
					comboBoxBOM.Clear();
				...
				if (dataGridItems.Rows.Count != 0)
				{
					...
					foreach (UltraGridRow row2 in dataGridItems.Rows)
					{
						if (row2.Cells["Item Code"].Value != null && !(row2.Cells["Item Code"].Value.ToString() == ""))
						{
							...
							decimal.TryParse(row2.GetCellValue("Qty").ToString(), out result);
							...
							if (row2.Cells["Qty"].Value != null && row2.Cells["Qty"].Value.ToString() != "")
							{
								d = decimal.Parse(row2.Cells["Qty"].Value.ToString());
							}
							if (row2.Cells["Cost"].Value != null && row2.Cells["Cost"].Value.ToString() != "")
							{
								d2 = decimal.Parse(row2.Cells["Cost"].Value.ToString());
							}
							row2.Cells["Total"].Value = (d * d2).ToString(Format.TotalAmountFormat);
						}
					}
				}
			}
		...
		private void CalculateTotalCost()
		{
			...
			try
			{
				foreach (UltraGridRow row in dataGridItems.Rows)
				{
					...
					{
						decimal.TryParse(row.Cells["Total"].Value.ToString(), out result2);
						d += result2;
					}
				}
				decimal d2 = default(decimal);
				...
				if (num > 0m && result > 0m)
				{
					textBoxUnitCost.Text = (num / result).ToString(Format.TotalAmountFormat);
				}
				else
				{
					textBoxUnitCost.Text = num.ToString(Format.TotalAmountFormat);
				}
			}
			catch (Exception e)
			{
		...
		private void CalculateUnitCost()
		{
			...
			try
			{
				...
				decimal.TryParse(textBoxQuantity.Text, out result2);
				if (result > 0m && result2 > 0m)
				{
					textBoxUnitCost.Text = (result / result2).ToString(Format.TotalAmountFormat);
				}
				else
				{
				...
				foreach (UltraGridRow row in dataGridItems.Rows)
				{
					...
					{
						decimal.TryParse(row.Cells["Qty"].Value.ToString(), out result3);
						d += result3;
					}
				}
				textBoxWeight.Text = (d / result2).ToString(Format.QuantityFormat);
			}
			catch (Exception e)
			{
				ErrorHelper.ProcessError(e);
		...
		private void dataGridItems_AfterCellUpdate(object sender, CellEventArgs e)
		{
			...
			if (e.Cell.Column.Key == "Item Code")
			{
				...
				dataGridItems.ActiveRow.Cells["Qty"].Value = 0;
				dataGridItems.ActiveRow.Cells["Unit"].Value = comboBoxGridItem.SelectedUnitID;
				dataGridItems.ActiveRow.Cells["ItemType"].Value = comboBoxGridItem.SelectedItemType;
				dataGridItems.ActiveRow.Cells["Unit"].ValueList = comboBoxGridItem.GetProductUnitsValueList(comboBoxGridItem.SelectedID);
				if (comboBoxGridItem.SelectedID != "" && (dataGridItems.ActiveRow.Cells["Location"].Value == null || dataGridItems.ActiveRow.Cells["Location"].Value.ToString() == "") && dataGridItems.ActiveRow.Index > 0)
				{
			...
			else if (isNewRecord && e.Cell.Column.Key == "Qty")
			{
				float result3 = 0f;
				float result4 = 0f;
				float.TryParse(e.Cell.Row.Cells["Qty"].Text.ToString(), out result3);
				float.TryParse(e.Cell.Row.Cells["Cost"].Value.ToString(), out result4);
				e.Cell.Row.Cells["Total"].Value = (result3 * result4).ToString(Format.TotalAmountFormat);
				CalculateTotalCost();
				CalculateUnitCost();
			}
			else if (isNewRecord && e.Cell.Column.Key == "Onhand" && e.Cell.Row.Cells["Qty"].Value != null && e.Cell.Row.Cells["Qty"].Value.ToString() != "")
			{
				float result5 = 0f;
				float result6 = 0f;
				float.TryParse(e.Cell.Row.Cells["Qty"].Text.ToString(), out result6);
				float.TryParse(e.Cell.Row.Cells["Onhand"].Value.ToString(), out result5);
				CalculateTotalCost();
			}
			if (e.Cell.Column.Key == "Qty" && dataGridItems.ActiveCell != null && (dataGridItems.ActiveCell.Column.Key == "Qty" || dataGridItems.ActiveCell.Column.Key == "Qty"))
			{
				...
				float result7 = 0f;
				float result8 = 0f;
				float.TryParse(e.Cell.Row.Cells["Qty"].Text.ToString(), out result7);
				float.TryParse(e.Cell.Row.Cells["Cost"].Value.ToString(), out result8);
				e.Cell.Row.Cells["Total"].Value = (result7 * result8).ToString(Format.TotalAmountFormat);
				decimal d = default(decimal);
				foreach (UltraGridRow row3 in dataGridItems.Rows)
				{
					decimal result9 = default(decimal);
					if (row3.Cells["Qty"].Value != null && !(row3.Cells["Qty"].Value.ToString() == ""))
					{
						decimal.TryParse(row3.Cells["Qty"].Value.ToString(), out result9);
						d += result9;
					}
				}
				decimal result10 = default(decimal);
				decimal.TryParse(textBoxQuantity.Text, out result10);
				textBoxWeight.Text = (d / result10).ToString(Format.QuantityFormat);
				CalculateTotalCost();
			}
		...
		private void dataGrid_BeforeRowDeactivate(object sender, CancelEventArgs e)
		{
			...
			if (activeRow.Cells["Qty"].Value.ToString() == "")
			{
		...
		private bool GetData()
		{
			try
			{
				...
				{
					currentData = new AssemblyBuildData();
				}
				DataRow dataRow = (!isNewRecord) ? currentData.AssemblyBuildTable.Rows[0] : currentData.AssemblyBuildTable.NewRow();
				...
				dataRow["AssemblyProductID"] = comboBoxAssembly.SelectedID;
				dataRow["Reference"] = textBoxRef1.Text;
				dataRow["Description"] = textBoxNote.Text;
				dataRow["QuantityBuild"] = ((!string.IsNullOrEmpty(textBoxQuantity.Text)) ? Convert.ToDecimal(textBoxQuantity.Text) : 0m);
				dataRow["UnitWeight"] = ((!string.IsNullOrEmpty(textBoxWeight.Text)) ? Convert.ToDecimal(textBoxWeight.Text) : 0m);
				dataRow["UnitCost"] = textBoxUnitCost.Text;
				...
				{
					DateTime value2 = dateTimePickerWorkCompDate.Value;
					dataRow["WorkCompDate"] = new DateTime(value2.Year, value2.Month, value2.Day, 11, 59, 59);
				}
				dataRow.EndEdit();
				if (IsNewRecord)
				{
					currentData.AssemblyBuildTable.Rows.Add(dataRow);
				}
				currentData.AssemblyBuildDetailTable.Rows.Clear();
				foreach (UltraGridRow row in dataGridItems.Rows)
				{
					DataRow dataRow2 = currentData.AssemblyBuildDetailTable.NewRow();
					dataRow2.BeginEdit();
					dataRow2["BOMProductID"] = row.Cells["Item Code"].Value.ToString();
					if (row.Cells["Qty"].Value != null && row.Cells["Qty"].Value.ToString() != "")
					{
						dataRow2["Quantity"] = row.Cells["Qty"].Value.ToString();
					}
					...
					dataRow2.EndEdit();
					currentData.AssemblyBuildDetailTable.Rows.Add(dataRow2);
				}
				currentData.AssemblyBuildExpenseTable.Rows.Clear();
				foreach (UltraGridRow row2 in dataGridExpense.Rows)
				{
					DataRow dataRow3 = currentData.AssemblyBuildExpenseTable.NewRow();
					dataRow3.BeginEdit();
					...
					dataRow3.EndEdit();
					currentData.AssemblyBuildExpenseTable.Rows.Add(dataRow3);
				}
				return true;
			}
			catch (Exception e)
		...
		private void SetupGrid()
		{
			try
			{
				...
				dataGridItems.DisplayLayout.Bands[0].Columns["Onhand"].Hidden = true;
				...
				dataGridItems.DisplayLayout.Bands[0].Columns["Factor Qty"].Header.VisiblePosition = 7;
				...
				dataGridItems.DisplayLayout.Bands[0].Columns["Factor Qty"].Format = "#,##0.00##";
		...
		public void LoadData(string voucherID)
		{
			try
			{
				...
				{
					currentData = Factory.AssemblyBuildSystem.GetAssemblyBuildByID(SystemDocID, voucherID);
					if (currentData != null)
					{
						FillData();
						IsNewRecord = false;
		...
		private void FillData()
		{
			try
			{
				...
				{
					DataRow dataRow = currentData.Tables["Assembly_Build"].Rows[0];
					dateTimePickerDate.Value = DateTime.Parse(dataRow["TransactionDate"].ToString());
					textBoxVoucherNumber.Text = dataRow["VoucherID"].ToString();
					textBoxRef1.Text = dataRow["Reference"].ToString();
					comboBoxLocation.SelectedID = dataRow["LocationID"].ToString();
					comboBoxAssembly.SelectedID = dataRow["AssemblyProductID"].ToString();
					textBoxNote.Text = dataRow["Description"].ToString();
					textBoxQuantity.Text = dataRow["QuantityBuild"].ToString();
					textBoxWeight.Text = dataRow["UnitWeight"].ToString();
					textBoxUnitCost.Text = dataRow["UnitCost"].ToString();
					...
					DataTable dataTable = dataGridItems.DataSource as DataTable;
					dataTable.Rows.Clear();
					if (currentData.Tables.Contains("Assembly_Build_Detail") && currentData.AssemblyBuildDetailTable.Rows.Count != 0)
					{
						dataGridItems.BeginUpdate();
						foreach (DataRow row in currentData.Tables["Assembly_Build_Detail"].Rows)
						{
							DataRow dataRow3 = dataTable.NewRow();
							dataRow3["Item Code"] = row["BOMProductID"];
						...
						foreach (DataRow row3 in currentData.Tables["Assembly_Build_Expense"].Rows)
						{
		...
		private bool SaveData(bool clearAfter)
		{
			...
			try
			{
				bool flag = Factory.AssemblyBuildSystem.CreateAssemblyBuild(currentData, !isNewRecord);
				if (!flag)
				{
		...
		{
			if (!IsNewRecord && !Global.IsUserAdmin && !AllowEditTransaction && Global.CurrentUser != Factory.SystemDocumentSystem.GetTransUserID("Assembly_Build", "VoucherID", SystemDocID, textBoxVoucherNumber.Text))
			{
			...
			checked
			{
				...
				if (dateTimePickerWorkCompDate.Value < dateTimePickerDate.Value)
				{
					ErrorHelper.WarningMessage("Work completion date should not be back date.");
					return false;
				}
				if (textBoxVoucherNumber.Text.Trim() == "" || comboBoxSysDoc.SelectedID == "" || comboBoxAssembly.SelectedID == "" || comboBoxLocation.SelectedID == "" || textBoxQuantity.Text == "")
				{
				...
				if (formManager.IsFieldDirty(textBoxVoucherNumber) && Factory.SystemDocumentSystem.ExistDocumentNumber("Assembly_Build", "VoucherID", SystemDocID, textBoxVoucherNumber.Text))
				{
					ErrorHelper.WarningMessage(UIMessages.DocumentNumberInUse);
		...
		private decimal GetTransactionBalance()
		{
			...
			foreach (UltraGridRow row in dataGridItems.Rows)
			{
				if (row.GetCellValue("Cost") != null && row.GetCellValue("Cost").ToString() != "")
				{
					result += decimal.Parse(row.Cells["Cost"].Value.ToString());
				}
			}
			return result;
		}
		...
		private void ClearForm()
		{
			try
			{
				...
				textBoxUnitCost.Text = 0.ToString(Format.TotalAmountFormat);
				comboBoxGridItem.Clear();
				comboBoxAssembly.Clear();
		...
		private bool Delete()
		{
			try
			{
				...
				return Factory.AssemblyBuildSystem.DeleteAssemblyBuild(SystemDocID, textBoxVoucherNumber.Text);
			}
			catch (Exception e)
			{
				ErrorHelper.ProcessError(e);
		...
		{
			string nextID = DatabaseHelper.GetNextID("Assembly_Build", "VoucherID", textBoxVoucherNumber.Text, "SysDocID", SystemDocID);
			if (!(nextID == ""))
			{
				LoadData(nextID);
			}
		...
		{
			string previousID = DatabaseHelper.GetPreviousID("Assembly_Build", "VoucherID", textBoxVoucherNumber.Text, "SysDocID", SystemDocID);
			if (!(previousID == ""))
			{
				LoadData(previousID);
			}
		...
		{
			string lastID = DatabaseHelper.GetLastID("Assembly_Build", "VoucherID", "SysDocID", SystemDocID);
			if (!(lastID == ""))
			{
				LoadData(lastID);
			}
		...
		{
			string firstID = DatabaseHelper.GetFirstID("Assembly_Build", "VoucherID", "SysDocID", SystemDocID);
			if (!(firstID == ""))
			{
				LoadData(firstID);
			}
		...
		private void toolStripButtonFind_Click(object sender, EventArgs e)
		{
			try
			{
				...
				{
					string text = Factory.DatabaseSystem.FindDocumentByNumber("Assembly_Build", "VoucherID", SystemDocID, toolStripTextBoxFind.Text);
					if (text != "")
					{
						LoadData(text);
					}
		...
		private void BuildAssemblyForm_Load(object sender, EventArgs e)
		{
			try
			{
				...
				SetupExpenseGrid();
				comboBoxSysDoc.FilterByType(SysDocTypes.AssemblyBuild);
				dateTimePickerDate.Value = DateTime.Now;
				SetSecurity();
				if (!base.IsDisposed)
				{
		...
		{
			new FormHelper().EditSysDoc(comboBoxSysDoc.SelectedID, SysDocTypes.AssemblyBuild);
		}
		...
		private bool SaveDraft()
		{
			try
			{
				if (GetData())
				{
					...
					{
						return Global.CompanySettings.SaveTransactionDraft(currentData, enterNameDialog.EnteredName, SysDocTypes.AssemblyBuild);
					}
				}
				return false;
			}
		...
		private bool LoadDraft()
		{
			try
			{
				DataSet settingsList = Factory.SettingSystem.GetSettingsList("", 67.ToString());
				...
				if (selectDocumentDialog.ShowDialog(this) == DialogResult.OK)
				{
					...
					DataSet dataSet = Global.CompanySettings.LoadTransactionDraft(key, SysDocTypes.AssemblyBuild);
					currentData = (dataSet as AssemblyBuildData);
					FillData();
				}
				return true;
			}
		...
		private void Print(bool isPrint, bool showPrintDialog, bool saveChanges)
		{
			try
			{
				if (!(IsDirty && saveChanges) || (ErrorHelper.QuestionMessage(MessageBoxButtons.YesNo, "You must save the document before printing.", "Do you want to save?") == DialogResult.Yes && SaveData(clearAfter: false)))
				{
					...
					DataSet assemblyBuildToPrint = Factory.AssemblyBuildSystem.GetAssemblyBuildToPrint(selectedID, text);
					if (assemblyBuildToPrint == null || assemblyBuildToPrint.Tables.Count == 0)
					{
						ErrorHelper.ErrorMessage("Cannot print the document.", "Document not found.");
					}
					else
					{
						PrintHelper.PrintDocument(assemblyBuildToPrint, selectedID, "Assembly Build", SysDocTypes.AssemblyBuild, isPrint, showPrintDialog);
					}
				}
			}
			catch (Exception e)
		...
		{
			new FormHelper().EditProduct(comboBoxAssembly.SelectedID);
		}
		...
		{
			FormActivator.BringFormToFront(FormActivator.BuildAssemblyListFormObj);
		}
		...
		private void loadFromMaterialToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			if (selectDocumentDialog.ShowDialog(this) == DialogResult.OK)
			{
				...
				foreach (UltraGridRow selectedRow in selectDocumentDialog.SelectedRows)
				{
					...
					string text = selectedRow.Cells["CostCategoryID"].Value.ToString();
					JobMaterialEstimateData jobMaterialEstimateByJobIDCostCategoryID = Factory.JobMaterialEstimateSystem.GetJobMaterialEstimateByJobIDCostCategoryID(sysDocID, voucherID, jobID, text);
					...
					comboBoxLocation.SelectedID = dataRow["LocationID"].ToString();
					comboBoxJob.SelectedID = dataRow["JobID"].ToString();
					comboBoxCostCategory.SelectedID = text.ToString();
					DataTable dataTable = dataGridItems.DataSource as DataTable;
					dataTable.Rows.Clear();
		...
		private float SelectFactoryQty(string itemCode, float CurrentQty)
		{
			float result = 0f;
			float result2 = 0f;
			...
			{
				float.TryParse(factoryQty.Tables[0].Rows[0]["Factor"].ToString(), out result2);
				result = ((!(factoryQty.Tables[0].Rows[0]["FactorType"].ToString() == "D")) ? (CurrentQty * result2) : (CurrentQty / result2));
			}
			return result;
		}
		...
		private void InitializeComponent()
		{
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Micromind.ClientUI.WindowsForms.DataEntries.Inventory.BuildAssemblyForm));
			...
			textBoxUnitCost = new Micromind.UISupport.UnitPriceTextBox();
			comboBoxJob = new Micromind.DataControls.JobComboBox();
			label7 = new System.Windows.Forms.Label();
			textBoxQuantity = new Micromind.UISupport.QuantityTextBox();
			label6 = new System.Windows.Forms.Label();
			comboBoxSysDoc = new Micromind.DataControls.SysDocComboBox();
			comboBoxAssembly = new Micromind.DataControls.ProductComboBox();
			...
			((System.ComponentModel.ISupportInitialize)comboBoxAssembly).BeginInit();
			...
			tabPageItems.Size = new System.Drawing.Size(708, 242);
			...
			dataGridItems.Size = new System.Drawing.Size(704, 239);
			...
			tabPageExpense.Size = new System.Drawing.Size(708, 242);
			...
			label323.Size = new System.Drawing.Size(563, 18);
			...
			label1.Location = new System.Drawing.Point(438, 51);
			label1.Name = "label1";
			label1.Size = new System.Drawing.Size(60, 13);
			label1.TabIndex = 20;
			label1.Text = "Reference:";
			textBoxRef1.Location = new System.Drawing.Point(501, 48);
			textBoxRef1.MaxLength = 30;
			textBoxRef1.Name = "textBoxRef1";
			textBoxRef1.Size = new System.Drawing.Size(136, 20);
			textBoxRef1.TabIndex = 7;
			textBoxNote.Location = new System.Drawing.Point(93, 122);
			textBoxNote.MaxLength = 255;
			textBoxNote.Name = "textBoxNote";
			textBoxNote.Size = new System.Drawing.Size(544, 20);
			textBoxNote.TabIndex = 13;
			label3.AutoSize = true;
			label3.Location = new System.Drawing.Point(15, 125);
			label3.Name = "label3";
			...
			appearance75.BackColor = System.Drawing.Color.FromArgb(194, 216, 252);
			appearance75.BorderColor = System.Drawing.Color.FromArgb(122, 150, 223);
			ultraGroupBox1.Appearance = appearance75;
			...
			ultraGroupBox1.Location = new System.Drawing.Point(11, 449);
			...
			appearance76.BorderColor = System.Drawing.Color.FromArgb(122, 150, 223);
			appearance76.FontData.BoldAsString = "True";
			appearance76.FontData.Name = "Tahoma";
			appearance76.ForeColor = System.Drawing.Color.FromArgb(194, 216, 252);
			appearance76.TextHAlignAsString = "Right";
			appearance76.TextVAlignAsString = "Middle";
			ultraLabel1.Appearance = appearance76;
			...
			panelDetails.Controls.Add(comboBoxAssembly);
			...
			panelDetails.Size = new System.Drawing.Size(655, 145);
			...
			textBoxWeight.Location = new System.Drawing.Point(370, 70);
			textBoxWeight.Name = "textBoxWeight";
			textBoxWeight.ReadOnly = true;
			textBoxWeight.Size = new System.Drawing.Size(105, 20);
			textBoxWeight.TabIndex = 164;
			...
			labelcostcategory.VisitedLinkAppearance = appearance78;
			...
			labelJob.Appearance = appearance79;
			labelJob.AutoSize = true;
			labelJob.Location = new System.Drawing.Point(243, 99);
			labelJob.Name = "labelJob";
			labelJob.Size = new System.Drawing.Size(25, 15);
			labelJob.TabIndex = 162;
			...
			labelJob.VisitedLinkAppearance = appearance80;
			...
			ultraFormattedLinkLabel6.Location = new System.Drawing.Point(227, 51);
			...
			ultraFormattedLinkLabel3.Appearance = appearance83;
			ultraFormattedLinkLabel3.AutoSize = true;
			ultraFormattedLinkLabel3.Location = new System.Drawing.Point(17, 52);
			...
			ultraFormattedLinkLabel3.VisitedLinkAppearance = appearance84;
			ultraFormattedLinkLabel3.LinkClicked += new Infragistics.Win.FormattedLinkLabel.LinkClickedEventHandler(ultraFormattedLinkLabel3_LinkClicked);
			label8.AutoSize = true;
			label8.Location = new System.Drawing.Point(18, 94);
			label8.Name = "label8";
			label8.Size = new System.Drawing.Size(69, 26);
			label8.TabIndex = 159;
			label8.Text = "Work Comp. \r\nDate:";
			dateTimePickerWorkCompDate.Format = System.Windows.Forms.DateTimePickerFormat.Short;
			dateTimePickerWorkCompDate.Location = new System.Drawing.Point(96, 96);
			dateTimePickerWorkCompDate.Name = "dateTimePickerWorkCompDate";
			dateTimePickerWorkCompDate.Size = new System.Drawing.Size(142, 20);
			dateTimePickerWorkCompDate.TabIndex = 12;
			...
			comboBoxCostCategory.Location = new System.Drawing.Point(479, 97);
			...
			comboBoxCostCategory.Size = new System.Drawing.Size(83, 20);
			comboBoxCostCategory.TabIndex = 11;
			...
			textBoxUnitCost.Location = new System.Drawing.Point(253, 70);
			textBoxUnitCost.Name = "textBoxUnitCost";
			textBoxUnitCost.ReadOnly = true;
			textBoxUnitCost.Size = new System.Drawing.Size(60, 20);
			textBoxUnitCost.TabIndex = 9;
			textBoxUnitCost.TextAlign = System.Windows.Forms.HorizontalAlignment.Right;
			...
			comboBoxJob.Location = new System.Drawing.Point(272, 97);
			comboBoxJob.MaxDropDownItems = 12;
			comboBoxJob.Name = "comboBoxJob";
			comboBoxJob.ShowInactiveItems = false;
			comboBoxJob.ShowQuickAdd = true;
			comboBoxJob.Size = new System.Drawing.Size(126, 20);
			comboBoxJob.TabIndex = 10;
			comboBoxJob.UseFlatMode = Infragistics.Win.DefaultableBoolean.True;
			label7.AutoSize = true;
			label7.Location = new System.Drawing.Point(195, 73);
			label7.Name = "label7";
			...
			textBoxQuantity.Location = new System.Drawing.Point(93, 70);
			textBoxQuantity.Name = "textBoxQuantity";
			textBoxQuantity.Size = new System.Drawing.Size(99, 20);
			...
			comboBoxAssembly.AllowedItemTypes = new Micromind.Common.Data.ItemTypes[0];
			comboBoxAssembly.Assigned = false;
			comboBoxAssembly.AutoCompleteMode = Infragistics.Win.AutoCompleteMode.Append;
			comboBoxAssembly.CalcManager = ultraCalcManager1;
			comboBoxAssembly.CharacterCasing = System.Windows.Forms.CharacterCasing.Upper;
			comboBoxAssembly.CustomReportFieldName = "";
			comboBoxAssembly.CustomReportKey = "";
			comboBoxAssembly.CustomReportValueType = 1;
			comboBoxAssembly.DescriptionTextBox = textBoxAssemblyName;
			appearance85.BackColor = System.Drawing.SystemColors.Window;
			appearance85.BorderColor = System.Drawing.SystemColors.InactiveCaption;
			comboBoxAssembly.DisplayLayout.Appearance = appearance85;
			comboBoxAssembly.DisplayLayout.BorderStyle = Infragistics.Win.UIElementBorderStyle.Solid;
			comboBoxAssembly.DisplayLayout.CaptionVisible = Infragistics.Win.DefaultableBoolean.False;
			...
			comboBoxAssembly.DisplayLayout.GroupByBox.Appearance = appearance86;
			appearance87.ForeColor = System.Drawing.SystemColors.GrayText;
			comboBoxAssembly.DisplayLayout.GroupByBox.BandLabelAppearance = appearance87;
			comboBoxAssembly.DisplayLayout.GroupByBox.BorderStyle = Infragistics.Win.UIElementBorderStyle.Solid;
			...
			comboBoxAssembly.DisplayLayout.GroupByBox.PromptAppearance = appearance88;
			comboBoxAssembly.DisplayLayout.MaxColScrollRegions = 1;
			comboBoxAssembly.DisplayLayout.MaxRowScrollRegions = 1;
			appearance89.BackColor = System.Drawing.SystemColors.Window;
			appearance89.ForeColor = System.Drawing.SystemColors.ControlText;
			comboBoxAssembly.DisplayLayout.Override.ActiveCellAppearance = appearance89;
			...
			comboBoxAssembly.DisplayLayout.Override.ActiveRowAppearance = appearance90;
			comboBoxAssembly.DisplayLayout.Override.BorderStyleCell = Infragistics.Win.UIElementBorderStyle.Dotted;
			comboBoxAssembly.DisplayLayout.Override.BorderStyleRow = Infragistics.Win.UIElementBorderStyle.Dotted;
			appearance91.BackColor = System.Drawing.SystemColors.Window;
			comboBoxAssembly.DisplayLayout.Override.CardAreaAppearance = appearance91;
			...
			comboBoxAssembly.DisplayLayout.Override.CellAppearance = appearance92;
			comboBoxAssembly.DisplayLayout.Override.CellClickAction = Infragistics.Win.UltraWinGrid.CellClickAction.EditAndSelectText;
			comboBoxAssembly.DisplayLayout.Override.CellPadding = 0;
			...
			comboBoxAssembly.DisplayLayout.Override.GroupByRowAppearance = appearance93;
			appearance94.TextHAlignAsString = "Left";
			comboBoxAssembly.DisplayLayout.Override.HeaderAppearance = appearance94;
			comboBoxAssembly.DisplayLayout.Override.HeaderClickAction = Infragistics.Win.UltraWinGrid.HeaderClickAction.SortMulti;
			comboBoxAssembly.DisplayLayout.Override.HeaderStyle = Infragistics.Win.HeaderStyle.WindowsXPCommand;
			appearance95.BackColor = System.Drawing.SystemColors.Window;
			appearance95.BorderColor = System.Drawing.Color.Silver;
			comboBoxAssembly.DisplayLayout.Override.RowAppearance = appearance95;
			comboBoxAssembly.DisplayLayout.Override.RowSelectors = Infragistics.Win.DefaultableBoolean.False;
			appearance96.BackColor = System.Drawing.SystemColors.ControlLight;
			comboBoxAssembly.DisplayLayout.Override.TemplateAddRowAppearance = appearance96;
			comboBoxAssembly.DisplayLayout.ScrollBounds = Infragistics.Win.UltraWinGrid.ScrollBounds.ScrollToFill;
			comboBoxAssembly.DisplayLayout.ScrollStyle = Infragistics.Win.UltraWinGrid.ScrollStyle.Immediate;
			comboBoxAssembly.DisplayLayout.ViewStyleBand = Infragistics.Win.UltraWinGrid.ViewStyleBand.OutlookGroupBy;
			comboBoxAssembly.DropDownSearchMethod = Infragistics.Win.UltraWinGrid.DropDownSearchMethod.Linear;
			comboBoxAssembly.Editable = true;
			comboBoxAssembly.FilterCustomerID = "";
			comboBoxAssembly.FilterString = "";
			comboBoxAssembly.FilterSysDocID = "";
			comboBoxAssembly.HasAllAccount = false;
			comboBoxAssembly.HasCustom = false;
			comboBoxAssembly.IsDataLoaded = false;
			comboBoxAssembly.Location = new System.Drawing.Point(93, 26);
			comboBoxAssembly.MaxDropDownItems = 12;
			comboBoxAssembly.Name = "comboBoxAssembly";
			comboBoxAssembly.Show3PLItems = true;
			comboBoxAssembly.ShowInactiveItems = false;
			comboBoxAssembly.ShowQuickAdd = true;
			comboBoxAssembly.Size = new System.Drawing.Size(193, 20);
			comboBoxAssembly.TabIndex = 3;
			comboBoxAssembly.UseFlatMode = Infragistics.Win.DefaultableBoolean.True;
			textBoxAssemblyName.Location = new System.Drawing.Point(289, 26);
			textBoxAssemblyName.MaxLength = 255;
			textBoxAssemblyName.Name = "textBoxAssemblyName";
			textBoxAssemblyName.ReadOnly = true;
			textBoxAssemblyName.Size = new System.Drawing.Size(348, 20);
			...
			comboBoxBOM.Location = new System.Drawing.Point(264, 48);
			...
			comboBoxBOM.Size = new System.Drawing.Size(161, 20);
			...
			comboBoxLocation.DisplayLayout.Appearance = appearance99;
			...
			comboBoxLocation.DisplayLayout.GroupByBox.Appearance = appearance100;
			appearance101.ForeColor = System.Drawing.SystemColors.GrayText;
			comboBoxLocation.DisplayLayout.GroupByBox.BandLabelAppearance = appearance101;
			...
			comboBoxLocation.DisplayLayout.GroupByBox.PromptAppearance = appearance102;
			...
			comboBoxLocation.DisplayLayout.Override.ActiveCellAppearance = appearance103;
			...
			comboBoxLocation.DisplayLayout.Override.ActiveRowAppearance = appearance104;
			...
			comboBoxLocation.DisplayLayout.Override.CardAreaAppearance = appearance105;
			...
			comboBoxLocation.DisplayLayout.Override.CellAppearance = appearance106;
			...
			comboBoxLocation.DisplayLayout.Override.GroupByRowAppearance = appearance107;
			appearance108.TextHAlignAsString = "Left";
			comboBoxLocation.DisplayLayout.Override.HeaderAppearance = appearance108;
			...
			comboBoxLocation.DisplayLayout.Override.RowAppearance = appearance109;
			...
			comboBoxLocation.DisplayLayout.Override.TemplateAddRowAppearance = appearance110;
			...
			comboBoxLocation.Location = new System.Drawing.Point(93, 48);
			...
			ultraTabControl1.Location = new System.Drawing.Point(12, 179);
			ultraTabControl1.Name = "ultraTabControl1";
			ultraTabControl1.SharedControlsPage = ultraTabSharedControlsPage1;
			ultraTabControl1.Size = new System.Drawing.Size(712, 268);
			...
			ultraTabSharedControlsPage1.Size = new System.Drawing.Size(708, 242);
			...
			((System.ComponentModel.ISupportInitialize)comboBoxAssembly).EndInit();
