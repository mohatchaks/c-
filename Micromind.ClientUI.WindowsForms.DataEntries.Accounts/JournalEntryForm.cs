...
{
	public class JournalEntryForm : Form, IForm, IWorkFlowForm
	{
		...
		private CompanyDivisionComboBox comboBoxGridDivision;
		...
		private void comboBoxSysDoc_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			if (!string.IsNullOrEmpty(comboBoxSysDoc.SelectedID))
			{
				dataGridItems.DisplayLayout.Bands[0].Columns["Division"].DefaultCellValue = comboBoxSysDoc.DivisionID;
			}
		}
		...
		private bool GetData()
		{
			try
			{
				...
				DataRow dataRow = (!isNewRecord) ? currentData.JournalTable.Rows[0] : currentData.JournalTable.NewRow();
				dataRow["JournalID"] = 0;
				dataRow["JournalDate"] = dateTimePickerDate.Value;
				dataRow["SysDocID"] = comboBoxSysDoc.SelectedID;
				dataRow["VoucherID"] = textBoxVoucherNumber.Text;
				dataRow["SysDocType"] = 1.ToString();
				...
				if (IsNewRecord)
				{
					currentData.JournalTable.Rows.Add(dataRow);
				}
				currentData.JournalDetailsTable.Rows.Clear();
				foreach (UltraGridRow row in dataGridItems.Rows)
				{
					DataRow dataRow2 = currentData.JournalDetailsTable.NewRow();
					dataRow2.BeginEdit();
					...
					if (currencySelector.SelectedID == "" || currencySelector.IsBaseCurrency)
					{
						...
						dataRow2["DebitFC"] = DBNull.Value;
						dataRow2["CreditFC"] = DBNull.Value;
					}
					else
					{
						if (row.Cells["Debit"].Value.ToString() != "")
						{
							dataRow2["DebitFC"] = row.Cells["Debit"].Value.ToString();
						}
						else
						{
							dataRow2["DebitFC"] = DBNull.Value;
						...
						if (row.Cells["Credit"].Value.ToString() != "")
						{
							dataRow2["CreditFC"] = row.Cells["Credit"].Value.ToString();
						}
						else
						{
							dataRow2["CreditFC"] = DBNull.Value;
					...
					if (row.Cells["Division"].Value.ToString() != "")
					{
						dataRow2["DivisionID"] = row.Cells["Division"].Value.ToString();
					}
					else
					{
						dataRow2["DivisionID"] = comboBoxSysDoc.DivisionID;
					}
					dataRow2["CompanyID"] = Global.CompanyID;
					dataRow2["RowIndex"] = row.Index;
					dataRow2.EndEdit();
					currentData.JournalDetailsTable.Rows.Add(dataRow2);
				}
				return true;
			}
			catch (Exception e)
		...
		private void SetupGrid()
		{
			try
			{
				...
				UltraGridColumn ultraGridColumn2 = dataGridItems.DisplayLayout.Bands[0].Columns["AttributeID2"];
				bool flag2 = dataGridItems.DisplayLayout.Bands[0].Columns["Division"].Hidden = true;
				...
				dataGridItems.DisplayLayout.Bands[0].Columns["Division"].Header.Caption = "Division";
				dataGridItems.DisplayLayout.Bands[0].Columns["Division"].CharacterCasing = CharacterCasing.Upper;
				dataGridItems.DisplayLayout.Bands[0].Columns["Division"].MaxLength = 64;
				dataGridItems.DisplayLayout.Bands[0].Columns["Division"].Width = 75;
				dataGridItems.DisplayLayout.Bands[0].Columns["Division"].InvalidValueBehavior = InvalidValueBehavior.RetainValueAndFocus;
				dataGridItems.DisplayLayout.Bands[0].Columns["Division"].Style = Infragistics.Win.UltraWinGrid.ColumnStyle.DropDownValidate;
				dataGridItems.DisplayLayout.Bands[0].Columns["Division"].ValueList = comboBoxGridDivision;
		...
		public void LoadData(string journalID)
		{
			try
			{
				...
				{
					currentData = Factory.JournalSystem.GetJournalVoucherByID(SystemDocID, journalID);
					if (currentData != null)
					{
						FillData();
						IsNewRecord = false;
		...
		private void FillData()
		{
			try
			{
				if (currentData != null && currentData.Tables.Count != 0 && currentData.Tables[0].Rows.Count != 0)
				{
					...
					dateTimePickerDate.Value = DateTime.Parse(dataRow["JournalDate"].ToString());
					...
					if (currentData.Tables.Contains("Journal_Details") && currentData.JournalDetailsTable.Rows.Count != 0)
					{
						foreach (DataRow row in currentData.Tables["Journal_Details"].Rows)
						{
							...
							{
								dataRow3["Debit"] = Math.Round(decimal.Parse(row["DebitFC"].ToString()), Global.CurDecimalPoints);
							}
							else if (row["Debit"] != DBNull.Value)
							{
							...
							{
								dataRow3["Credit"] = Math.Round(decimal.Parse(row["CreditFC"].ToString()), Global.CurDecimalPoints);
							}
							else if (row["Credit"] != DBNull.Value)
							{
							...
							dataRow3["Division"] = row["DivisionID"].ToString();
		...
		private bool SaveData(bool clearAfter)
		{
			...
			try
			{
				bool flag2 = (!isNewRecord) ? Factory.JournalSystem.UpdateJournalVoucher(currentData) : Factory.JournalSystem.CreateJournalVoucher(currentData);
				if (flag2)
				{
					flag = true;
				}
		...
		private bool ValidateData()
		{
			...
			if (!IsNewRecord && !Global.IsUserAdmin && !AllowEditTransaction && Global.CurrentUser != Factory.SystemDocumentSystem.GetTransUserID("Journal", "VoucherID", SystemDocID, textBoxVoucherNumber.Text))
			{
			...
			if (formManager.IsFieldDirty(textBoxVoucherNumber) && Factory.SystemDocumentSystem.ExistDocumentNumber("Journal", "VoucherID", SystemDocID, textBoxVoucherNumber.Text))
			{
				ErrorHelper.WarningMessage(UIMessages.DocumentNumberInUse);
		...
		private bool Delete()
		{
			try
			{
				...
				return Factory.JournalSystem.DeleteJournalVoucher(SystemDocID, textBoxVoucherNumber.Text);
			}
			catch (Exception e)
			{
				ErrorHelper.ProcessError(e);
		...
		{
			string nextID = DatabaseHelper.GetNextID("Journal", "VoucherID", textBoxVoucherNumber.Text, "SysDocID", SystemDocID);
			if (!(nextID == ""))
			{
				LoadData(nextID);
			}
		...
		{
			string previousID = DatabaseHelper.GetPreviousID("Journal", "VoucherID", textBoxVoucherNumber.Text, "SysDocID", SystemDocID);
			if (!(previousID == ""))
			{
				LoadData(previousID);
			}
		...
		{
			string lastID = DatabaseHelper.GetLastID("Journal", "VoucherID", "SysDocID", SystemDocID);
			if (!(lastID == ""))
			{
				LoadData(lastID);
			}
		...
		{
			string firstID = DatabaseHelper.GetFirstID("Journal", "VoucherID", "SysDocID", SystemDocID);
			if (!(firstID == ""))
			{
				LoadData(firstID);
			}
		...
		private void toolStripButtonFind_Click(object sender, EventArgs e)
		{
			try
			{
				...
				{
					string text = Factory.DatabaseSystem.FindDocumentByNumber("Journal", "VoucherID", SystemDocID, toolStripTextBoxFind.Text);
					if (text != "")
					{
						LoadData(text);
					}
		...
		private void Print(bool isPrint, bool showPrintDialog, bool saveChanges)
		{
			try
			{
				if (!(IsDirty && saveChanges) || (ErrorHelper.QuestionMessage(MessageBoxButtons.YesNo, "You must save the document before printing.", "Do you want to save?") == DialogResult.Yes && SaveData(clearAfter: false)))
				{
					...
					DataSet journalToPrint = Factory.JournalSystem.GetJournalToPrint(systemDocID, text);
					...
					{
						PrintHelper.PrintDocument(journalToPrint, systemDocID, "Journal Entry", SysDocTypes.GJournal, isPrint, showPrintDialog);
					}
				}
			}
			catch (Exception e)
		...
		{
			FormActivator.BringFormToFront(FormActivator.JournalListFormObj);
		}
		...
		private void reverseToolStripMenuItem_Click(object sender, EventArgs e)
		{
			if (ErrorHelper.QuestionMessageYesNo("Are you sure to create a reverse copy of this document?") == DialogResult.Yes)
			{
				...
				foreach (UltraGridRow row2 in dataGridItems.Rows)
				{
					...
					if (row2.Cells["Debit"].Value != null && row2.Cells["Debit"].Value.ToString() != "")
					{
						num2 = decimal.Parse(row2.Cells["Debit"].Value.ToString());
					}
					if (row2.Cells["Credit"].Value != null && row2.Cells["Credit"].Value.ToString() != "")
					{
						num3 = decimal.Parse(row2.Cells["Credit"].Value.ToString());
					}
					if (num3 == 0m)
					{
						dataGridItems.Rows[num].Cells["Debit"].Value = DBNull.Value;
					}
					else
					{
						dataGridItems.Rows[num].Cells["Debit"].Value = num3;
					}
					if (num2 == 0m)
					{
						dataGridItems.Rows[num].Cells["Credit"].Value = DBNull.Value;
					}
					else
					{
						dataGridItems.Rows[num].Cells["Credit"].Value = num2;
					}
					num = checked(num + 1);
				}
			}
		...
		public void ShowForApproval(string sysDocID, string voucherID, int approvalTaskID)
		{
			...
			formManager.ShowApprovalPanel(approvalTaskID, "Journal", "VoucherID");
		}
		...
		private void toolStripButtonExcelImport_Click(object sender, EventArgs e)
		{
			...
			foreach (UltraGridRow row in dataGridItems.Rows)
			{
				if (row.Cells["Account Code"].Value.ToString() != "")
				{
					string text = row.Cells["Account Code"].Value.ToString();
					if (Factory.DatabaseSystem.ExistFieldValue("Account", "AccountID", text))
					{
						string value = Factory.DatabaseSystem.GetFieldValue("Account", "AccountName", "AccountID", text).ToString() ?? null;
						row.Cells["Account Name"].Value = value;
		...
		private void InitializeComponent()
		{
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Micromind.ClientUI.WindowsForms.DataEntries.Accounts.JournalEntryForm));
			...
			comboBoxGridDivision = new Micromind.DataControls.CompanyDivisionComboBox();
			...
			((System.ComponentModel.ISupportInitialize)comboBoxGridDivision).BeginInit();
			SuspendLayout();
			...
			labelCurrency.Location = new System.Drawing.Point(15, 49);
			...
			ultraFormattedLinkLabel2.Location = new System.Drawing.Point(271, 4);
			...
			panelDetails.Size = new System.Drawing.Size(655, 95);
			...
			currencySelector.Location = new System.Drawing.Point(123, 47);
			...
			mmLabel1.Location = new System.Drawing.Point(15, 27);
			...
			comboBoxGridCostCenter.Location = new System.Drawing.Point(672, 25);
			...
			dataGridItems.Size = new System.Drawing.Size(714, 302);
			...
			comboBoxPropertyUnit.Location = new System.Drawing.Point(343, 260);
			...
			comboBoxProperty.Location = new System.Drawing.Point(298, 219);
			...
			comboBoxGridDivision.Assigned = false;
			comboBoxGridDivision.AutoCompleteMode = Infragistics.Win.AutoCompleteMode.Append;
			comboBoxGridDivision.CharacterCasing = System.Windows.Forms.CharacterCasing.Upper;
			comboBoxGridDivision.CustomReportFieldName = "";
			comboBoxGridDivision.CustomReportKey = "";
			comboBoxGridDivision.CustomReportValueType = 1;
			comboBoxGridDivision.DescriptionTextBox = null;
			...
			comboBoxGridDivision.DisplayLayout.Appearance = appearance154;
			comboBoxGridDivision.DisplayLayout.BorderStyle = Infragistics.Win.UIElementBorderStyle.Solid;
			comboBoxGridDivision.DisplayLayout.CaptionVisible = Infragistics.Win.DefaultableBoolean.False;
			...
			comboBoxGridDivision.DisplayLayout.GroupByBox.Appearance = appearance155;
			appearance156.ForeColor = System.Drawing.SystemColors.GrayText;
			comboBoxGridDivision.DisplayLayout.GroupByBox.BandLabelAppearance = appearance156;
			comboBoxGridDivision.DisplayLayout.GroupByBox.BorderStyle = Infragistics.Win.UIElementBorderStyle.Solid;
			...
			comboBoxGridDivision.DisplayLayout.GroupByBox.PromptAppearance = appearance157;
			comboBoxGridDivision.DisplayLayout.MaxColScrollRegions = 1;
			comboBoxGridDivision.DisplayLayout.MaxRowScrollRegions = 1;
			...
			comboBoxGridDivision.DisplayLayout.Override.ActiveCellAppearance = appearance158;
			...
			comboBoxGridDivision.DisplayLayout.Override.ActiveRowAppearance = appearance159;
			comboBoxGridDivision.DisplayLayout.Override.BorderStyleCell = Infragistics.Win.UIElementBorderStyle.Dotted;
			comboBoxGridDivision.DisplayLayout.Override.BorderStyleRow = Infragistics.Win.UIElementBorderStyle.Dotted;
			appearance160.BackColor = System.Drawing.SystemColors.Window;
			comboBoxGridDivision.DisplayLayout.Override.CardAreaAppearance = appearance160;
			...
			comboBoxGridDivision.DisplayLayout.Override.CellAppearance = appearance161;
			comboBoxGridDivision.DisplayLayout.Override.CellClickAction = Infragistics.Win.UltraWinGrid.CellClickAction.EditAndSelectText;
			comboBoxGridDivision.DisplayLayout.Override.CellPadding = 0;
			...
			comboBoxGridDivision.DisplayLayout.Override.GroupByRowAppearance = appearance162;
			appearance163.TextHAlignAsString = "Left";
			comboBoxGridDivision.DisplayLayout.Override.HeaderAppearance = appearance163;
			comboBoxGridDivision.DisplayLayout.Override.HeaderClickAction = Infragistics.Win.UltraWinGrid.HeaderClickAction.SortMulti;
			comboBoxGridDivision.DisplayLayout.Override.HeaderStyle = Infragistics.Win.HeaderStyle.WindowsXPCommand;
			appearance164.BackColor = System.Drawing.SystemColors.Window;
			appearance164.BorderColor = System.Drawing.Color.Silver;
			comboBoxGridDivision.DisplayLayout.Override.RowAppearance = appearance164;
			comboBoxGridDivision.DisplayLayout.Override.RowSelectors = Infragistics.Win.DefaultableBoolean.False;
			appearance165.BackColor = System.Drawing.SystemColors.ControlLight;
			comboBoxGridDivision.DisplayLayout.Override.TemplateAddRowAppearance = appearance165;
			comboBoxGridDivision.DisplayLayout.ScrollBounds = Infragistics.Win.UltraWinGrid.ScrollBounds.ScrollToFill;
			comboBoxGridDivision.DisplayLayout.ScrollStyle = Infragistics.Win.UltraWinGrid.ScrollStyle.Immediate;
			comboBoxGridDivision.DisplayLayout.ViewStyleBand = Infragistics.Win.UltraWinGrid.ViewStyleBand.OutlookGroupBy;
			comboBoxGridDivision.DropDownSearchMethod = Infragistics.Win.UltraWinGrid.DropDownSearchMethod.Linear;
			comboBoxGridDivision.Editable = true;
			comboBoxGridDivision.FilterString = "";
			comboBoxGridDivision.HasAllAccount = false;
			comboBoxGridDivision.HasCustom = false;
			comboBoxGridDivision.IsDataLoaded = false;
			comboBoxGridDivision.Location = new System.Drawing.Point(506, 229);
			comboBoxGridDivision.MaxDropDownItems = 12;
			comboBoxGridDivision.Name = "comboBoxGridDivision";
			comboBoxGridDivision.ShowInactiveItems = false;
			comboBoxGridDivision.ShowQuickAdd = true;
			comboBoxGridDivision.Size = new System.Drawing.Size(100, 20);
			comboBoxGridDivision.TabIndex = 129;
			comboBoxGridDivision.UseFlatMode = Infragistics.Win.DefaultableBoolean.True;
			comboBoxGridDivision.Visible = false;
			base.AutoScaleDimensions = new System.Drawing.SizeF(6f, 13f);
			base.AutoScaleMode = System.Windows.Forms.AutoScaleMode.Font;
			BackColor = System.Drawing.Color.WhiteSmoke;
			base.ClientSize = new System.Drawing.Size(740, 498);
			base.Controls.Add(comboBoxGridDivision);
			...
			((System.ComponentModel.ISupportInitialize)comboBoxGridDivision).EndInit();
			ResumeLayout(false);
			PerformLayout();
		}
	}
