...
{
	public class AnalysisGroupDetailsForm : Form, IForm
	{
		private AnalysisGroupsData currentData;

		private const string TABLENAME_CONST = "Analysis_Group";
		...
		public AnalysisGroupDetailsForm()
		{
			InitializeComponent();
		}
		...
		private bool GetData()
		{
			try
			{
				...
				{
					currentData = new AnalysisGroupsData();
				}
				DataRow dataRow = (!isNewRecord) ? currentData.AnalysisGroupsTable.Rows[0] : currentData.AnalysisGroupsTable.NewRow();
				dataRow.BeginEdit();
				...
				if (isNewRecord)
				{
					currentData.AnalysisGroupsTable.Rows.Add(dataRow);
				}
				return true;
			}
			catch (Exception e)
		...
		public void LoadData(string id)
		{
			try
			{
				if (!(id.Trim() == "") && CanClose())
				{
					currentData = Factory.AnalysisGroupsSystem.GetAnalysisGroupByID(id.Trim());
		...
		private bool SaveData()
		{
			...
			try
			{
				...
				if (isNewRecord)
				{
					flag = Factory.AnalysisGroupsSystem.CreateAnalysisGroup(currentData);
					if (flag)
					{
						ComboDataHelper.SetRefreshStatus(DataComboType.AnalysisGroup, needRefresh: true);
					}
				}
				else
				{
					flag = Factory.AnalysisGroupsSystem.UpdateAnalysisGroup(currentData);
				}
				if (!flag)
				{
		...
		private bool ValidateData()
		{
			...
			if (!IsNewRecord && !Global.IsUserAdmin && !AllowEditCard && Global.CurrentUser != Factory.SystemDocumentSystem.GetCardUserID("Analysis_Group", "GroupID", textBoxCode.Text))
			{
			...
			if (isNewRecord && Factory.DatabaseSystem.ExistFieldValue("Analysis_Group", "GroupID", textBoxCode.Text.Trim()))
			{
		...
		{
			textBoxCode.Text = PublicFunctions.GetNextCardNumber("Analysis_Group", "GroupID");
			textBoxName.Clear();
			textBoxNote.Clear();
		...
		private bool Delete()
		{
			try
			{
				...
				return Factory.AnalysisGroupsSystem.DeleteAnalysisGroup(textBoxCode.Text);
			}
			catch (Exception e)
			{
				ErrorHelper.ProcessError(e);
		...
		{
			LoadData(DatabaseHelper.GetNextID("Analysis_Group", "GroupID", textBoxCode.Text));
		}

		private void toolStripButtonPrevious_Click(object sender, EventArgs e)
		{
			LoadData(DatabaseHelper.GetPreviousID("Analysis_Group", "GroupID", textBoxCode.Text));
		}

		private void toolStripButtonLast_Click(object sender, EventArgs e)
		{
			LoadData(DatabaseHelper.GetLastID("Analysis_Group", "GroupID"));
		}

		private void toolStripButtonFirst_Click(object sender, EventArgs e)
		{
			LoadData(DatabaseHelper.GetFirstID("Analysis_Group", "GroupID"));
		}
		...
		private void toolStripButtonFind_Click(object sender, EventArgs e)
		{
			try
			{
				...
				else if (Factory.DatabaseSystem.ExistFieldValue("Analysis_Group", "GroupID", toolStripTextBoxFind.Text.Trim()))
				{
		...
		{
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Micromind.ClientUI.WindowsForms.DataEntries.Accounts.AnalysisGroupDetailsForm));
			...
			base.Name = "AnalysisGroupDetailsForm";
			Text = "Analysis Group";
