...
{
	public class SalaryAdditionForm : Form, IForm
	{
		private bool supressExpenseMessage;

		private SalaryData currentData;

		private const string TABLENAME_CONST = "Salary_Addition";
		...
		private BenefitComboBox comboBoxAddition;
		...
		private ToolStripMenuItem loadTicketAmountToolStripMenuItem;

		private ToolStripMenuItem loadPayrollItemToolStripMenuItem;

		private ToolStripMenuItem loadLeaveSalaryTakenLeaveToolStripMenuItem;
		...
		private ToolStripMenuItem loadEmployeeToolStripMenuItem;
			...
			checked
			{
				try
				{
					if (dataGridItems.ActiveRow != null && dataGridItems.ActiveRow != null)
					{
						...
						if (key == "Addition Code")
						{
							for (int i = 1; i < dataGridItems.Rows.Count; i++)
							{
								_ = dataGridItems.Rows[i];
								if ((dataGridItems.Rows[i].Cells["Addition Code"].Value == null || dataGridItems.Rows[i].Cells["Addition Code"].Value.ToString() == "") && dataGridItems.Rows[i].Index > 0)
								{
									dataGridItems.Rows[i].Cells["Addition Code"].Value = dataGridItems.Rows[i - 1].Cells["Addition Code"].Value;
								}
							}
						}
		...
		private void dataGrid_BeforeCellDeactivate(object sender, CancelEventArgs e)
		{
			...
			checked
			{
				if (dataGridItems.ActiveCell.Column.Key.ToString() == "Amount" && isNewRecord && IsChangedallowed)
				{
		...
		private bool GetData()
		{
			try
			{
				...
				DataRow dataRow = (!isNewRecord) ? currentData.SalaryAdditionTable.Rows[0] : currentData.SalaryAdditionTable.NewRow();
				...
				if (IsNewRecord)
				{
					currentData.SalaryAdditionTable.Rows.Add(dataRow);
				}
				currentData.SalaryAdditionDetailTable.Rows.Clear();
				foreach (UltraGridRow row in dataGridItems.Rows)
				{
					DataRow dataRow2 = currentData.SalaryAdditionDetailTable.NewRow();
					dataRow2.BeginEdit();
					...
					dataRow2["AdditionCode"] = row.Cells["Addition Code"].Value.ToString();
					...
					if (amountType != "")
					{
						dataRow2["AmountType"] = amountType;
					}
					else
					{
						dataRow2["AmountType"] = DBNull.Value;
					}
					...
					dataRow2.EndEdit();
					if (row.Cells["Amount"].Value.ToString() != "0.0000")
					{
						currentData.SalaryAdditionDetailTable.Rows.Add(dataRow2);
					}
				}
				return true;
			}
		...
		private void FillData()
		{
			try
			{
				if (currentData != null && currentData.Tables.Count != 0 && currentData.Tables["Salary_Addition"].Rows.Count != 0)
				{
					DataRow dataRow = currentData.Tables["Salary_Addition"].Rows[0];
					...
					if (currentData.Tables.Contains("Salary_Addition_Detail") && currentData.SalaryAdditionDetailTable.Rows.Count != 0)
					{
						if (currentData.SalaryAdditionDetailTable.DefaultView.ToTable(true, "EmployeeID").Rows.Count == 1 && currentData.SalaryAdditionDetailTable.Rows.Count > 1)
						{
						...
						foreach (DataRow row in currentData.Tables["Salary_Addition_Detail"].Rows)
						{
							...
							dataRow3["Addition Code"] = row["AdditionCode"].ToString();
		...
		private void SetupGrid(bool status)
		{
			try
			{
				...
				dataTable.Columns.Add("Addition Code");
				...
				dataGridItems.DisplayLayout.Bands[0].Columns["Addition Code"].CharacterCasing = CharacterCasing.Upper;
				dataGridItems.DisplayLayout.Bands[0].Columns["Addition Code"].MaxLength = 15;
				dataGridItems.DisplayLayout.Bands[0].Columns["Addition Code"].InvalidValueBehavior = InvalidValueBehavior.RetainValueAndFocus;
				dataGridItems.DisplayLayout.Bands[0].Columns["Addition Code"].Style = Infragistics.Win.UltraWinGrid.ColumnStyle.DropDownValidate;
				dataGridItems.DisplayLayout.Bands[0].Columns["Addition Code"].ValueList = comboBoxAddition;
				...
				dataGridItems.DisplayLayout.Bands[0].Summaries["Amount"].DisplayFormat = Format.GridAmountFormat;
				if (!status)
				{
				...
				dataGridItems.DisplayLayout.Bands[0].Columns["Addition Code"].ExcludeFromColumnChooser = ExcludeFromColumnChooser.False;
		...
		public void LoadData(string voucherID)
		{
			try
			{
				...
				{
					currentData = Factory.SalarySystem.GetSalaryAdditionByID(voucherID);
					if (currentData != null)
					{
						FillData();
						IsNewRecord = false;
		...
		private bool SaveData(bool clearAfter)
		{
			...
			try
			{
				bool flag = Factory.SalarySystem.CreateSalaryAddition(currentData, !isNewRecord);
				if (!flag)
				{
		...
		private bool ValidateData()
		{
			...
			{
				_ = dataGridItems.Rows[i];
				if (dataGridItems.Rows[i].Cells["Addition Code"].Value.ToString() == "")
				{
					ErrorHelper.InformationMessage("Please select addition code for all the entries.");
		...
		private bool Delete()
		{
			try
			{
				...
				return Factory.SalarySystem.DeleteSalaryAddition(textBoxVoucherNumber.Text);
			}
			catch (Exception e)
			{
				ErrorHelper.ProcessError(e);
		...
		{
			string nextID = DatabaseHelper.GetNextID("Salary_Addition", "VoucherID", textBoxVoucherNumber.Text);
			if (!(nextID == ""))
			{
				LoadData(nextID);
			}
		...
		{
			string previousID = DatabaseHelper.GetPreviousID("Salary_Addition", "VoucherID", textBoxVoucherNumber.Text);
			if (!(previousID == ""))
			{
				LoadData(previousID);
			}
		...
		{
			string lastID = DatabaseHelper.GetLastID("Salary_Addition", "VoucherID");
			if (!(lastID == ""))
			{
				LoadData(lastID);
			}
		...
		{
			string firstID = DatabaseHelper.GetFirstID("Salary_Addition", "VoucherID");
			if (!(firstID == ""))
			{
				LoadData(firstID);
			}
		...
		private void toolStripButtonFind_Click(object sender, EventArgs e)
		{
			try
			{
				...
				{
					string text = Factory.DatabaseSystem.FindDocumentByNumber("Salary_Addition", "VoucherID", SystemDocID, toolStripTextBoxFind.Text);
					if (text != "")
					{
						LoadData(text);
					}
		...
		private void JournalLeavesForm_Load(object sender, EventArgs e)
		{
			try
			{
				...
				if (checkBoxMultiPeriod.Checked)
				{
					...
					bool flag2 = LabelNoofpayment.Visible = true;
					bool flag4 = labelNoofmonth.Visible = flag2;
					bool visible = numericnoofpayments.Visible = flag4;
		...
		private string GetNextVoucherNumber()
		{
			try
			{
				return Factory.SystemDocumentSystem.GetNextDocumentNumber("Salary_Addition", "VoucherID");
			}
			catch (Exception e)
			{
				ErrorHelper.ProcessError(e);
		...
		private void Print(bool isPrint, bool showPrintDialog, bool saveChanges)
		{
			try
			{
				if (!(IsDirty && saveChanges) || (ErrorHelper.QuestionMessage(MessageBoxButtons.YesNo, "You must save the document before printing.", "Do you want to save?") == DialogResult.Yes && SaveData(clearAfter: false)))
				{
					...
					DataSet salaryAdditionToPrint = Factory.SalarySystem.GetSalaryAdditionToPrint(text);
					if (salaryAdditionToPrint == null || salaryAdditionToPrint.Tables.Count == 0)
					{
						ErrorHelper.ErrorMessage("Cannot print the document.", "Document not found.");
					}
					else
					{
						PrintHelper.PrintDocument(salaryAdditionToPrint, "", "Salary Addition", SysDocTypes.None, isPrint, showPrintDialog);
					}
				}
			}
			catch (Exception e)
		...
		private void CalculateMultiperiod()
		{
			...
			text = ultraGridRow.Cells["Addition Code"].Value.ToString();
		...
		private void toolStripButtonAttach_Click(object sender, EventArgs e)
		{
			try
			{
				if (!isNewRecord)
				{
					...
					docManagementForm.EntitySysDocID = "SAL_ADD";
					docManagementForm.EntityName = "SAL_ADD";
		...
		private void loadTicketAmountToolStripMenuItem_Click(object sender, EventArgs e)
		{
			IsNewRecord = true;
			CalculateSalaryForm calculateSalaryForm = new CalculateSalaryForm();
			if (calculateSalaryForm.ShowDialog() == DialogResult.OK)
			{
				DataSet dataSet = Factory.SalarySystem.LoadTicketAmount(calculateSalaryForm.FromEmployee, calculateSalaryForm.ToEmployee, calculateSalaryForm.FromDepartment, calculateSalaryForm.ToDepartment, calculateSalaryForm.FromLocation, calculateSalaryForm.ToLocation, calculateSalaryForm.FromType, calculateSalaryForm.ToType, calculateSalaryForm.FromDivision, calculateSalaryForm.ToDivision, calculateSalaryForm.FromSponsor, calculateSalaryForm.ToSponsor, calculateSalaryForm.FromGroup, calculateSalaryForm.ToGroup, calculateSalaryForm.FromGrade, calculateSalaryForm.ToGrade, calculateSalaryForm.FromPosition, calculateSalaryForm.ToPosition, calculateSalaryForm.FromBank, calculateSalaryForm.ToBank, calculateSalaryForm.FromAccount, calculateSalaryForm.ToAccount, dateTimePickerDate.Value, calculateSalaryForm.MultipleEmployees);
		...
		private void loadPayrollItemToolStripMenuItem_Click(object sender, EventArgs e)
		{
			IsNewRecord = true;
			CalculateSalaryForm calculateSalaryForm = new CalculateSalaryForm();
			if (calculateSalaryForm.ShowDialog() == DialogResult.OK)
			{
				DataSet dataSet = Factory.SalarySystem.LoadPayrollItem(calculateSalaryForm.FromEmployee, calculateSalaryForm.ToEmployee, calculateSalaryForm.FromDepartment, calculateSalaryForm.ToDepartment, calculateSalaryForm.FromLocation, calculateSalaryForm.ToLocation, calculateSalaryForm.FromType, calculateSalaryForm.ToType, calculateSalaryForm.FromDivision, calculateSalaryForm.ToDivision, calculateSalaryForm.FromSponsor, calculateSalaryForm.ToSponsor, calculateSalaryForm.FromGroup, calculateSalaryForm.ToGroup, calculateSalaryForm.FromGrade, calculateSalaryForm.ToGrade, calculateSalaryForm.FromPosition, calculateSalaryForm.ToPosition, calculateSalaryForm.FromBank, calculateSalaryForm.ToBank, calculateSalaryForm.FromAccount, calculateSalaryForm.ToAccount, dateTimePickerDate.Value, calculateSalaryForm.MultipleEmployees, "");
				if (dataSet != null && dataSet.Tables.Count != 0 && dataSet.Tables[0].Rows.Count != 0)
				{
					IsChangedallowed = false;
					amountType = "L";
		...
		private void loadLeaveSalaryTakenLeaveToolStripMenuItem_Click(object sender, EventArgs e)
		{
			IsNewRecord = true;
			CalculateSalaryForm calculateSalaryForm = new CalculateSalaryForm();
			if (calculateSalaryForm.ShowDialog() == DialogResult.OK)
			{
				DateTime value = dateTimePickerDate.Value;
				int day = new DateTime(value.Year, value.Month, 1).AddMonths(1).AddDays(-1.0).Day;
				DateTime startDate = value.AddDays(checked(1 - value.Day));
				DateTime endDate = new DateTime(value.Year, value.Month, 1).AddMonths(1).AddDays(-1.0);
				DataSet dataSet = Factory.SalarySheetSystem.CalculateSalarySheet(calculateSalaryForm.FromEmployee, calculateSalaryForm.ToEmployee, calculateSalaryForm.FromDepartment, calculateSalaryForm.ToDepartment, calculateSalaryForm.FromLocation, calculateSalaryForm.ToLocation, calculateSalaryForm.FromType, calculateSalaryForm.ToType, calculateSalaryForm.FromDivision, calculateSalaryForm.ToDivision, calculateSalaryForm.FromSponsor, calculateSalaryForm.ToSponsor, calculateSalaryForm.FromGroup, calculateSalaryForm.ToGroup, calculateSalaryForm.FromGrade, calculateSalaryForm.ToGrade, calculateSalaryForm.FromPosition, calculateSalaryForm.ToPosition, calculateSalaryForm.FromBank, calculateSalaryForm.ToBank, calculateSalaryForm.FromAccount, calculateSalaryForm.ToAccount, startDate, endDate, value.Year, value.Month, calculateSalaryForm.MultipleEmployees);
				decimal result = default(decimal);
				decimal result2 = default(decimal);
				decimal result3 = default(decimal);
				...
				if (dataSet != null && dataSet.Tables.Count != 0 && dataSet.Tables[0].Rows.Count != 0)
				{
					...
					{
						DataRow dataRow2 = dataTable.NewRow();
						decimal.TryParse(row["AnnualLeaves"].ToString(), out result2);
						if (row.Table.Columns.Contains("ToLessTakenAbs"))
						{
							decimal.TryParse(row["ToLessTakenAbs"].ToString(), out result);
						}
						if (row.Table.Columns.Contains("ToLessTakenAnn"))
						{
							decimal.TryParse(row["ToLessTakenAnn"].ToString(), out result3);
						}
						result2 -= result3;
						if (!(result2 == 0m))
						{
							num = Factory.SalarySystem.GetTotalLeaveSalary(row["EmployeeID"].ToString());
							if (!checkBoxMultiPeriod.Checked)
							{
								dataRow2["EmpNo"] = row["EmployeeID"].ToString();
								dataRow2["EmpName"] = row["EmployeeName"].ToString();
							}
							dataRow2["Remarks"] = "Annual Leave Salary";
							if (row["GrossSalary"] != DBNull.Value)
							{
								dataRow2["Amount"] = decimal.Parse((num / (decimal)day * result2).ToString()).ToString(Format.TotalAmountFormat);
							}
							else
							{
								dataRow2["Amount"] = DBNull.Value;
		...
		{
			FormActivator.BringFormToFront(FormActivator.SalaryAdditionListFormObj);
		}
		...
		private void loadEmployeeToolStripMenuItem_Click(object sender, EventArgs e)
		{
		...
		private void ShowEmployeeSelector()
		{
			...
			ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof(SalaryAdditionForm));
		...
		private void InitializeComponent()
		{
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Micromind.ClientUI.WindowsForms.DataEntries.Projects.SalaryAdditionForm));
			...
			loadEmployeeToolStripMenuItem = new System.Windows.Forms.ToolStripMenuItem();
			loadTicketAmountToolStripMenuItem = new System.Windows.Forms.ToolStripMenuItem();
			loadPayrollItemToolStripMenuItem = new System.Windows.Forms.ToolStripMenuItem();
			loadLeaveSalaryTakenLeaveToolStripMenuItem = new System.Windows.Forms.ToolStripMenuItem();
			...
			comboBoxAddition = new Micromind.DataControls.BenefitComboBox();
			...
			((System.ComponentModel.ISupportInitialize)comboBoxAddition).BeginInit();
			SuspendLayout();
			...
			toolStripDropDownButton1.DropDownItems.AddRange(new System.Windows.Forms.ToolStripItem[6]
			{
				...
				toolStripSeparator6,
				loadEmployeeToolStripMenuItem,
				loadTicketAmountToolStripMenuItem,
				loadPayrollItemToolStripMenuItem,
				loadLeaveSalaryTakenLeaveToolStripMenuItem
			});
			...
			loadEmployeeToolStripMenuItem.Name = "loadEmployeeToolStripMenuItem";
			loadEmployeeToolStripMenuItem.Size = new System.Drawing.Size(239, 22);
			loadEmployeeToolStripMenuItem.Text = "Load Employees";
			loadEmployeeToolStripMenuItem.Click += new System.EventHandler(loadEmployeeToolStripMenuItem_Click);
			loadTicketAmountToolStripMenuItem.Name = "loadTicketAmountToolStripMenuItem";
			loadTicketAmountToolStripMenuItem.Size = new System.Drawing.Size(239, 22);
			loadTicketAmountToolStripMenuItem.Text = "Load Ticket Amount";
			loadTicketAmountToolStripMenuItem.Click += new System.EventHandler(loadTicketAmountToolStripMenuItem_Click);
			loadPayrollItemToolStripMenuItem.Name = "loadPayrollItemToolStripMenuItem";
			loadPayrollItemToolStripMenuItem.Size = new System.Drawing.Size(239, 22);
			loadPayrollItemToolStripMenuItem.Text = "Load LeaveSalary";
			loadPayrollItemToolStripMenuItem.Click += new System.EventHandler(loadPayrollItemToolStripMenuItem_Click);
			loadLeaveSalaryTakenLeaveToolStripMenuItem.Name = "loadLeaveSalaryTakenLeaveToolStripMenuItem";
			loadLeaveSalaryTakenLeaveToolStripMenuItem.Size = new System.Drawing.Size(239, 22);
			loadLeaveSalaryTakenLeaveToolStripMenuItem.Text = "Load Leave Salary(Taken Leave)";
			loadLeaveSalaryTakenLeaveToolStripMenuItem.Click += new System.EventHandler(loadLeaveSalaryTakenLeaveToolStripMenuItem_Click);
			...
			dateTimePickerDate.Location = new System.Drawing.Point(293, 4);
			...
			panelDetails.Size = new System.Drawing.Size(855, 66);
			panelDetails.TabIndex = 0;
			appearance.FontData.BoldAsString = "True";
			appearance.FontData.Name = "Tahoma";
			LabelEmployee.Appearance = appearance;
			LabelEmployee.AutoSize = true;
			LabelEmployee.Location = new System.Drawing.Point(441, 6);
			LabelEmployee.Name = "LabelEmployee";
			LabelEmployee.Size = new System.Drawing.Size(62, 15);
			LabelEmployee.TabIndex = 160;
			...
			LabelEmployee.LinkClicked += new Infragistics.Win.FormattedLinkLabel.LinkClickedEventHandler(ultraFormattedLinkLabel1_LinkClicked);
			buttonFillDetails.Location = new System.Drawing.Point(735, 25);
			buttonFillDetails.Name = "buttonFillDetails";
			buttonFillDetails.Size = new System.Drawing.Size(99, 23);
			buttonFillDetails.TabIndex = 159;
			...
			LabelNoofpayment.Location = new System.Drawing.Point(572, 30);
			...
			LabelNoofmonth.Location = new System.Drawing.Point(438, 30);
			...
			linkLabelVoucherNumber.Location = new System.Drawing.Point(12, 7);
			...
			comboBoxEmployee.Location = new System.Drawing.Point(509, 3);
			...
			LabelPeriod.Location = new System.Drawing.Point(240, 7);
			...
			dataGridItems.Location = new System.Drawing.Point(11, 103);
			...
			dataGridItems.Size = new System.Drawing.Size(844, 379);
			...
			comboBoxAddition.Assigned = false;
			comboBoxAddition.AutoCompleteMode = Infragistics.Win.AutoCompleteMode.Append;
			comboBoxAddition.CalcManager = ultraCalcManager1;
			comboBoxAddition.CharacterCasing = System.Windows.Forms.CharacterCasing.Upper;
			comboBoxAddition.CustomReportFieldName = "";
			comboBoxAddition.CustomReportKey = "";
			comboBoxAddition.CustomReportValueType = 1;
			comboBoxAddition.DescriptionTextBox = null;
			appearance53.BackColor = System.Drawing.SystemColors.Window;
			appearance53.BorderColor = System.Drawing.SystemColors.InactiveCaption;
			comboBoxAddition.DisplayLayout.Appearance = appearance53;
			comboBoxAddition.DisplayLayout.BorderStyle = Infragistics.Win.UIElementBorderStyle.Solid;
			comboBoxAddition.DisplayLayout.CaptionVisible = Infragistics.Win.DefaultableBoolean.False;
			appearance54.BackColor = System.Drawing.SystemColors.ActiveBorder;
			appearance54.BackColor2 = System.Drawing.SystemColors.ControlDark;
			appearance54.BackGradientStyle = Infragistics.Win.GradientStyle.Vertical;
			appearance54.BorderColor = System.Drawing.SystemColors.Window;
			comboBoxAddition.DisplayLayout.GroupByBox.Appearance = appearance54;
			appearance55.ForeColor = System.Drawing.SystemColors.GrayText;
			comboBoxAddition.DisplayLayout.GroupByBox.BandLabelAppearance = appearance55;
			comboBoxAddition.DisplayLayout.GroupByBox.BorderStyle = Infragistics.Win.UIElementBorderStyle.Solid;
			...
			comboBoxAddition.DisplayLayout.GroupByBox.PromptAppearance = appearance56;
			comboBoxAddition.DisplayLayout.MaxColScrollRegions = 1;
			comboBoxAddition.DisplayLayout.MaxRowScrollRegions = 1;
			appearance57.BackColor = System.Drawing.SystemColors.Window;
			appearance57.ForeColor = System.Drawing.SystemColors.ControlText;
			comboBoxAddition.DisplayLayout.Override.ActiveCellAppearance = appearance57;
			...
			comboBoxAddition.DisplayLayout.Override.ActiveRowAppearance = appearance58;
			comboBoxAddition.DisplayLayout.Override.BorderStyleCell = Infragistics.Win.UIElementBorderStyle.Dotted;
			comboBoxAddition.DisplayLayout.Override.BorderStyleRow = Infragistics.Win.UIElementBorderStyle.Dotted;
			appearance59.BackColor = System.Drawing.SystemColors.Window;
			comboBoxAddition.DisplayLayout.Override.CardAreaAppearance = appearance59;
			...
			comboBoxAddition.DisplayLayout.Override.CellAppearance = appearance60;
			comboBoxAddition.DisplayLayout.Override.CellClickAction = Infragistics.Win.UltraWinGrid.CellClickAction.EditAndSelectText;
			comboBoxAddition.DisplayLayout.Override.CellPadding = 0;
			...
			comboBoxAddition.DisplayLayout.Override.GroupByRowAppearance = appearance61;
			appearance62.TextHAlignAsString = "Left";
			comboBoxAddition.DisplayLayout.Override.HeaderAppearance = appearance62;
			comboBoxAddition.DisplayLayout.Override.HeaderClickAction = Infragistics.Win.UltraWinGrid.HeaderClickAction.SortMulti;
			comboBoxAddition.DisplayLayout.Override.HeaderStyle = Infragistics.Win.HeaderStyle.WindowsXPCommand;
			appearance63.BackColor = System.Drawing.SystemColors.Window;
			appearance63.BorderColor = System.Drawing.Color.Silver;
			comboBoxAddition.DisplayLayout.Override.RowAppearance = appearance63;
			comboBoxAddition.DisplayLayout.Override.RowSelectors = Infragistics.Win.DefaultableBoolean.False;
			appearance64.BackColor = System.Drawing.SystemColors.ControlLight;
			comboBoxAddition.DisplayLayout.Override.TemplateAddRowAppearance = appearance64;
			comboBoxAddition.DisplayLayout.ScrollBounds = Infragistics.Win.UltraWinGrid.ScrollBounds.ScrollToFill;
			comboBoxAddition.DisplayLayout.ScrollStyle = Infragistics.Win.UltraWinGrid.ScrollStyle.Immediate;
			comboBoxAddition.DisplayLayout.ViewStyleBand = Infragistics.Win.UltraWinGrid.ViewStyleBand.OutlookGroupBy;
			comboBoxAddition.DropDownSearchMethod = Infragistics.Win.UltraWinGrid.DropDownSearchMethod.Linear;
			comboBoxAddition.Editable = true;
			comboBoxAddition.FilterString = "";
			comboBoxAddition.HasAllAccount = false;
			comboBoxAddition.HasCustom = false;
			comboBoxAddition.IsDataLoaded = false;
			comboBoxAddition.IsNonFinancial = false;
			comboBoxAddition.Location = new System.Drawing.Point(769, 300);
			comboBoxAddition.MaxDropDownItems = 12;
			comboBoxAddition.Name = "comboBoxAddition";
			comboBoxAddition.ShowInactiveItems = false;
			comboBoxAddition.ShowQuickAdd = true;
			comboBoxAddition.Size = new System.Drawing.Size(100, 20);
			comboBoxAddition.TabIndex = 132;
			comboBoxAddition.UseFlatMode = Infragistics.Win.DefaultableBoolean.True;
			comboBoxAddition.Visible = false;
			...
			base.Controls.Add(comboBoxAddition);
			base.Icon = (System.Drawing.Icon)resources.GetObject("$this.Icon");
			MinimumSize = new System.Drawing.Size(885, 595);
			base.Name = "SalaryAdditionForm";
			Text = "Salary Addition";
			...
			((System.ComponentModel.ISupportInitialize)comboBoxAddition).EndInit();
			ResumeLayout(false);
			PerformLayout();
		}
	}
