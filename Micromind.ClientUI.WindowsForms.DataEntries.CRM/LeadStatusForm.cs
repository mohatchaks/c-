...
{
	public class LeadStatusForm : Form, IForm
	{
		private LeadStatusData currentData;

		private const string TABLENAME_CONST = "Lead_Status";

		private const string IDFIELD_CONST = "LeadStatusID";
		...
		private bool GetData()
		{
			try
			{
				...
				{
					currentData = new LeadStatusData();
				}
				DataRow dataRow = (!isNewRecord) ? currentData.LeadStatusTable.Rows[0] : currentData.LeadStatusTable.NewRow();
				dataRow.BeginEdit();
				dataRow["LeadStatusID"] = textBoxCode.Text.Trim();
				...
				if (isNewRecord)
				{
					currentData.LeadStatusTable.Rows.Add(dataRow);
				}
				return true;
			}
			catch (Exception e)
		...
		public void LoadData(string id)
		{
			try
			{
				...
				{
					currentData = Factory.LeadStatusSystem.GetLeadStatusByID(id.Trim());
		...
		private void FillData()
		{
			try
			{
				if (currentData != null && currentData.Tables.Count != 0 && currentData.Tables[0].Rows.Count != 0)
				{
					...
					textBoxCode.Text = dataRow["LeadStatusID"].ToString();
		...
		private bool SaveData()
		{
			...
			try
			{
				...
				if (isNewRecord)
				{
					flag = Factory.LeadStatusSystem.CreateLeadStatus(currentData);
					if (flag)
					{
						ComboDataHelper.SetRefreshStatus(DataComboType.LeadStatus, needRefresh: true);
					}
				}
				else
				{
					flag = Factory.LeadStatusSystem.UpdateLeadStatus(currentData);
				}
				if (!flag)
				{
		...
		private bool ValidateData()
		{
			...
			if (isNewRecord && Factory.DatabaseSystem.ExistFieldValue("Lead_Status", "LeadStatusID", textBoxCode.Text.Trim()))
			{
		...
		{
			textBoxCode.Text = PublicFunctions.GetNextCardNumber("Lead_Status", "LeadStatusID");
			textBoxName.Clear();
			textBoxNote.Clear();
			checkBoxInactive.Checked = false;
			formManager.ResetDirty();
		...
		private bool Delete()
		{
			try
			{
				...
				bool num = Factory.LeadStatusSystem.DeleteLeadStatus(textBoxCode.Text);
				if (num)
				{
		...
		{
			LoadData(DatabaseHelper.GetNextID("Lead_Status", "LeadStatusID", textBoxCode.Text));
		}

		private void toolStripButtonPrevious_Click(object sender, EventArgs e)
		{
			LoadData(DatabaseHelper.GetPreviousID("Lead_Status", "LeadStatusID", textBoxCode.Text));
		}

		private void toolStripButtonLast_Click(object sender, EventArgs e)
		{
			LoadData(DatabaseHelper.GetLastID("Lead_Status", "LeadStatusID"));
		}

		private void toolStripButtonFirst_Click(object sender, EventArgs e)
		{
			LoadData(DatabaseHelper.GetFirstID("Lead_Status", "LeadStatusID"));
		}
		...
		private void toolStripButtonFind_Click(object sender, EventArgs e)
		{
			try
			{
				...
				else if (Factory.DatabaseSystem.ExistFieldValue("Lead_Status", "LeadStatusID", toolStripTextBoxFind.Text.Trim()))
				{
		...
		{
			new FormHelper().ShowList(DataComboType.LeadStatus);
		}
		...
		{
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Micromind.ClientUI.WindowsForms.DataEntries.CRM.LeadStatusForm));
			...
			panelButtons.Location = new System.Drawing.Point(0, 106);
			...
			base.ClientSize = new System.Drawing.Size(474, 146);
